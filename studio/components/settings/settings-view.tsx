...
function Section({
  ...
  return (
    <section id={id} aria-labelledby={`${id}-heading`} className="settings-section">
    ...
    <div className="settings-page">
      ...
      >
        ...
        {external ? (
          <div className="transport-note">
            <span>i</span>
            <p>
              Provider selection and credentials stay with the remote daemon.
              Studio only receives the model inventory that deployment exposes.
            </p>
          </div>
        ) : (
          <>
            <p className="settings-copy">
              ...
              mecated&rsquo;s conventional credentials file, then restart Studio.
            </p>
            ...
            <div className="key-safety">
              ...
              <p>
                ...
                <code>providers.openrouter.api_key</code> in the auth file. A
                missing credential fails loudly instead of falling back.
              </p>
            </div>
          </>
        )}
          ...
          <form onSubmit={saveModelRouter}>
              ...
              <div className="router-managed" role="status">
                ...
                <p>
                  ...
                  <small>
                    ...
                    slots, and guardrails. Router-only editing is locked to keep
            ...
            >
              ...
              <div className="router-grid">
                <div>
                  <label htmlFor="router-classifier">Classifier model</label>
                  <input
                    id="router-classifier"
                    list="router-model-options"
                    value={routerClassifierModel}
                    onChange={(event) => setRouterClassifierModel(event.target.value)}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(event) => setRouterDefaultCategory(event.target.value)}
                  >
              ...
              <div className="input-hint">
                The classifier is one small extra call per routable delegation.
                Explicit model choices still take precedence.
              </div>
                ...
                {routerCategories.map((category, index) => (
                  <fieldset className="router-category" key={index}>
                    ...
                    <div className="router-grid">
                      ...
                      <div>
                        <label htmlFor={`router-model-${index}`}>Gateway model</label>
                        <input
                          id={`router-model-${index}`}
                          list="router-model-options"
                          value={category.model}
                          onChange={(event) =>
                            updateRouterCategory(index, { model: event.target.value })
                          }
                    ...
                    <textarea
                      ...
                      onChange={(event) =>
                        updateRouterCategory(index, { description: event.target.value })
                      }
                      maxLength={300}
              ...
              <div className="key-safety router-safety">
                <span>i</span>
                <p>
                  Routing applies to plain Subagents, unpinned agent definitions,
                  team members, and parallel branches. It never changes the
                  Provider bound to a Session.
                </p>
              </div>
            </fieldset>
            {routerState === "error" && (
      ...
      >
        {external ? (
          <div className="transport-note">
            <span>i</span>
            <p>Gateway connections are managed by the external deployment.</p>
          </div>
        ) : (
          <>
            {mcpConnected && (
            ...
            <form onSubmit={connectMcp}>
              <div className="field-row">
                <div>
                  ...
                  <input
                    ...
                    value={mcpName}
                    onChange={(event) =>
                      setMcpName(event.target.value.replace(/[^A-Za-z0-9_]/g, ""))
                    }
                    placeholder="gateway"
                  />
                </div>
              ...
              <button
                ...
                type="button"
                disabled={!mcpName.trim() || !mcpUrl.trim() || mcpState === "saving"}
              ...
              <div className="input-hint">
                Uses the gateway&rsquo;s OAuth sign-in with PKCE. No password or
                access token is stored by the browser.
              </div>
              <label htmlFor="mcp-token">
                Existing bearer token <span className="optional">Advanced</span>
              </label>
              <input
                id="mcp-token"
                type="password"
              ...
              />
              <div className="input-hint">
                Only use this when your gateway administrator supplied a current
                access token.
              </div>
              <div className="input-hint">
                Gateway URLs must use HTTPS. A local HTTP gateway is accepted only
                when the controller is explicitly started with{" "}
                <code>MECATL_ALLOW_INSECURE_LOOPBACK_MCP=1</code>.
              </div>
              <div className="key-safety">
                <span>✓</span>
                <p>
                  Credentials stay in the loopback controller&rsquo;s memory and
                  are never stored in this app or repository.
                </p>
              </div>
              {mcpState === "error" && (
                <div className="credential-error" role="alert">
