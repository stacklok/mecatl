...
**Skills can write themselves, under lifecycle control.** `SkillDraft` lets the model propose a versioned agent-owned draft after a genuinely non-obvious procedure, but the draft is never active. Evidence-backed reflection can evaluate procedures: review mode stages PASS/ABSTAIN and rejects FAIL; auto mode activates only PASS. Activation atomically refreshes the live `Skill` catalog without restart, while operator/project/user/driver skills keep precedence and remain immutable. The old quarantine plus `mecated skills promote` workflow remains a deprecated compatibility path.
