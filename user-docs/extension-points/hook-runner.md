...
- [Permissions & guardrails](/what-you-get/permissions.md) — the rule-based permission layer (Layer 1) that gates every tool call before hooks fire, and the model-backed guardrail checker (Layer 2) that is wired on top of `HookRunner`.
- [The agent loop](/what-you-get/agent-loop.md) — the full turn structure and where `PreToolUse` / `PostToolUse` fit in the dispatch sequence.
- [PermissionPolicy](/extension-points/permission-policy.md) — the other port that fires on every tool call, before hooks, to decide allow / ask / deny.
- [Tool catalog](/extension-points/tool-catalog.md) — add, replace, or remove tools from the catalog the loop dispatches against.
