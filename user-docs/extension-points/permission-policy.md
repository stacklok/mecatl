...
For the full operator surface — YAML config shape, scope semantics, workspace trust, the `subagent:` config block, plan mode, and the `--posture` ladder — see [Permissions & guardrails](../what-you-get/permissions.md).
...
- [Permissions & guardrails](../what-you-get/permissions.md) — the full operator surface: posture ladder, YAML config shape, workspace trust, the `subagent:` block, plan mode, and Layer 2 guardrails.
...
- [Agent loop](../what-you-get/agent-loop.md) — where `Evaluate` is called in the dispatch sequence and how the `permission.ask` pause/resume handshake works.
- [Hook system](../what-you-get/hooks.md) — `PreToolUse`/`PostToolUse` hooks that run after the permission decision and before/after tool execution.
