...
- [Pick your deployment shape](/getting-started/deployment-decision.md) — when to add a lease backend and which one to choose.
- [SessionStore & EventLog](session-store.md) — the session persistence port; the lease sits on top of it.
- [Cloud-native deployment (mecak8s)](/deployment/mecak8s.md) — the k8s topology that wires the `k8slease` backend and Redis together.
- [Hook system](/what-you-get/hooks.md) — other service-layer lifecycle seams.
