...
- **`tool.FileSystem`** and **`tool.Workspace`** — these define the filesystem abstraction the tools execute against. They live in `engine/tool` because `port` already imports `tool` (for `LLMRequest.Tools []tool.ToolSpec`) and `Tool.Execute` takes a `Workspace` — moving `Workspace` into `port` would create a `port↔tool` import cycle. Implementations are `engine/adapter/memfs` (tests) and `internal/adapter/osfs` (production).
