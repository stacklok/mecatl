...
| Slash-command palette controls | `↑` / `↓` select the previous or next command. `pgup` / `pgdn` (`ScrollU` / `ScrollD`) page through the selected command's wrapped description before moving to another command. `tab` / `enter` choose the command. While idle, `esc` hides the palette until you leave command input; during a run, `esc` cancels the run. Above and below indicators report hidden physical rows. |
