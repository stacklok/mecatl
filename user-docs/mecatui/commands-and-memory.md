...
These commands intentionally do not make memory universally available or grant access to a remote server's configuration. If a command is absent or says it is unavailable, use the server's advertised capability and policy as the source of truth. Builders configuring stores, retention, or learning should use the [memory guide](/building/what-you-get/memory.md).
