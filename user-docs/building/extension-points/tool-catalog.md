...
- [The agent loop](/building/what-you-get/agent-loop.md) — how the dispatcher executes the catalog's tools and applies the read-parallel / mutate-serial model.
- [Permissions & guardrails](/building/what-you-get/permissions.md) — how tool calls are evaluated before the catalog executes them, and how guardrails inspect the data crossing the tool boundary.
- [Hook system](/building/what-you-get/hooks.md) — `PreToolUse` and `PostToolUse` hooks that decorate catalog execution without modifying tool implementations.
