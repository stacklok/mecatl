...
Scheduled-tasks has its own REST surface under `/v1/schedules` — see [Scheduled tasks](/building/what-you-get/scheduled-tasks.md#managing-schedules-in-chat-grpc-and-rest).
...
- [The agent loop](/building/what-you-get/agent-loop.md) — how events map to the internal run lifecycle (`session.init` → `turn.start` → `tool.call` → `permission.ask` → `result`).
