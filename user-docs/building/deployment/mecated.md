...
| Flag | Default | Notes |
|---|---|---|
| `--mcp-server name=URL` | (none) | Remote MCP server, repeatable. Per-server bearer token from `MCP_<NAME>_TOKEN`; a token-bearing URL must be `https` (or `http` to loopback); `MCP_<NAME>_DISABLE_NOTIFICATIONS=true` disables the standalone SSE notification stream on a GET-hostile gateway |
