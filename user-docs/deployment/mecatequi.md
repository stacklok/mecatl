...
| `--trust-project` | `false` | **Opt-in on headless roots.** Admit the cloned repo's project-tier steering (AGENTS.md, project rules/agents/skills/soul/commands). **OFF by default** — the fail-safe default yields allow-all approvals + a working child shell but NO repo steering; pass this ONLY when the scheduler trusts the repo ([ADR 0094](https://github.com/stacklok/mecatl/blob/main/docs/adr/0094-opt-in-project-ingestion.md)). Supply your own framing via `--instructions`. |
