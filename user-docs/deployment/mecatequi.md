...
| `--no-project-trust` | `false` | **Operator-tier only.** Suppress the cloned repo's project-tier ingestion (AGENTS.md, project rules/agents/skills/soul/commands) while keeping `--posture auto`'s approvals and the subagent shell. Recommended for all cloned-repo scheduler runs where the git ref is an untrusted surface ([ADR 0092](https://github.com/stacklok/mecatl/blob/main/docs/adr/0092-no-project-trust-pin.md)). Supply your own framing via `--instructions`. |
