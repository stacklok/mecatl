...
func TestPDFArtifactStorage_Flags(t *testing.T) {
	for _, argv := range [][]string{
		...
		if _, err := parseFlags(argv); err == nil { t.Fatalf("parseFlags(%q) accepted incomplete or insecure artifact configuration", argv) }
	}
	...
	if err != nil { t.Fatalf("parseFlags(valid S3): %v", err) }
	if cfg.artifactS3Bucket != "private-pdf" || cfg.artifactS3Region != "eu-west-1" || cfg.artifactS3Endpoint != "https://minio.example:9000" { t.Fatalf("parsed S3 flags = %+v", cfg) }
}
