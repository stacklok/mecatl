...
func parseFlagsModeOut(mode commandMode, argv []string, out io.Writer) (*flag.FlagSet, config, error) {
	...
	fs.IntVar(&cfg.maxRunTokens, "max-run-tokens", 0, "Maximum cumulative input+output tokens per engine run. The same ceiling is inherited but independently enforced for Subagents, Parallel branches, team members, and lead synthesis; child spend is not charged to the parent, so a delegation tree can exceed it. A run that crosses it ends cleanly with stop=budget. Default: unlimited; pass a positive value to cap. (0 also means unlimited.)")
