...
func TestMecatuiEffortHandoffRecovery_Scenario1_FailuresRetainUsableSource(t *testing.T) {
	...
	beforeBlocks := len(m.conv.testBlocks())
	m, cmd := effortHandoffPick(t, m)
	m = driveEffortHandoffFailure(t, m, cmd)
	if m.sessionID != "sess-test-0001" || m.phase != phaseIdle || len(m.conv.testBlocks()) != beforeBlocks || !m.prompt.Focused() {
		t.Fatalf("fork failure lost source usability: id=%q phase=%v blocks=%d focused=%v", m.sessionID, m.phase, len(m.conv.testBlocks()), m.prompt.Focused())
	}
...
func TestMecatuiEffortHandoffRecovery_Scenario1_AdoptsTargetBeforeClosingSource(t *testing.T) {
	...
	beforeBlocks := len(m.conv.testBlocks())
	m = feedCmd(t, m, cmd)
	if m.sessionID != "sess-fork-1" || len(m.conv.testBlocks()) != beforeBlocks || m.sessionTitle != "successor" || m.resolvedSessionModel.ReasoningEffort != "low" {
		t.Fatalf("target was not adopted with local projection preserved: id=%q blocks=%d title=%q effort=%q", m.sessionID, len(m.conv.testBlocks()), m.sessionTitle, m.resolvedSessionModel.ReasoningEffort)
	}
