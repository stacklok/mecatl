...
func TestScrollBackToBottomResticks(t *testing.T) {
	...
	if m.conversationView.mode != followTail || !m.vp.AtBottom() {
		t.Errorf("after re-sticking a delta should stay pinned: stuck=%v atBottom=%v", m.conversationView.mode == followTail, m.vp.AtBottom())
	}
}
...
func TestResizeReDerivesStuck(t *testing.T) {
	...
	m, _ = pressKey(m, tea.KeyPressMsg{Code: tea.KeyPgUp})
	if m.conversationView.mode == followTail || m.vp.AtBottom() {
		t.Fatalf("precondition: pgup should unstick and leave off-bottom (stuck=%v atBottom=%v)", m.conversationView.mode == followTail, m.vp.AtBottom())
	}
	...
	if m.conversationView.mode == followTail != m.vp.AtBottom() {
		t.Errorf("after resize stuck=%v must reflect AtBottom()=%v", m.conversationView.mode == followTail, m.vp.AtBottom())
	}
	if !m.vp.AtBottom() {
