...
func TestPaletteOpensOnSlash(t *testing.T) {
	...
	// Merged set: 4 built-ins (clear, help, session, retry) + 3 workspace rows.
	if len(m.palette.filtered) != 7 {
		t.Fatalf("filtered = %d, want 7 (4 built-ins + 3 workspace)", len(m.palette.filtered))
	}
	view := m.View().Content
...
func TestPaletteFetchesOnce(t *testing.T) {
	...
	// Merged: 4 built-ins + 3 fetched workspace rows = 7.
	if !m.palette.open || len(m.palette.filtered) != 7 {
		t.Fatalf("palette not populated from fetch: open=%v filtered=%d (want 7)", m.palette.open, len(m.palette.filtered))
	}
...
// palette. The merged order is [clear, help, session, retry, fix, review, refactor], so five ↓
// land on "review" (index 5, a workspace row → text-completed, not run).
func TestPaletteNavigateAndComplete(t *testing.T) {
	...
	if m.palette.cursor != 5 {
		t.Fatalf("cursor = %d, want 5 after 5×↓", m.palette.cursor)
	}
	if m.palette.filtered[5].Name != "review" || m.palette.filtered[5].Builtin {
		t.Fatalf("row 5 = %+v, want workspace 'review'", m.palette.filtered[5])
	}
...
func TestPaletteUnknownPrefixShowsNote(t *testing.T) {
	...
	// Bare "/" opens: clear, help, session, and retry are always present.
	m = typeRune(t, m, '/')
	if !m.palette.open {
		t.Fatalf("bare '/' should open the palette (built-ins always exist)")
	}
	if len(m.palette.filtered) != 4 {
		t.Fatalf("bare '/' filtered = %d, want 4 built-ins", len(m.palette.filtered))
	}
...
func TestPaletteNilCommanderShowsBuiltins(t *testing.T) {
	...
	if len(m.palette.filtered) != 4 {
		t.Fatalf("filtered = %d, want 4 built-ins (clear, help, session, retry)", len(m.palette.filtered))
	}
