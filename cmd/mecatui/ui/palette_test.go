...
func TestPaletteOpensOnSlash(t *testing.T) {
	...
	// Merged set: 6 built-ins (clear, help, title, session, retry, diagnostics) + 3 workspace rows.
...
// palette. The merged order is [clear, help, title, session, retry, diagnostics, fix, review, refactor], so seven ↓
...
func TestPaletteUnknownPrefixShowsNote(t *testing.T) {
	...
	// Bare "/" opens: clear, help, title, session, retry, and diagnostics are always present.
	m = typeRune(t, m, '/')
	if !m.palette.open {
...
func TestPaletteNilCommanderShowsBuiltins(t *testing.T) {
	...
	if len(m.palette.filtered) != 6 {
		t.Fatalf("filtered = %d, want 6 built-ins (clear, help, title, session, retry, diagnostics)", len(m.palette.filtered))
	}
