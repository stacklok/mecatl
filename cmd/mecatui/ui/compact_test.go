...
func TestCompactNoOpErrorAndStaleCompletion(t *testing.T) {
	...
	m = mm.(Model)
	if len(m.conv.testBlocks()) != blocks || m.statusMsg != status {
...
func TestCompactSameSessionStaleTokenDoesNotClearPending(t *testing.T) {
	...
	m = mm.(Model)
	if !m.compactPending || m.compactRequestToken != 9 || len(m.conv.testBlocks()) != blocks {
		t.Fatalf("stale same-session completion changed state: pending=%v token=%d blocks=%d", m.compactPending, m.compactRequestToken, len(m.conv.testBlocks()))
	}
}
