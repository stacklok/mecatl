...
func parseFlags(args []string) (config, error) {
	...
	fs.StringVar(&cfg.logLevel, "log-level", defaultLogLevel,
		"minimum severity written to the diagnostics log at $XDG_STATE_HOME/mecatl/mecatui.log: debug|info|warn|error. debug adds the LLM stream-lifecycle detail (per-attempt timeouts, retry-with-backoff) that is otherwise unobtainable without a rebuild. --quiet still discards everything regardless of level; diagnostics NEVER go to stderr")
