...
func validateEmbeddedProvider(c config) error {
	...
	return errors.New("no LLM provider configured: run `mecatui llm setup`, set a provider credential, use --auth-file, enable a ToolHive gateway, pass --mock, or connect to mecated; see https://mecatl.dev/docs/features/choose-models")
}
