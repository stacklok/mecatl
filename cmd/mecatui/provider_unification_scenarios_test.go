...
func TestProviderUnification_Scenario2_ProviderCommandGrammar(t *testing.T) {
	for _, args := range [][]string{
		...
		{"mecatui", "providers", "login", "corp", "--no-browser"},
		{"mecatui", "providers", "add", "corp", "--no-login"},
		{"mecatui", "providers", "remove", "corp"},
	} {
...
func TestProviderUnification_Scenario3_ProviderHelpHierarchy(t *testing.T) {
	for _, action := range []string{"setup", "add", "login", "logout", "set-default", "remove"} {
		got := resolveInvocation([]string{"mecatui", "providers", action, "--help"})
		if got.err != nil || got.mode != modeLogin {
			t.Fatalf("help for %s = %+v", action, got)
		}
	}
}
