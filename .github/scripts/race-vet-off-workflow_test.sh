...
for job in test-race-root test-race-ui; do
  block="$(awk -v job="$job" '
    $0 == "  " job ":" { in_job = 1 }
  ...
  case "$job" in
    test-race-root) expected_wrappers=7 ;;
    test-race-ui) expected_wrappers=1 ;;
  esac
  ...
  # Step names are descriptive, not commands. Remove them before rejecting a
  # direct race invocation in a run scalar or block body.
...
if [[ "$(grep -Fc "$forward" "$workflow")" -ne 2 ]]; then
  fail 'race_vet_off forwarding is not limited to the two race jobs'
fi
