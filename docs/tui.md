...
| `--log-level` | `info` | minimum severity written to `$XDG_STATE_HOME/mecatl/mecatui.log`: `debug` \| `info` \| `warn` \| `error`. `debug` adds the LLM stream-lifecycle detail (per-attempt timeouts, retry-with-backoff) that is otherwise unobtainable without a rebuild. `--quiet` still discards everything regardless of level; an unknown value fails startup |
...
`--log-level` sets the floor (default `info`, the level every sink was previously
hardcoded to). Pass `--log-level debug` when diagnosing a stalled or failing model
stream: it reveals the resilience layer's per-attempt-timeout and
retry-with-backoff lines. The two paths that end a turn terminally — a permanent
