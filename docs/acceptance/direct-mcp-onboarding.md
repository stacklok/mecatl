...
- **Compatibility / migration:** Existing hand-authored direct profiles, `key_env`, environment readers, preregistered clients, CIMD clients, direct-DCR `openid` records, login recovery flags, broker profiles, and session client MCP remain readable. ADR 0325's `openid`, no-refresh, registration-response-scope, and exact-name lifecycle rules remain authoritative. Legacy `key_env` profiles remain marker-free and resolve only through their explicit environment reference; onboarding never rewrites or migrates them. New lifecycle states extend ADR 0325 only for removal. Global changes affect newly started daemons; hot reload, Windows local onboarding, generic MCP scope negotiation, SDK helpers, and machine-readable CLI output are deferred.
