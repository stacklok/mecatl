...
- [x] Approve legacy/native credential-domain coexistence. — Decision: existing `local.key_env` profiles retain the legacy `mecatl-mcp-oauth` namespace; native `local.key` profiles use `mecatl-mcp-oauth-native/v1`. Native markers bind namespace, backend, and a domain-separated key-locator digest. A native `pending` marker is created before its key and carries a random initialization identity, so only that matching crash-left artifact may be resumed to `ready`; an artifact with no marker remains ambiguous and fails closed. Commands load only the selected profile, never enumerate, copy, decrypt, or migrate legacy records, and preserve all legacy behavior.
