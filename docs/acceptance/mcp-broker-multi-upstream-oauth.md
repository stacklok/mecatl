...
The same fixture completes the first ToolHive upstream authorization and then
makes the second authorization or authenticated discovery fail. ToolHive owns
the error and any internal retry state. Mecatl observes only the terminal
aggregate operation, clears its pre-prompt gate, exposes no backend-specific
control data, and never admits a partial catalogue. A later fresh mecatl
enrollment delegates a new operation to ToolHive; it does not revive or inspect
Mecatl-held per-upstream grants because none exist.

**Acceptance:**
- AC2.1: A failure, denial, expiry, callback mismatch, or discovery error in either ToolHive upstream produces one terminal mecatl enrollment outcome, clears the gate, and publishes no partial workspace catalogue.
  - verify: `TestADR_0298_ToolHiveChainFailureDoesNotPublishPartialCatalogueE2E`
...
4. All three scenarios pass offline against real ToolHive handlers and two independent fake upstreams; no test calls a live OAuth server, ToolHive deployment, or model provider.
