...
The smallest change that makes unscoped `InspectSession` root views cheap and deterministic: `status` and the other basic/root projections must read the authorized target directly and must not traverse the global JSONL lineage index. Related/delegation views and reads selected by a scope handle retain their existing lineage scan and root/relationship/ownership/retention revalidation.

## Why this scope cut

- **Root views are not lineage queries.** Snapshot status, transcript, activity, performance, network, and other unscoped root projections are target-bound reads; they do not need the global descendant/tombstone inventory. Avoiding that traversal prevents unrelated retained sessions from determining the cost or outcome of a basic inspection.
- **Scoped evidence remains fail-closed.** `related`, `delegation`, and any `scope_handle`-based inspection still use the bounded lineage reader and rescan/revalidate the applicable root, relationship, owner, incarnation, retention, and handle state as required by ADRs 0256–0258.
...
A debug session inspects a target with a JSONL-backed store containing unrelated retained roots, descendants, and tombstones. An unscoped `InspectSession` call for `status` (and the basic/root view set) returns from the target snapshot/event sources without opening or traversing the global lineage index. A `related` or `delegation` call, or a call carrying a valid `scope_handle`, performs the bounded lineage scan and the existing incarnation-aware revalidation before projecting evidence. If scoped lineage is unavailable, incomplete, stale, or no longer authorized, the response remains explicit/fail-closed rather than falling back to an unscoped result.
...
| valid `scope_handle` | yes | revalidate root, relationship, incarnation, owner, retention, and handle before projection |
