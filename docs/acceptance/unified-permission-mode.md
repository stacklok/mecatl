...
- [ ] Which mechanism carries the allow-all grant, given it must keep binding child engines. An earlier draft of this plan resolved it as "per-call extra rules driven by the session tier" and that is provably wrong as written: `childRules` injects the grant at build time from `cfg.AllowAllTools`, all three child constructors build their session at `session.ModeDefault` (`engine/agent/subagent.go`, `parallel.go`, `teamsupervisor.go`), and `internal/app/allowall_test.go` pins the grant reaching BOTH rule sets as an explicit kill-switch. Moving it per-call without another change silently drops allow-all for every subagent, parallel branch, and team member. Three candidate resolutions, each with a real cost. (a) Keep the grant build-time from the CEILING: nothing regresses, but then `trusted` and `yolo` have no effect at all in `permpolicy.Evaluate`, which is the only place the domain consults the value, so three of the six tiers carry no domain meaning and widening a domain type to hold them is hard to justify. (b) Move it per-call AND have child sessions inherit their parent's effective tier instead of `ModeDefault`: the tiers become meaningful and the binding survives, but it changes child semantics (a child of a `default`-tier session under an `auto` ceiling would no longer be allow-all, which it is today) and it makes allow-all session-selectable, which is the shape ADR 0022 decision 1 refused, now expressed as a rule rather than a bypass. (c) Build the per-session engine, including its child rule sets, from the session's effective tier fixed at create, and restrict `SetMode` to tiers that carry no composition effect: most faithful, largest change, and it makes some tier transitions require a new session.
...
- AC3.5: Whichever mechanism the open allow-all decision selects, the grant still binds child engines: under an allow-all-granting configuration a subagent, a parallel branch, and a team member each resolve allow-all, and the `internal/app/allowall_test.go` kill-switch still passes against whatever rule sets the resolution produces.
