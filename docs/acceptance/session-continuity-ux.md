...
- AC4.2: Each titled row shows ADR-0350's fixed raw readable session handle; the current chat stays visible with a `current` marker and cannot be redundantly opened.
...
- AC5.4: The compact header uses ADR-0350's ordinary handle, remains width-safe, and `/session` is discoverable from slash completion and `?` help.
