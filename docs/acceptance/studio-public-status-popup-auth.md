...
**Decision record:** [ADR 0361](../adr/0361-studio-anonymous-status-and-popup-login.md)
...
- [x] Work class and delivery path. — Decision: Architectural under the security/trust and public API criteria; create ADR 0361 and open an independent Split Plan / Interface PR targeting `main`.
...
- [x] Blocked or closed popup. — Decision: keep the original tab mounted and offer another user-initiated popup attempt or a manual new-tab sign-in link using the ordinary validated `return_to` redirect flow, then recheck the original tab on focus; no automatic same-tab redirect or draft serialization.
...
- [x] Existing callback compatibility. — Decision: bind popup mode in the sealed login transaction while keeping the registered callback URL, local alias, validated `return_to` redirect for ordinary login and manual new-tab fallback, and ADR 0351's sealed-cookie, SDK, and one-origin boundaries.
- [x] Popup browser isolation. — Decision: keep `Cross-Origin-Opener-Policy: same-origin` by default; only SPA HTML documents that can initiate login use `same-origin-allow-popups`, and only popup callback HTML uses `unsafe-none` so an issuer round trip can retain the opener. Callback script and API responses keep the default policy.
- [x] Anonymous status budget. — Decision: limit `/api/v1/status` to 120 requests per 60 seconds per client address and replica, independently of the configured auth budget. A limit response is `429` problem details with `Retry-After` and no-store; the browser treats it as an unconfirmed status, not proof of a BFF or daemon outage.
...
- [x] Initial anonymous presentation. — Decision: the public shell card owns the initial status and sign-in prompt; the banner handles outages and sign-in recovery once a workspace is mounted. Both use outage-before-sign-in precedence, and a pending connection observation stays neutral. This removes the duplicate sign-in actions observed during local preview.
...
- **CLI / config:** None — no flag or environment variable changes. Continue deriving callback origin and cookie `Secure` from `STUDIO_PUBLIC_URL`; keep the existing OIDC discovery, session-secret, authentication rate-limit, and runtime-mode settings. The public-status budget is fixed and separate, with no new deployment setting.
...
- **Security / authority:** `GET /api/v1/status` is the sole new anonymous exception under `/api/v1/*`; its `200` body returns no deployment, capability, credential, issuer, account, email, subject, error detail, or raw SDK status. It retains Host validation, security headers, no CORS, and `Cache-Control: private, no-store`. A separate fixed-window per-client-address limiter permits 120 requests per 60 seconds per replica before returning `429` problem details with `Retry-After` and no-store, without consuming the login budget. `/api/v1/runtime`, `/api/v1/settings/runtime`, `/api/v1/storage/health`, and every feature route remain session-gated in OIDC mode. An authenticated OIDC session requires a nonempty verified ID-token subject and its opaque account key; subjectless tokens and legacy sessions fail closed. Login keeps Authorization Code + PKCE, sealed cookies, state validation, rate limiting, same-origin mutation checks, and double-submit CSRF. The raw subject remains in BFF-readable sealed credentials and transient server memory; audit events omit it. A popup callback posts only the result enum to its exact origin; the opener verifies origin, source, and active attempt, then validates the session through the BFF. Callback HTML and script carry no code, state, credential, subject, email, or account key, are uncached, and use a no-referrer response policy. COOP remains `same-origin` on API, health, static assets, redirects, and callback script; only served SPA HTML uses `same-origin-allow-popups`, and only callback HTML on the existing two paths uses `unsafe-none`. The existing CSP, no-CORS posture, and frame protections remain.
- **Compatibility / migration:** Add `GET /api/v1/status` and optional `email` on authenticated `GET /api/v1/auth/session`; retain `{mode, status, account?}` for the existing modes, but require `account` whenever OIDC status is `authenticated`. Keep the existing opaque account-key derivation. Add optional `flow=popup` to `GET /api/v1/auth/login`; omission retains the ordinary redirect and validated `return_to` behavior, which the `noopener` manual new-tab fallback uses to land on the current route. The issuer redirect URI remains `STUDIO_PUBLIC_URL/api/v1/auth/callback` (or the current local `/oauth/callback` alias); a valid popup transaction selects the new HTML callback response. Existing sealed sessions and transactions without the new optional fields remain readable if they have a usable subject; subjectless legacy sessions are invalidated. Regenerate OpenAPI and the browser client from source schemas in the implementation PR. No `/api/v1/runtime` field becomes public.
...
**Acceptance:**
- AC1.1: an anonymous `GET /api/v1/status` returns exactly the two declared fields with `200`, no-store caching, and the existing Host/CSP/no-CORS policy. A separate 120-per-minute client-address budget returns no-store `429` problem details with `Retry-After` when exhausted, without consuming the login budget; the browser treats `429` as unconfirmed status rather than a BFF outage.
  ...
  - verify: vitest:apps/server/src/routes/status.test.ts#cHVibGljIHN0YXR1cyBoYXMgaXRzIG93biBib3VuZGVkIHJhdGUgYnVkZ2V0 — `apps/server/src/routes/status.test.ts :: "public status has its own bounded rate budget"`.
  - verify: vitest:apps/web/src/features/auth/auth-recovery.test.tsx#YSB0aHJvdHRsZWQgc3RhdHVzIGNoZWNrIHN0YXlzIG5ldXRyYWw — `apps/web/src/features/auth/auth-recovery.test.tsx :: "a throttled status check stays neutral"`.
...
- AC2.2: a valid popup transaction returns a CSP-compatible same-origin callback page that posts only the declared result with an exact target origin and closes; the opener accepts only an exact active-popup message from its own origin and verifies the session with the BFF. COOP is `same-origin` except for served SPA HTML (`same-origin-allow-popups`) and callback HTML (`unsafe-none`); other API and asset responses retain the default.
  ...
  - verify: vitest:apps/server/src/http/security.test.ts#cG9wdXAgQ09PUCBpcyBsaW1pdGVkIHRvIFNQQSBkb2N1bWVudHMgYW5kIGNhbGxiYWNrIEhUTUw — `apps/server/src/http/security.test.ts :: "popup COOP is limited to SPA documents and callback HTML"`.
...
**Acceptance:**
- AC3.1: blocked, manually closed, openerless, or message-less popups leave the original tab and draft mounted, show retry and a user-initiated new-tab sign-in link with `noopener noreferrer` and ordinary redirect flow to the validated current route, and recheck the original session on focus or popup closure before showing a failure.
  ...
  - verify: playwright:apps/web/e2e/auth-recovery.spec.ts#bWFudWFsIG5ldy10YWIgc2lnbi1pbiByZWRpcmVjdHMgYmFjayB0byB0aGUgY3VycmVudCByb3V0ZQ — `apps/web/e2e/auth-recovery.spec.ts :: "manual new-tab sign-in redirects back to the current route"`.
...
- AC3.4: an initial anonymous tab has one public-shell card for status and sign-in; a mounted workspace uses the #1844 banner for outages and sign-in recovery. Both presentations treat a failed status fetch as BFF unavailable, `connection: unavailable` as daemon unavailable, a failed session check as a retryable verification outage, then `signInRequired` as auth recovery; `checking` is neutral. The initial card shows no sign-in action during an outage. Anonymous rendering requests no detailed runtime or storage health; storage health is shown only after authentication.
...
1. Source schema, generated OpenAPI/browser client, BFF tests, web state tests, and an adversarial security review prove the public-response allowlist, authenticated routes, callback message checks, cookie/CSRF/origin/COOP controls, separate status rate limiting, and absence of the raw subject from browser data and audit logs.
...
4. Update the [Studio architecture owner](../architecture.md#mecatl-studio) for implemented behavior and the [Studio deployment guide](../../user-docs/building/deployment/studio.md) for browser sign-in and recovery in the implementation PR. The Plan / Interface PR keeps intended behavior here and in ADR 0361.
