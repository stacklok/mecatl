...
**Status:** draft, 2026-09-14. Product choices are approved; the two unchecked shipping-readiness decisions under “Human decisions”—the real-checker route/quality thresholds and evidence/trajectory capacity sizing—remain intentionally unresolved.
...
- [ ] **Shipping readiness — real-checker quality gate:** name the authorized `models.slots.guardrail` route and approve numerical release thresholds for false warnings, false blocks, missed adversarial cases, latency, and spend after a separately authorized evaluation. No live evaluation has run and this document invents neither results nor thresholds. Until checked, expanded defaults may be implemented and tested behind opt-in configuration but must not be described as empirically effective or ready to ship.
- [ ] **Shipping readiness — evidence/trajectory capacity:** approve deployment bounds for cumulative review evidence and delegation-root trajectory retention after sizing them against the selected checker context window and measured evaluation corpus. The implementation contract requires explicit `max_evidence_handles` (count), `max_evidence_bytes` (bytes per review), `max_trajectory_facts` (count per delegation-root Run), and `max_trajectory_bytes` (bytes per delegation-root Run); this document invents no values. The native textual Read preview remains bounded by its existing 2,000-line and 25,000-byte per-call caps, but `ReadVersion` currently reads a whole file before shaping that preview, so it is not proof of allocation safety. Until this decision is checked, an implementation must reject an unavailable or incomplete capacity path before oversized allocation and return unresolved; it must never proceed on a blindly “acceptable” assessment.
...
| review evidence handles/previews | one review, bounded by owner/session/environment/route/caller and approved count/byte capacity | assessment completion, cancellation, timeout, or root termination | reset-by-design; stale/unavailable handle is unresolved |
...
- AC1.2: Run once executes once; Don't ask again is offered only when implicated script/config/target dependencies are identified and version-bound, matches only the exact displayed session/environment/caller/tool/effective-call/target/dependency digest, and never reaches permission learning or permission replay. A known script change misses; an incomplete dynamic dependency sets `repeat_available=false` with a precise human limitation while Run once remains available; complete safe direct actions retain repeat.
  - verify: `TestADR_0334_ContextualGuardrails_Scenario1_ExactRepeatGrant`
...
- AC2.3: checker failure maps to interactive approval or unattended block under `fail`, and only explicit `warn` continues with warning; findings, deterministic denial, stale/forbidden evidence, and authority failure never use the operational opt-out.
...
- AC4.2: ledger lifetime/cleanup follows root Run conventions; approved aggregate count/byte bounds are enforced before allocation, relevant facts are never silently dropped, any compaction is authorization-equivalent, and incomplete evidence fails unresolved rather than blindly acceptable.
...
- AC6.3: offline mocks prove protocol only and make no efficacy claim; separately authorized evaluation records false warnings, false blocks, misses, latency, investigation frequency, and spend for the selected route.
  - verify: `TestADR_0334_ContextualGuardrails_Scenario6_QualityReportSchema`
- AC6.4: broader-default release remains blocked until both unchecked readiness decisions (route/quality thresholds and evidence/trajectory capacity values) are approved and their actual reports/tests pass.
  - verify: `TestADR_0334_ContextualGuardrails_Scenario6_ReadinessGate`
...
- AC7.4: generated proto, API snapshots/changelog, strict config reference, owning user docs, and Mecatui approval/status UI agree with the seven interface categories.
  - verify: `TestADR_0334_ContextualGuardrails_Scenario7_InterfaceConformance`
...
1. Both readiness decisions are checked: an authorized real-checker report meets approved route/quality thresholds, and approved evidence/trajectory count-and-byte bounds pass capacity tests. Until then the plan and ADR remain Draft and the Plan / Interface merge gate remains closed.
2. Implementation updates living architecture/implementation notes, owning user docs, generated configuration reference, proto, API snapshots/changelog, and the minimum List 1/List 2 inventory rows without marking behavior shipped before it lands.
3. Every named acceptance proof exists in the actual harness path; paired quality fixtures distinguish protocol tests from empirical evaluation.
4. `task lint`, `task test`, `task docs`, `task api:check`, `task ac-trace-strict`, `task site:build`, and `go run ./cmd/mecademo` pass; ordinary tests remain offline.
5. The implementation PR links the merged Plan / Interface commit and stops on contract drift.
