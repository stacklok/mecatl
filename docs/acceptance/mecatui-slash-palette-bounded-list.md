...
- [x] Palette fitting policy — Decision: use `bounded.Wrap`, preserving command descriptions across bounded physical rows. The physical-row budget, overflow indicators, and cursor movement follow that wrapped layout; the existing remappable `ScrollU`/`ScrollD` actions page through an oversized selected command's physical segments.
- [x] Short-terminal behavior — Decision: derive the palette body height from the rows remaining after preserving the prompt and footer, capped at twelve physical rows including up to two overflow indicators. When no command-content row fits, suppress the entire palette card for that frame; typing continues to filter and opening the palette again on a larger frame restores it.
- [x] Presentation amendment — Decision: use an outer-card width of `min(128, terminal width)` display cells, changing only on terminal resize. Render the command name bold and the description muted. Wrapped description continuations carry a `│` hanging prefix, and every entry is capped at three physical lines; truncated command or description text ends in the one-cell ellipsis `…`.
## Interface contract
...
- AC1.3: Every visible command row is rendered through `presentListRow` with one selection cell and an equal-width blank unselected gutter. Command names are bold and prominent while descriptions are muted; description continuations use a `│` hanging prefix. No entry exceeds three physical lines, and a clipped command or description ends in `…`. The selected row does not use the padded, bordered permission-button `askButtonActive` treatment; the resulting presentation remains readable under every built-in theme.
  - verify: `TestMecatuiSlashPaletteBoundedList_Scenario1_StandardRowPresentation`
- AC1.4: While idle, typing still edits and filters the prompt; Tab or Enter still dispatches an eligible built-in or completes a workspace command; and Escape still hides the palette until command mode is left. While a run streams, the visible palette owns Up, Down, `ScrollU`, `ScrollD`, Tab, Enter, and Escape: Enter retains its existing dispatch/completion behavior and Escape dismisses the palette rather than cancelling the run. After dismissal, ordinary running Enter and Escape handling resumes. When short geometry suppresses the palette card, it claims none of those navigation, completion, or dismissal keys and ordinary phase/prompt handling receives them. Command discovery still runs at most once per session; no Home, End, mouse, or click interaction is claimed by the palette.
