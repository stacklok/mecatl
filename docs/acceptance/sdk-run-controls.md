...
- AC2.4: Existing owned `Run` stream controls, disposal, and watch bookkeeping remain unchanged; controls neither require nor create an attachment. `Run` and `AttachedRun` expose only explicit verdict APIs; removed alpha compatibility aliases are not restored.
  - verify: vitest:sdk/typescript/test/run-controls.test.ts#cnVuIGFuZCBhdHRhY2hlZCBjb250cm9scyBleHBvc2Ugb25seSBleHBsaWNpdCB2ZXJkaWN0IEFQSXM — `sdk/typescript/test/run-controls.test.ts :: "run and attached controls expose only explicit verdict APIs"`
