...
**Status:** in-progress — direct user-authorized implementation-PR amendment, 2026-09-25.
...
**Implementation-PR amendment (2026-09-25, directly user-authorized):** Replace the normal inventory's twelve-row cap with all usable physical rows remaining after fixed chrome, retaining the compact fallback only when that chrome plus one row cannot fit. Pad a shorter noncompact panel through its positive offered height so the shared input/footer/status chrome remains bottom-justified. Correct the empty-state tab label so Other—and every tab, with Drafts both visible and hidden—renders without an out-of-range panic.
...
**Acceptance:**
- AC1.1: At tiny, short, narrow, and normal offered panel geometry—including a visible pagination-status row—the complete normal `/sessions` panel has no ANSI-stripped line wider than its offered display-cell width and no rendered height greater than its offered height. At positive noncompact geometry it returns exactly its offered height, padding a shorter result so shared input/footer/status chrome remains bottom-justified. Its list body uses all remaining usable physical rows after fixed tab chrome, visible pagination status, and action footer, including any above/below indicator chrome; every row reserves the standard two-cell selection/state gutter through `presentListRow`, retains the existing state badge and terminal-safe session fields, and keeps selected/unselected styling distinct. If tab chrome, visible pagination status, action footer, and one list row cannot fit, the surface instead renders its one-line width-clipped compact fallback, constructs no list, and exposes only Close; nonpositive geometry renders no panel.
  - verify: `TestMecatuiSessionsBoundedList_Scenario1_GeometryPresentationAndIndicators`, `TestSessionsRenderFillsOfferedBodyAndKeepsFooterBottomJustified`
