...
**Status:** landed, 2026-09-25. The implementation candidate, including the directly user-authorized amendment, passed its acceptance proofs and aggregate gates; this transition becomes authoritative when its implementation PR merges.
...
**Implementation-PR amendment (2026-09-25, directly user-authorized):** Replace the normal inventory's twelve-row cap with all usable physical rows remaining after fixed chrome, retaining the compact fallback only when that chrome plus one row cannot fit. Correct the empty-state tab label so Other—and every tab, with Drafts both visible and hidden—renders without an out-of-range panic.
