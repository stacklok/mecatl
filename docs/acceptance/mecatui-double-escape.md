...
- AC1.1: `View()` requests `tea.KeyboardEnhancements.ReportEventTypes`, and the model
  ...
  - verify: `TestADR_0303_DoubleEscape_Scenario1_ViewRequestsEventTypesAndTracksSupport`
- AC1.2: on a terminal that does not report event-type support, Escape never arms or
  clears the draft (fail closed).
  - verify: `TestADR_0303_DoubleEscape_Scenario1_UnsupportedTerminalFailsClosed`
- AC1.3: on a supporting terminal, the first non-repeat physical `Escape` is silently
  ...
  - verify: `TestADR_0303_DoubleEscape_Scenario1_FirstPressArmsExactExpiry`
- AC1.4: an Escape `tea.KeyReleaseMsg` establishes distinctness; only a second
  ...
  - verify: `TestADR_0303_DoubleEscape_Scenario1_PressReleasePressClearsThroughClearPrompt`
- AC1.5: a held key or Bubble Tea repeat event never counts as the second deliberate
  ...
  - verify: `TestADR_0303_DoubleEscape_Scenario1_RepeatBeforeReleaseCannotClear`
- AC1.6: any intervening non-`Escape` key disarms the gesture and continues through
  ...
  - verify: `TestADR_0303_DoubleEscape_Scenario1_InterveningKeyDisarmsAndRoutesNormally`
- AC1.7: expiry disarms without changing the draft, and an expiry from an older
  ...
  - verify: `TestADR_0303_DoubleEscape_Scenario1_ExactExpiryAndGenerationGuard`
- AC1.8: selection, slash palette, mention, approval, every overlay/modal owner, and
  ...
  - verify: `TestADR_0303_DoubleEscape_Scenario1_OwnersConsumeAndDisarm`, `TestADR_0303_DoubleEscape_Scenario1_AllEscapeOwnersSuppressGesture`
- AC1.9: existing ClearPrompt remains an accessible universal alternative through its
  ...
  - verify: `TestADR_0303_DoubleEscape_Scenario1_ClearPromptAlternativeRemainsAccessible`
- AC1.10: the first press remains silent and clearing adds no completion toast unless
  ...
  - verify: `TestADR_0303_DoubleEscape_Scenario1_FirstPressAndCompletionHaveNoNewStatus`
...
- AC2.1: live help names the two physical Escape presses, explicitly says enhanced
  ...
  - verify: `TestADR_0303_DoubleEscape_Scenario2_LiveHelpExplainsRequirementAndAlternative`
...
1. The named scenarios and ADR-0303 proofs pass offline, including fail-closed
