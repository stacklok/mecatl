...
[ProviderDiscovery](../architecture/mecatl.modelith.md#providerdiscovery) and the context
portion of the [resolution invariants](../architecture/mecatl.modelith.md#invariants), using
...
- [ ] Approve request and retry bounds. Recommendation: ten seconds per ordinary provider attempt and per admission/picker operation; ten-second per-provider cooldown from attempt completion; no sleeps or automatic retry loop. Unattempted demand starts immediately, in-flight demand joins, failed/empty demand retries once on the next eligible request, and cooldown demand returns current evidence immediately. An explicit picker open can also refresh a healthy provider after cooldown. Startup is one-shot and skips already-attempted providers; native authenticated providers remain demand-only. Preserve the exceptional ToolHive 1.5-second and Codex five-second bootstrap budgets and default-selection behavior. This replaces the current two-second stale-fetch bound and global cooldown, and extends demand recovery to every available lister provider.
- [ ] Approve the freshness boundary. Recommendation: ship coherent observation/outcome/inventory publication and pure context resolution now; preserve resolve-at-use context windows. Do not claim generation-bound engine capabilities or reasoning: engines built before admission can retain their construction-time capability/effort values. Defer factory/admission reordering and full session re-derivation as a separate slice, along with reasoning-field presence and UI model-identity/footer reconciliation. No periodic refresh, durable metadata cache, or persisted effective-model identity is added.
...
- **Exported Go APIs / interfaces:** No changes to `engine/`, provider-module APIs, or `port.LLMRequest`. The host-internal `server.ModelInventory` becomes read-only: `CurrentModelSnapshot() ModelSnapshot`, where `server.ModelSnapshot` contains `Models []*mecatlv1.ModelInfo` and `ProviderStatus []*mecatlv1.ProviderStatus`. Add `Service.ListModelSnapshot(context.Context) ModelSnapshot` for the two wire handlers; retain `Service.ListModels` as a models-only projection for existing callers. Keep `server.Config.AwaitContextWindow func(context.Context, string, string) error` and `SetModelsRefresher(func(context.Context))`. The TUI client adds `IsContextWindowUnavailable(error) bool`, checking code/domain/reason exactly, so `ui` stays proto-free. No generic coordinator port is introduced.
...
a monotonically increasing attempt ID; start/completion times; a safe classified latest
outcome; last non-empty successful immutable observations and their observation time;
next eligible attempt time; and one active completion channel/cancel function. An in-flight
attempt retains its preceding completed outcome and observations. Empty is an actual
zero-entry lister success, measured before adding configured/catalog picker floors.
No-lister eligibility is registry data, not fabricated successful discovery.

Completion validates owner liveness, attempt identity, and fetch-context validity, then
publishes one immutable replacement containing observations, outcome, evidence, and derived
inventory/status. A genuine timeout records a failed outcome but publishes no late result.
An invalidated completion cannot publish anything. Waiters wake only after publication.
A timed-out attempt releases its in-flight slot only after its lister returns; cancellation
must not permit overlapping fetches. Existing lister cancellation is part of the offline
shutdown proof. Use one short publication critical section across provider deltas, with no network or
adapter remint inside it. Skipped providers never enter a delta; preserve the skipped-native
fix in PR 1798. A cancelled/obsolete older attempt cannot restore catalog fallback over a
newer success. Default healing consumes only an accepted non-empty result, once, outside
the publication lock; it remains registry-owned.
...
`ListModels` and `ProviderStatuses` across separate publications.
...
- **Picker:** `SetModelsRefresher` requests available lister providers concurrently under
  one ten-second caller wait bound. Each provider starts/joins independently; one slow
  provider cannot delay another provider's publication or occupy its admission budget.
  Concurrency is at most one fetch per registered provider, not per session/caller.
...
admission evidence. Existing server gates must prevent unknown blocked targets from
reaching prompt recording, retry preparation, approval execution, compaction, or inference.
This slice does not pin an entire run to an observation generation.
...
| `server.Service` inventory and HTTP/gRPC ListModels response assembly | Use the combined read-only snapshot for the wired Build. Existing setter-backed nil-inventory fallback remains isolated for standalone Service construction; it cannot mutate owner state. |
| `cmd/mecatui/client` classifier and `ui.failStartupRunEntry` | Preserve retry/back machinery and transcript restoration; select bounded discovery-specific copy for the exact structured admission error. Retain generic safe copy for all other errors. |
...
**Acceptance:**
- AC2.1: A completion atomically publishes its observations, outcome, non-empty evidence, models, and safe statuses. Channel-barrier tests cannot observe successful admission evidence before its context metadata or mix model/status generations in one HTTP/gRPC ListModels response. DiscoverModels and ListModels project identical rows from the same captured snapshot.
  - verify: `TestProviderModelDiscovery_Scenario2_CoherentPublication`
- AC2.2: Delayed obsolete/cancelled completions, including an old failure after newer success, cannot change accepted metadata, outcome, or projection. Publishing another provider and skipping a native provider preserve their current observations byte-for-byte.
  - verify: `TestProviderModelDiscovery_Scenario2_ObsoleteAndSkippedPublications`
...
- AC3.2: New/default sessions, explicit selectors, mode-selected targets, failed-step retry, and restart approval resumption gate the actual effective identity. Failed admission records no new prompt, compaction, inference, retry preparation, or approval consumption; pending/retry data and genuine history survive. Existing reopen/history-repair and lease-retention semantics remain intact.
  - verify: `TestProviderModelDiscovery_Scenario3_AllRunEntryPaths`
...
- AC4.2: Failure, empty response, unauthorized response, and timeout each permit only one next attempt after ten seconds from completion. Calls during cooldown do not fetch or sleep; provider B's retries are independent of A. A picker open can refresh a healthy provider after its cooldown; elapsed time alone performs no work.
...
- AC5.3: The rejection retains the adopted transcript, session binding, and original submitted draft. Back restores the editable draft; Retry resends it to the same session only after explicit user action. Background metadata completion never auto-replays a prompt. One successful retry adds exactly one persisted user prompt and no duplicate transcript row.
  - verify: `TestProviderModelDiscovery_Scenario5_TranscriptDraftAndExplicitRetry`
- AC5.4: Ordinary new-session admission rejection offers the same safe recovery meaning; unrelated startup/transport/auth failures retain their existing closed handling. The existing tenant-path leak test continues to pass.
  - verify: `TestProviderModelDiscovery_Scenario5_NewSessionAndUnrelatedErrors`, `TestADR_0108_FirstPromptRevalidatesAtomically`
...
4. Update the living provider/context architecture and IMPLEMENTATION-NOTES; update ADR 0027's resource inventory and fidelity ledger when the owner actually exists. Record shipped/deferred scope only in PRODUCTION-READINESS. Preserve accepted ADR 0342 decision text.
...
in Human decisions. The principal residual is construction-time capability/effort staleness,
not context-window readiness. Retaining positive observations for a whole Build also accepts
metadata age risk after an upstream change; discovery cannot prove authorization or guarantee
the provider's real inference limit. Shared detached work can outlive the last interested
caller by up to its attempt bound, so shutdown/join ordering and adapter cancellation must
remain executable tests. Future model generations or age policy require a separate contract,
not extra conditionals in read-side helpers.
