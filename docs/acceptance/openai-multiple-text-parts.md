...
**Status:** draft, 2026-09-06. Derived from [stacklok/mecatl#738](https://github.com/stacklok/mecatl/issues/738).
...
2. `docs/architecture/providers.md` documents [ADR 0301](../adr/0301-openai-visible-text-delta-projection.md)'s ordered multipart `ChunkText` projection and the intentional loss of text-part identities without changing the phase, reasoning, function-call, or ADR 0239 retry contracts.
...
- **Narrow ADR supersession.** [ADR 0301](../adr/0301-openai-visible-text-delta-projection.md) supersedes only ADR 0017 §2's single-visible-text-part policy. It preserves the one-string domain representation and ADR 0239's ordered semantic-stream behavior; a future need to preserve part boundaries would be a separate, costly API decision.
