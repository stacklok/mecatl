...
**ADR:** [ADR 0291](../adr/0291-server-owned-session-placement.md)
...
This scenario enforces [ADR 0291](../adr/0291-server-owned-session-placement.md) at the public creation boundary.
...
session ID and bounded display metadata, never the exact private `EnvironmentRef`.
  - verify: `TestADR_0291_CreateSessionAcceptsOnlyDefaultOrNoFS`
...
closed before persistence or filesystem access, never binding a mixed generation.
  - verify: `TestADR_0291_BindRejectsRebindBetweenAuthorizationAndResolution`
...
choices are indistinguishable and never fall back to a default.
  - verify: `TestADR_0291_ScopedWorktreeSelectorsFailClosed`
...
`EnvironmentRef`, or path-shaped selector. Old path-bearing requests are unsupported.
  - verify: `TestADR_0291_PublicHarnessContractContainsNoFilesystemPaths`
...
identify its configured physical root.
  - verify: `TestADR_0291_SessionAndSnapshotPersistOnlyEnvironmentRef`
...
revision metadata.
  - verify: `TestADR_0291_DiscoveryIsSessionScopedAndOwnerAuthorized`
...
result without invoking a filesystem/default resolver, command source, or worktree lister.
  - verify: `TestADR_0291_NoFSDiscoveryDoesNotInvokeFilesystemProviders`
...
registry/map.
  - verify: `TestADR_0291_WorktreeSelectorIsScopedAndMatchedAgainstCurrentInventory`
...
posture; the response exposes no exact private ref or path.
  - verify: `TestADR_0291_ClearSessionCreatesEmptyInheritedSuccessor`
...
client binding.
  - verify: `TestADR_0291_ClearSessionIsLeaseSafeAndNonDestructive`
...
and original placement scope before access.
  - verify: `TestADR_0291_ArtifactHandlesCannotReplayAsPlacementSelectors`
...
no fork root, placement path, exact private ref, or reusable selector.
  - verify: `TestADR_0291_DelegationObservabilityContainsNoPlacementPath`
...
intent; models cannot choose schedule selectors.
  - verify: `TestADR_0291_ScheduleResolvesSelectorBeforePersistingExactEnvironmentRef`
...
`Reattach`. No public Harness/HTTP/client mapper projects that ref.
  - verify: `TestADR_0291_DriverStorageCarriesExactPrivateEnvironmentRef`
...
placement; mismatch is rejected and cwd cannot select or construct authority.
  - verify: `TestADR_0291_ACPBindAndLoadAssertConfiguredPlacement`
...
only.
  - verify: `TestADR_0291_PathSurfaceInventoryEnforcesPublicBoundary`
...
placement providers before serving; no stable public placement-ID registry is required.
  - verify: `TestADR_0291_CompositionConfiguresProviderOwnedPlacements`
...
| Client-selected host paths, including embedded/loopback | Permanently excluded by [ADR 0291](../adr/0291-server-owned-session-placement.md). |
