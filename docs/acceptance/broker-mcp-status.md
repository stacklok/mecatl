...
**Status:** in-progress, 2026-09-10. T01 broker inspection, T02 service/API/composition, and T03 client/UI/docs are integrated on the implementation accumulator; final verification and parent review remain outstanding; this candidate is not landed.
...
- [x] Unified broker setup UI scope extension — Decision: user approved `/mcp` and Ctrl+O as the primary broker inventory-and-setup surface. When the existing caller-owned whole-bundle enrollment controller is actually available for the current session, it offers Connect tools; a pending local enrollment shows Continue in browser and Cancel setup. `/tools-connect` and `/tools-cancel` remain equivalent compatibility shortcuts. Opening and refresh remain read-only broker inspection; setup reuses the existing controller, browser presentation, poll, and completion flow without a new controller, per-connector action, authority, or protocol. Direct MCP behavior remains unchanged.
...
- **Compatibility / migration:** Additive protobuf capability/RPC; old clients ignore the new bit and old servers leave it false. No store migration or public engine API baseline change. Direct `ListMcpSources`, resources/prompts/groups and their semantics remain unchanged. New mecatui registers `/mcp` when either capability and its corresponding collaborator are available; broker-only mode never issues direct resource/prompt/group calls, including shortcuts. In broker mode only, `/mcp`/Ctrl+O also expose the existing whole-bundle enrollment controller when its current-session capability, collaborator, and local session state make it eligible: Connect tools starts or resumes the same bundle, a pending local bundle shows Continue in browser and Cancel setup, and `/tools-connect`/`/tools-cancel` remain equivalent compatibility shortcuts. No catalogue state authorizes a setup action; opening and refresh are inspection-only. Server/client DTOs remain separated from UI proto types. Bind each response to the requested session and request generation so late responses cannot overwrite a switched session or a newer refresh. Implementation updates generated contracts through `task generate`, the tracked SDK's applicable generated/public surface, and public user docs in the same implementation PR.
...
**Acceptance:**
- AC3.1: `/mcp` and Ctrl+O open for broker capability, showing enrollment, connector name, catalogue state and tool count. The broker panel is the primary inventory-and-setup surface: it offers Connect tools only when the existing whole-bundle enrollment capability, controller, and active local session state make it eligible; it shows Continue in browser and Cancel setup for a pending local bundle. It never derives eligibility from `not_started` or another catalogue value. Hidden/unknown rows use an em dash rather than implying zero discovered tools; truncation is explicit. Refresh rereads local state only.
...
- AC3.4: `/tools-connect` and `/tools-cancel` remain compatibility shortcuts for the exact same existing whole-bundle actions offered by broker `/mcp`; pending cancel/reopen flows continue through the existing browser presentation, poll, and completion path. Opening or refreshing `/mcp` invokes no enrollment control. Ineligible sessions show no setup action, and direct MCP rendering and controls are unchanged.
  - verify: `TestBrokerMCPStatus_Scenario3_UnifiedSetupActions`
- AC3.5: Operator docs explain `/mcp` in broker-only Kubernetes, local refresh versus probes, static/lazy versus enrolled catalogues, unified whole-bundle setup actions, and restart limitations without suggesting global MCP as a workaround.
