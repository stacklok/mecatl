...
**Status:** draft, amended 2026-09-25. Exact proposed contracts are authored below; the concrete production source/configuration choice plus API, schema, security, and this HarnessContext amendment still require approval by merge.
...
- [x] Set the first-slice source and execution boundary. — Decision: use a blank tool-seeded PVC workspace, existing filesystem tools, foreground Shell, and independently configured deployment/operator/API instruction and command sources selected by the shared HarnessContext policy. The native first slice leaves PVC-backed context unselected: poisoned PVC instructions and commands are not discovered, and selecting a future exact-backend PVC source requires separate authorization and native adapter review. Add no native source or inventory API. Defer Git clone, private credentials, remote project ingestion, background commands, isolated delegation, five-kind parity, and delegation or schedule expansion; schedules remain excluded.
- [ ] Choose the concrete production source/configuration for standalone mecak8s: explicit operator-provided context files/adapter configuration or an API source. Existing adapters may be reused; this decision implies no new source protocol. The current local adapter requires `cfg.Workspace`, which conflicts with native `--workspace`, while the gRPC driver is commands-only and mecak8s exposes no `CommandSourceURL`.
...
approval. The unchecked source configuration, API, schema, security, and release-maturity decisions remain open.
...
- **Exported Go APIs / interfaces:** No native-specific `engine/` exported API change. The integration consumes the shared [HarnessContext interface contract](harness-context.md#interface-contract). `tool.Environment` remains identity + Workspace + independent ReadLedger + optional bound CommandRunner (`engine/tool/environment.go`). Host-only seams in the implementation's `internal/adapter/server/placement.go` are specified below; Kubernetes dependencies stay outside the engine. A future selected PVC source must use shared principal-scoped `UsesExecutionWorkspace` registration with authoritative SessionID/owner/profile and selector-free `AcquireExecutionWorkspace`; the first slice registers no such source. When selected, that callback exposes an exact-authorized read-only workspace with no runner or execution ReadLedger, and its release closes only the source borrow. Independent native sources receive no acquisition callback.
...
- **CLI / config:** Disabled by default. Mecak8s accepts `--execution-enabled`, `--execution-endpoint`, `--execution-profile`, `--execution-tls-ca`, `--execution-tls-cert`, and `--execution-tls-key`. Enablement requires all five string settings and OIDC caller ownership enforcement; it conflicts with `--workspace` and `--redis-filesystem`. Harness sources are selected only by the shared operator-tier `harness_context` policy; execution enablement, profile, provider availability, PVC contents, and source transport do not select them. The first slice registers independent native deployment/operator/API instruction and command adapters and leaves PVC-backed context unselected. The separate `deploy/helm/mecatl-execution` chart owns the provider deployment. The default mecak8s chart has no provider dependency. Exact provider profiles and security configuration are below.
...
The native first slice consumes the shared [HarnessContext source-authority contract](harness-context.md)
instead of selecting or suppressing context by execution kind. It registers independently configured
deployment/operator/API instruction and command sources; the PVC remains execution-only and unselected.
Proofs run through production `app.Build`, public Service/discovery, and the actual `executionclient`
boundary with an offline signed-gRPC fixture. A source pre-bound to `memfs` does not prove this integration.
The placement, governance, permission, trust, fencing, and unsupported-capability boundaries remain owned by
[draft ADR 0364](../adr/0364-native-kubernetes-execution.md).

**Acceptance:**
- AC6.1: Native source selection is observable independently from execution: (1) with a healthy provider, exact Attach and AcquireRun precede configured instruction markers reaching actual model requests and configured commands reaching slash invocation; `no-fs` provides the same context behavior without an execution run, file tools, or runner; (2) conflicting poisoned AGENTS/command content in the unselected PVC is absent, with zero HarnessContext workspace-acquisition and provider requests; and (3) after provider loss, an already-started Service with a stored authorized session continues independent public command discovery/listing, but no model-request or slash-invocation availability is claimed. Startup validation remains fail-closed. Source ordering, provenance, collision handling, admission, and failure semantics are the shared policy's, not native backend rules.
  - verify: `TestNativeHarnessContextIndependentSources`, `TestNativeHarnessContextUnavailableAndNoFS`; PENDING — these new native integration proofs must use production Build, public Service/discovery, and the actual executionclient gRPC boundary rather than only reference or memfs adapters.
- AC6.2: Native failure and authority behavior is explicit: (1) a required-source failure is returned with no host, PVC, process-cwd, operator-global, or other unconfigured fallback; (2) two separate Builds rebind an existing session to current source policy and reject revoked source authorization without preserving a stale grant; (3) source reads neither seed the execution ReadLedger nor authorize Edit; (4) schedules, background Shell, isolated Subagent/Parallel/Team delegation, SkillDraft, Git/source operations, unsupported worktree successors, and delegation or schedule expansion return named capability/precondition errors without local execution or silent omission; and (5) cancellation permits no late binding publication, while Build shutdown waits for in-flight source creation and cleanup under the canonical shared [AC4.4/AC6.3 lifecycle](harness-context.md#scenario-4---source-failure-does-not-change-authority). Existing governance/fencing, configured Ask/Deny, root-aware trust, `no-fs`, and Clear/Fork same-remote-reference behavior remain unchanged.
  - verify: `TestNativeHarnessContextRequiredSourceFailure`, `TestNativeHarnessContextRebindsCurrentPolicyAndAuthorization`, `TestNativeHarnessContextSourceReadsDoNotAuthorizeEdit`, `TestRemoteExecutionRealFactoryCarriesPostureAndAttenuatedCatalog`, `TestExecutionPreflightRejectsIncompleteAndIncompatibleCLI`, `TestServiceUsesRealMTLSProviderStoreAndReleasesOnlyAfterDrain`, `TestCommandStatusIsUnimplementedOnlyAfterAuthorization`; PENDING — the three new HarnessContext proofs and shared lifecycle conformance must be rerun through the native production boundaries; historical blanket-suppression evidence does not satisfy the amendment.
...
| AC6.1–AC6.2 | PENDING amendment evidence, blocked on the unchecked concrete production source/configuration decision: `internal/app/native_harness_context_test.go` should own `TestNativeHarnessContextIndependentSources`, `TestNativeHarnessContextUnavailableAndNoFS`, `TestNativeHarnessContextRequiredSourceFailure`, `TestNativeHarnessContextRebindsCurrentPolicyAndAuthorization`, and `TestNativeHarnessContextSourceReadsDoNotAuthorizeEdit`. These proofs must traverse production Build and public Service/discovery against the actual `internal/adapter/executionclient` signed-gRPC fixture: with a healthy provider, exact Attach/AcquireRun precede positive instruction/command markers reaching model requests and slash invocation; poisoned unselected PVC content causes zero context-acquisition/provider requests; after provider loss, an already-started Service with a stored authorized session retains independent discovery/listing only; no-FS exposes context without execution tools/runner; required-source errors have no unconfigured fallback; separate Builds apply current policy and revoked authorization; and source reads cannot authorize Edit. Shared lifecycle conformance must also preserve cancellation with no late publication and shutdown waiting for creation/cleanup. Existing `TestRemoteExecutionRealFactoryCarriesPostureAndAttenuatedCatalog`, `TestRemoteExecutionPreservesOperatorPermissionsUnderAuto`, `TestExecutionPreflightRejectsIncompleteAndIncompatibleCLI`, `TestServiceUsesRealMTLSProviderStoreAndReleasesOnlyAfterDrain`, and `TestCommandStatusIsUnimplementedOnlyAfterAuthorization` remain evidence for governance, Ask/Deny, trust, unsupported delegation/schedules/background Shell/worktree successors, placement, and fencing only. Historical operator-global/blanket-suppression evidence is not completion evidence for amended AC6.1–AC6.2. |
...
|---|---|---|
| GitHub cloning, private-repository credentials, authoritative remote project trust, and PVC-backed AGENTS/commands/rules/skills | Separate source-ingestion architecture and separately authorized exact-backend adapter | Blank/PVC execution plus independently configured deployment/operator/API instructions and commands are sufficient for slice 1; PVC context is deliberately unselected. |
| Native source or inventory APIs, durable HarnessContext identity, history transfer, `SessionAccess`, or new source lifecycle protocol | Shared HarnessContext contract or later demonstrated need | Consume the shared registration, policy, binding, and lifecycle contracts without a native variant. |
| Native five-kind source parity, repository-context enablement, delegation expansion, and schedule expansion | Separate reviewed expansion | Slice 1 integrates instruction and command sources and preserves existing unsupported-capability gates. |
...
3. `task ac-trace-strict` resolves every final `verify:` proof, including AC2.4, AC4.4–AC4.5, AC5.7, and the five newly named native HarnessContext tests under AC6.1–AC6.2. Resolve the MISSING/partial behavioral coverage noted above through production Build, public Service/discovery, and the actual executionclient boundary; named-test resolution or memfs-only reference coverage is not AC completion.
...
- Scope, retry, Helm lifecycle, disabled startup, remote filesystem conformance, and earlier source-suppression tests exist in the pinned implementation source. Final production Kind/native live evidence is complete at the tested runtime SHA; human reviews remain pending. The amended AC6.1–AC6.2 HarnessContext proofs are PENDING and cannot reuse the historical operator-global/blanket-suppression run. Existing output/CAS and unsupported-successor proofs remain valid for their narrower boundaries; source/proof locations absent on the plan branch stay explicitly implementation-scoped, and implementation code must not be added merely to satisfy plan-branch gates.
