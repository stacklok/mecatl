...
**Status:** draft, amended 2026-09-25. Exact proposed contracts are authored below; API, schema, security, and this HarnessContext amendment still require approval by merge.
...
**Acceptance:**
- AC6.1: Native source selection is observable independently from execution: (1) distinct configured instruction and command markers reach actual model requests, public command listing, and slash expansion; (2) conflicting poisoned AGENTS/command content in the unselected PVC is absent, with zero HarnessContext workspace-acquisition and provider requests; (3) provider unavailability does not break independent command discovery; and (4) a `no-fs` session receives independent context while exposing no execution file tools or runner. Source ordering, provenance, collision handling, admission, and failure semantics are the shared policy's, not native backend rules.
