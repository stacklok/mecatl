...
| AC7.2 | `e2e/k8s_execution/qualification_test.go`: `TestKindExecutionQualification`, run by the successful production profile. Complete actual-TUI/OIDC coding journey `TestNativeTUIOIDCBlankWorkspaceCodingJourney` is PENDING; it includes pending proof `TestNativePendingApprovalStartupRecovery` for the exact-resume persisted ordinary-ask recovery and its Allow Once/Deny, no-auto-submit, ownership, replay-gap, stale/malformed, and unsupported-watch boundaries. |
...
3. `task ac-trace-strict` resolves every final `verify:` proof, including AC2.4, AC4.1, AC4.3–AC4.5, AC5.1, AC5.7, and AC7.2. The five current-candidate end-to-end journey targets are `TestNativeTUIOIDCBlankWorkspaceCodingJourney`, `TestNativeSameDeploymentOIDCUserIsolation`, `TestNativeTUIReconnectAndRestartPreservesWorkspace`, `TestNativeCancellationAndAuthorityLossFailClosed`, and `TestNativeCapacityRetirementRestoresAllocation`. They remain PENDING and must not be inferred from narrower existing tests. `TestNativeTUIOIDCBlankWorkspaceCodingJourney` includes pending `TestNativePendingApprovalStartupRecovery`, the real-gRPC persisted-restart exact-resume ordinary-ask proof; it must establish explicit Allow Once/Deny (with no execution on Deny), no automatic prompt/verdict, and refusal of foreign, gapped, stale/malformed, or durable-watch-unsupported recovery. AC6.1–AC6.2 use the actual candidate shared-contract proofs named above; the removed five native source-adapter proposals are not MVP blockers.
