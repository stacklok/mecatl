...
implementation candidate `f63055665e97372316847bb6f5a5aaa859cf3c66` is source/test evidence only, not a new
Kind, live-provider, or user-journey run.
...
- AC2.4: After transient PVC or Pod creation failure, including prolonged quota or configured-profile capacity saturation, an existing allocation converges when capacity returns without unrelated CR/reference changes, restart, or manual reconcile. Operator-authorized retirement and UID-preconditioned deletion release the allocation only after retained workspace safety and reference exclusion are established; the next authorized session can then allocate. Retries remain rate-limited and continue while the object exists; wrong ownership and missing/mismatched authoritative UIDs never trigger adoption or replacement.
...
The MVP qualification topology uses one mecak8s replica; existing provider replica-safety functionality remains
supported but is optional MVP proof. The stack additionally requires the one explicitly invoked native-provider
...
- AC7.2: Through the actual TUI, a company user completes OIDC login/connect, creates a blank persistent session, prompts the agent to code, observes a file edit, approves or denies a configured protected action through Ask, and receives foreground test results. The deterministic flow seeds a small Go fixture through file tools, asks the harness to implement a function/tests, runs `go test` through bound Shell, and verifies the persisted artifact and restart/reattach behavior. This path needs no custom HarnessContext source or new browser UI.
  - verify: `TestKindExecutionQualification` covers coding/artifact/restart assertions and passed at the tested runtime SHA; PENDING current-candidate journey target: `TestNativeTUIOIDCBlankWorkspaceCodingJourney` for the actual TUI/OIDC → blank session → prompt/edit → protected Ask → foreground test-result path. Existing implementation candidate `TestNativeBuildPreservesIndependentContext` and `TestRemoteExecutionPreservesOperatorPermissionsUnderAuto` cover narrower composition and Ask/Deny boundaries only.
...
`f63055665e97372316847bb6f5a5aaa859cf3c66`. Candidate-source labels establish only their stated
...
| AC7.2 | `e2e/k8s_execution/qualification_test.go`: `TestKindExecutionQualification`, run by the successful production profile. Complete actual-TUI/OIDC coding journey `TestNativeTUIOIDCBlankWorkspaceCodingJourney` is PENDING. |
