...
- [x] Default visibility — Decision: a completed, acceptable review with disposition `execute` or `release_result` is benign and hidden by default; every other or unknown combination remains visible.
...
- **Events / persistence:** No event or persistence change. Mecatui derives benign presentation only when `inspection=complete`, `assessment=acceptable`, and disposition is `execute` or `release_result`. Durable guardrail hook events remain captured for live and replay rendering. Live-only detail RPC responses remain conversation blocks for the active client but are not made durable by this work.
- **Security / authority:** Presentation never changes inspection, holding, release, execution, approval, diagnostics, or permission authority. Missing guardrail metadata and every unknown, unresolved, prohibited, operational-failure, advisory, ask, withhold, deny, and warning disposition fail visible. Classification uses structured fields only, never checker-authored prose.
...
**Acceptance:**
- AC1.1: A guardrail review is benign only when inspection is complete, assessment is acceptable, and disposition is `execute` or `release_result`.
...
**Acceptance:**
- AC2.1: With shipped defaults, benign live hook cards and their correlated live detail notices remain in conversation state but contribute no rendered lines; attention-worthy hooks, details, approvals, and existing error surfaces remain visible.
  - verify: `TestQuietBenignGuardrailNotices_Scenario2_DefaultLiveVisibility`
...
**Acceptance:**
- AC3.1: Missing `hook_notices` or missing `show_benign` resolves to false; `hook_notices.show_benign: true` makes retained benign live and replay notices visible while details are collapsed.
  - verify: `TestQuietBenignGuardrailNotices_Scenario3_ClientSetting`
- AC3.2: The strict client parser rejects unknown `hook_notices` members and non-boolean `show_benign` values without disclosing configuration values; server and deprecated settings files cannot enable the behavior.
  - verify: `TestQuietBenignGuardrailNotices_Scenario3_StrictConfigOwnership`
