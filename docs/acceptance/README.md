...
- [Contextual guardrail usage accounting](contextual-guardrail-usage-accounting.md) — records contextual reviewer and path-escape checker usage on the reviewed session through dispatcher-ordered drain, preserving exact guardrail model attribution without a live pending projection. Status: in-progress under explicit operator authorization in the implementation PR.
