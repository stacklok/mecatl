...
- [Direct MCP Dynamic Client Registration](direct-mcp-dcr.md) — in-progress contract for durable public-client DCR in a local direct MCP profile, with no-refresh access grants and explicit re-login after expiry; complete refresh is tracked in issue #1355. Status: in-progress.
