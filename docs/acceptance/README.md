...
- [Caller separation](caller-separation.md) — enforce OIDC caller isolation over
  ...
  draft.
- [Session storage continuity](session-storage-continuity.md) — historical storage plan: bounded current snapshots, indexed metadata, and maintenance jobs landed; its writable legacy-adoption criteria were superseded by ADR 0288. Status: draft historical record.
...
- [Listener-scoped workspace authority](listener-scoped-workspace-authority.md) — historical
  draft superseded by ADR 0288's path-free contract; retained for context and excluded from
