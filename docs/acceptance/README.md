...
- [Mecatui submitted-prompt history](mecatui-submitted-prompt-history.md) — adds bounded, attachment-local Previous/Next traversal over accepted model-facing text while preserving multiline editing, queued edit-back precedence, visible-surface arrow ownership, and text-only recall. Status: proposed.
