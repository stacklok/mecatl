...
- [Live authenticated MCP metadata replaces static stand-ins](authenticated-mcp-metadata-replaces-static-standins.md) — replace pre-auth protected-tool placeholders with each session's complete, safely admitted, immutable authenticated catalogue. Status: in-progress.
