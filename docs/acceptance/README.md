...
- [Direct MCP Dynamic Client Registration](direct-mcp-dcr.md) — landed durable public-client DCR in a local direct MCP profile, with no-refresh access grants and explicit re-login after expiry; complete refresh is tracked in issue #1355 and live local-mecatui qualification remains human-run. Status: landed.
