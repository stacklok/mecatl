...
- [Abandon unavailable legacy MCP custody](mcp-legacy-key-abandon.md) — explicitly removes only a selected direct-DCR settings entry when its legacy external wrapping key is absent, retaining encrypted records and making no external call. Status: proposed under an explicit waiver of a separate Plan / Interface PR.
...
- [Mecatl Studio chat approvals, review, and Escape](studio-chat-approvals-review-escape.md) — exact run-and-ask plan verdicts, inline permission and authorization review, one-layer Escape handling, and evidence-bound developer steer trace. Status: proposed.
