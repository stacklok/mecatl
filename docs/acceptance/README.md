...
- [Mecatui overlay text selection](mecatui-overlay-text-selection.md) — extends root-owned mouse selection and clipboard behavior to the visible panel, overlay, modal, replay, and fatal body while preserving owner actions, hidden-state isolation, and terminal-native fallback modes. Status: landed in this implementation candidate; authoritative on merge.
