...
- [x] Approve native credential identity and storage (human-approved encrypted-only environment-key amendment, 2026-09-11). — Decision: `mecated` and embedded `mecatui` share one explicit operator-configured canonical `llm.credential_home` and the distinct encrypted namespace `mecatl/provider-oidc/v1`. Shared operator-only `llm.credential_key` defaults to `source: keyring`; explicit `source: environment` requires a valid `MECATL_*` `key_env` reference (for example `MECATL_NATIVE_LLM_CREDENTIAL_KEY`) containing canonical padded standard base64 decoding to exactly 32 bytes. Keyring forbids `key_env`; unknown sources/fields and invalid combinations fail closed. Record identity binds schema version, endpoint ID, canonical gateway origin/base path, discovery-verified issuer, client ID, resource audience, sorted/deduplicated scopes, exact redirect URI, and separately represented issuer/gateway trust identities including policy and loaded CA-bundle digest. Any identity change means not enrolled. Key source/reference are not identity fields. Unknown schema, unavailable selected key source/store, or corrupt identity fails closed. There is no plaintext refresh-token storage, `auth.yaml`, environment token persistence, memory-only persistence, automatic home/root/backend discovery, or migration fallback. Environment mode never accesses a keyring; missing/malformed keys fail before OAuth or credential mutation, and unreadable existing ciphertext is rejected before authorization. Source/key changes never silently migrate, re-encrypt, or overwrite records. Transient access/refresh tokens and decoded keys exist only in process memory as needed and never reach output, diagnostics, or model content.
...
- [x] Approve supported hosts and caller-token boundary. — Decision: Only embedded local `mecatui` performs interactive browser/loopback enrollment. Standalone `mecated` consumes and refreshes an existing record under the same configured credential home, OS identity, and selected encryption key and never launches a browser. There is no remote token transfer or login RPC. Current `mecated` drops the raw caller bearer at the authentication boundary and retains only the verified principal; v1 never forwards or retains caller credentials. Forwarded caller access-token mode requires an explicit shared-resource/gateway contract and can apply only to synchronous caller-bound work. OAuth token exchange requires a separate RFC 8693-style contract. `mecak8s` and `mecatequi` support are deferred.
