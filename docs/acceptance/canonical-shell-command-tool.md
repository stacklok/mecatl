...
**Status:** proposed. Amendment: move the parser-backed portability diagnostic to an engine-internal execution-compatibility helper, with no new exported helper API. This change requires review before implementation resumes.
...
- [x] Parser ownership and API boundary — Decision: place the parser-backed diagnostic in `engine/internal/shellcompat`, shared only by the engine’s Shell implementations. It is execution-compatibility feedback, not governance policy. Its AST implementation and concrete error stay unexported; add no exported helper API beyond the approved Shell rename.
...
- AC3.6: The portability parser is shared only through `engine/internal/shellcompat`; no exported governance or tool diagnostic helper, AST value, or concrete diagnostic error is added to the engine API.
  - verify: `TestShellCompatibilityDiagnostic`, `task api:check`
