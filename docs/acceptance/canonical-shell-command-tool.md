...
**Status:** approved, 2026-09-08. The issue owner reviewed and approved this behavioral and interface contract; the merged plan commit is the implementation baseline.
...
**ADR:** [ADR 0317](../adr/0317-canonical-shell-command-tool.md) — canonical Shell command-tool identity and portable POSIX diagnostics.
...
- [x] Exported engine API migration — Decision: make Shell-named exported Go symbols canonical and remove Bash-named exported symbols before public release. Legacy compatibility is limited to configuration values and the legacy flag. Update `engine/api/*.txt` and `engine/CHANGELOG.md` as a breaking engine API change.
...
| Reuse `mvdan.cc/sh/v3` in unrelated command-processing paths | future approved plan | This issue proves one command-tool use only. |
