...
- **Tool schemas:** Register and advertise `Shell` with the existing `command`, `timeout_ms`, `temp_scope`, and agent-only `background` arguments. Register and advertise `ShellStatus` with the existing status-tool schema. Do not register or normalize a new agent `Bash` tool call: it remains an unknown tool and cannot execute. The tool description directs the model to the authoritative `shell:` field in the trusted environment system-prompt block and states that the bounded diagnostic applies only when that configured path has basename `sh` or `dash`.
...
- AC3.4: The built engine’s model-visible Shell description directs the model to the authoritative `shell:` field in the trusted environment system-prompt block and states that the bounded diagnostic applies only to `sh`/`dash` path basenames; permission, guardrail, trust, and secret-scrubbing behavior remain unchanged by a compatibility error.
