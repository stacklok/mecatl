...
**ADR:** [ADR 0364](../adr/0364-bounded-singleton-mcp-broker-correctness.md) — bounded receipts, admission, readiness, and deployment truth.
...
- [ADR 0364](../adr/0364-bounded-singleton-mcp-broker-correctness.md) keeps correctness process-local and bounded while closing ADR 0362's incomplete receipt contract.
- [ADR 0363](../adr/0363-single-replica-mcp-broker-topology.md) remains the one-replica `Recreate` topology; this work makes its readiness, drain, and deployment claims true.
...
remote-adapter contract in [ADR 0364](../adr/0364-bounded-singleton-mcp-broker-correctness.md)
...
the recovery boundary tightened by [ADR 0364](../adr/0364-bounded-singleton-mcp-broker-correctness.md).
...
is anchored by [ADR 0363](../adr/0363-single-replica-mcp-broker-topology.md) and the diagnostics
...
This makes the deployment consequences of [ADR 0363](../adr/0363-single-replica-mcp-broker-topology.md)
and [ADR 0364](../adr/0364-bounded-singleton-mcp-broker-correctness.md) executable.
...
- AC4.7: The ADR 0364 resource-ledger amendment inventories the drain coordinator and propagation waiter, active operations, attachment/lifecycle and Execute receipts, logical-session retention, every sweeper/timer, verifier/readiness resources, ToolHive process, and replacement remote clients, with owner, capacity/retention, close/join order, and restart disposition tied to their constructors and shutdown paths.
...
|---|---|---|
| Cross-process durable Execute receipts or exactly-once upstream effects | distributed broker phase | [ADR 0364](../adr/0364-bounded-singleton-mcp-broker-correctness.md) explicitly bounds receipts to one incarnation |
| Interchangeable replicas, callback failover, and shared outer authority | distributed broker phase | [ADR 0363](../adr/0363-single-replica-mcp-broker-topology.md) remains single-replica `Recreate` |
