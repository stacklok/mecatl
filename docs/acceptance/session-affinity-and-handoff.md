...
**ADR:** [ADR-0293](../adr/0293-session-correlation-and-affinity.md) — the exact end-to-end header contract, compatibility floor, lease-loss behavior, and infrastructure boundary.
...
- [ADR-0293](../adr/0293-session-correlation-and-affinity.md) supersedes, rather
...
- AC1.1: The canonical transport contract accepts a non-empty printable-ASCII HTTP field
  ...
  - verify: `TestADR_0293_SessionHeaderLegalValue` and provider parity vector tests
- AC1.2: OpenAI Responses, OpenAI Chat Completions, and Anthropic retain ADR-0216's
  ...
  - verify: `TestADR_0293_ProviderSessionHeaderExact`
- AC1.3: When the run context has no session ID or carries an illegal value, each
  ...
  behavior.
  - verify: `TestADR_0293_ProviderSessionHeaderOptional`
- AC1.4: A child, member, compaction, resumed, or recovered run projects the
  ...
  - verify: `TestADR_0293_ProviderUsesAuthoritativeRunContext`
- AC1.5: A race-enabled concurrent test shares one provider client between two distinct
  ...
  - verify: `TestADR_0293_ProviderSessionHeaderConcurrentIsolationRace` (run with `-race`)
...
- AC2.1: Every session-bound unary and server-streaming RPC accepts one legal
  ...
  byte-for-byte; missing metadata remains compatible. A debug `CreateSession` binds
  `debug_target_session_id`; ordinary creation has no client-selected source or placement identity.
...
- AC2.2: Duplicate metadata values, an illegal value, or a mismatch fail with
  ...
  - verify: `TestADR_0293_GRPCHeaderFailureIsNonDisclosing`
...
- AC2.4: After a valid first frame, approval, cancel, child-cancel, steer, and
  ...
  - verify: `TestADR_0293_ConverseControlsStaySessionBound`
- AC2.5: A missing header keeps existing `Converse` and unary/server-stream behavior
  byte-compatible.
  - verify: `TestADR_0293_GRPCMissingHeaderCompatibility`
...
- AC3.2: Duplicate values, illegal bytes, and byte-mismatched values are rejected before
  ...
  - verify: `TestADR_0293_HTTPHeaderFailureIsNonDisclosing`
- AC3.3: Escaped path IDs are compared after the server's normal path decoding; the
  ...
  - verify: `TestADR_0293_HTTPDecodedPathEquality`
- AC3.4: Header validation grants no access: authentication, caller ownership, and
  ...
  - verify: `TestADR_0293_AffinityHeaderGrantsNoAuthority`
...
- AC4.2: mecatui opens a session-bound `Converse` before prompt or retry and all later
  ...
  - verify: `TestADR_0293_MecatuiOpenConverseCompatibility`
...
- AC4.4: Additive TypeScript raw helpers can bind an explicit legal session ID on either
  ...
  - verify: `TestADR_0293_TypeScriptRawHelperCompatibility`
...
- AC5.2: The inventory test fails when a new session-bound mutator is added without an
  ...
  - verify: `TestADR_0293_AllSessionMutatorsClassified`
- AC5.3: `CloseSession` returns `FailedPrecondition` while a local run is active or
  ...
  - verify: `TestADR_0293_CloseGRPCAndHTTPRejectLiveOrAwaitingRun`
- AC5.4: A persisted awaiting session with no live local run retains its durable
  ...
  - verify: `TestADR_0293_CloseGRPCAndHTTPPreservePersistedAwaitingResumePoint`
...
- AC5.6: Lease-renewal loss while a local run is awaiting approval stops and invalidates
  ...
  - verify: `TestADR_0293_AwaitingLeaseLossRetractsLocalAskPreservesSnapshot`
...
- AC5.8: With no `SessionLease` configured, admission, mutation, close, drain, and
  ...
  - verify: `TestADR_0293_OptionalLeaseCompatibilityAndUnsupportedFallback`
- AC5.9: Lease identity remains scoped to the durable session across runs, retries,
  ...
  - verify: `TestADR_0293_LeaseRemainsSessionScoped`
...
- AC6.1: Once drain begins, new prompt, retry, resume, and out-of-band mutation entries
  ...
  shutdown path.
  - verify: `TestADR_0293_DrainStopsAdmissionBeforeOwnershipChange`
- AC6.2: Drain preserves a persisted awaiting session's durable `PendingAsk`; when no
  ...
  - verify: `TestADR_0293_DrainPreservesAwaitingResumePoint`
...
- AC6.4: If an executing run cannot join before the shutdown bound, mecak8s does not
  ...
  - verify: `TestADR_0293_DrainTimeoutRetainsLeaseForTTLTakeover`
- AC6.5: Mecak8s exposes separate positive bounds for Service drain, gRPC graceful stop,
  ...
  - verify: `TestADR_0293_TerminationBudgetFitsPodGracePeriod` and `TestADR_0293_TerminationGracePeriodIsConfigurableAndFitsDefaults`
...
- AC7.2: Before lease TTL expiry, every survivor request for that session cannot acquire
  ...
  provider call.
  - verify: `TestADR_0293_PreTTLRequestsCannotAcquireOrRun`
- AC7.3: After TTL expiry, exactly one modeled survivor acquires the Kubernetes lease;
  ...
  - verify: `TestADR_0293_PostTTLSingleSurvivorAcquires`
...
- AC7.5: The first provider request after continuation carries the exact same durable
  ...
  - verify: `TestADR_0293_HandoffEndToEndCorrelation`
...
- AC8.2: Helm tests and schema/lint gates pass without adding a gateway or affinity
  ...
  - verify: `TestADR_0293_HelmHasNoAffinityPolicySurface`
- AC8.3: architecture, usage, implementation notes, and public user docs describe the
  ...
  - verify: `TestADR_0293_DocumentationContract`
...
|---|---|---|
| Owner-to-owner forwarding of a live session or control | separate future ADR and acceptance plan | [ADR-0293](../adr/0293-session-correlation-and-affinity.md) deliberately chooses client retry plus lease handoff; forwarding needs an authenticated internal protocol |
...
| Exactly-once provider calls, tool execution, or other external side effects across owner death | external idempotency design, if required | [ADR-0293](../adr/0293-session-correlation-and-affinity.md) promises neither transactional external systems nor storage-level fencing |
| Using the affinity field for authentication, authorization, caller ownership, tracing, idempotency, cache identity, or a fencing token | permanently excluded from this contract | [ADR-0293](../adr/0293-session-correlation-and-affinity.md) |
...
The `TestADR_0293_*` tests pin durable decisions. The
