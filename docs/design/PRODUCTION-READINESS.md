...
|---|---|---|---|
| Multi-provider / multi-model | ✅ P0+P1+live listing & metadata · ✅ same-provider history carryover (issue #20) · ✅ experimental manual `openai-codex` subscription token (ADR 0104) · ✅ internal opaque credential-store substrate with local encrypted-file backend (issue #519; unwired, explicit key/root only) · ⛔ disk cache (P2) · ⛔ managed OAuth integration/key acquisition/per-client routing/remote or Kubernetes secret backends (P3) | [MULTI-PROVIDER.md](../adr/0016-multi-provider.md) · [0104](../adr/0104-openai-subscription-manual-token.md) · [0108](../adr/0108-credential-store.md) | [providers](../architecture/providers.md) · [credential store](../architecture.md#internal-credential-store) |
| MCP authorization-code client | ✅ official Go SDK public profile qualified offline (ADR 0109) · ✅ adapter-local persistent controller for constrained preregistered/CIMD embeddings (ADR 0110; unwired by default) · ✅ opt-in host loopback runtime + one-shot authenticated-connect/persistence operation (ADR 0112) · ⛔ canonical profile resolver, credential-store key acquisition, `mecated mcp login` wiring, DCR, and enforceable pre-presentation SDK metadata-profile gate (#523/upstream) | [0109](../adr/0109-mcp-oauth-sdk-profile.md) · [0110](../adr/0110-mcp-oauth-controller.md) · [0112](../adr/0112-mcp-oauth-loopback-runtime.md) | [extensibility](../architecture/extensibility.md) |
