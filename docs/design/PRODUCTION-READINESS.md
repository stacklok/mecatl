...
| MCP authorization-code client | ✅ official Go SDK constrained public profile qualified offline (ADR 0109) · ✅ adapter controller, mutable/read-only credential sources, loopback runtime, strict operator profiles, three-root runtime wiring, and `mecated mcp login` for mutable local stores (ADRs 0110–0113) · ✅ hermetic login→two-process warm restore→refresh rotation→reconnect acceptance gate (#524) · ⛔ DCR and enforceable pre-presentation upstream SDK metadata-profile gates; ACP cannot supply OAuth profiles/authorization, and no per-session/inline/discovered OAuth | [0109](../adr/0109-mcp-oauth-sdk-profile.md) · [0110](../adr/0110-mcp-oauth-controller.md) · [0111](../adr/0111-read-only-credential-source.md) · [0112](../adr/0112-mcp-oauth-loopback-runtime.md) · [0113](../adr/0113-operator-mcp-auth-profiles.md) | [extensibility](../architecture/extensibility.md) |
