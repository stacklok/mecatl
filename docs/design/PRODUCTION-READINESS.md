...
| Dedicated stored-session debugger | ✅ target-bound durable no-fs debug session, exact `{InspectSession}` catalog, bounded fenced status/transcript/activity/performance/network evidence, persisted sanitized resilience-attempt decisions, restart-safe rehydration, hierarchical first prompt, privacy disclosure, and DEBUG identity · ⛔ live target following, raw audit/tool records, raw logs/pprof, packet capture, support bundles | [0254](../adr/0254-session-debugger-admin-transport.md) · [0255](../adr/0255-sanitized-network-attempt-evidence.md) | [overview](../architecture.md) · [TUI](../tui.md#debug-a-stored-session) · [observability](../architecture/observability.md) |
