...
| Direct MCP source reconciliation + session refresh | 🔨 PR #1673: ToolHive workloads and MCP `list_changed` notifications republish an immutable direct-MCP runtime; runs keep the runtime they started with; new tool names reach an existing session only through the owner's `RefreshMcpSources` (mecatui `/mcp-refresh`) | [0353](../adr/0353-mcp-source-reconciliation.md) | [extensibility](../architecture/extensibility.md) |
...
| MCP client reconnect on connection drop | ✅ | client-side; one bounded reconnect/retry for concrete session loss only; structured JSON-RPC 400/404 and HTTP 429/502/503/504 remain one-call failures and are never replayed (ADRs 0056, 0114); consumes `notifications/*` over the standalone SSE stream (ADR 0057); the catalog is republished on notification per ADR 0353 |
