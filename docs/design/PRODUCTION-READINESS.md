...
| Broker credential continuity after replacement | 🟡 | Shipped: encrypted ToolHive custody in a dedicated broker Redis (external or chart-managed singleton StatefulSet), Stage/Commit at enrollment, fresh B2 authority after confirmed broker-instance loss, and invalidation on delete/cancel/rotation/expiry ([ADR 0329](../adr/0329-broker-replacement-fresh-authority.md)). A composed two-broker proof covers recovery, crash and ambiguous-save ordering, the host assertion boundary, pre-prompt gating, no execution replay, and profile changes. Deferred: a JWT-verified gRPC peer in that proof and the real-Redis ACL command oracle; HA/takeover, callback recovery, purge, index cleanup, immediate revocation (a copied recovered bearer lives up to two minutes), restore/rollback detection, zero-loss storage, and ADR 0327's general NetworkPolicy drift. A broker profile change is reported as gRPC reason 8 and invalidates the stored custody. |
