...
| Broker credential continuity after replacement | 🟡 | Shipped: encrypted ToolHive custody in a dedicated broker Redis (external or chart-managed singleton StatefulSet), Stage/Commit at enrollment, fresh B2 authority after confirmed broker-instance loss, and invalidation on delete/cancel/rotation/expiry ([ADR 0329](../adr/0329-broker-replacement-fresh-authority.md)). A composed two-broker proof covers replacement recovery, fresh outer state, fixed retention, and interrupted browser flows. Deferred: the remaining composed Scenario 2/3 cases (parked-call interruption, crash ordering, JWT-verified peer) and the real-Redis ACL command oracle; HA/takeover, callback recovery, purge, index cleanup, immediate revocation (a copied recovered bearer lives up to two minutes), restore/rollback detection, zero-loss storage, and ADR 0327's general NetworkPolicy drift. A broker profile change is reported as gRPC reason 8 and invalidates the stored custody. |
