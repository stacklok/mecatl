...
|---|---:|---|
| review evidence | 16 handles; 400,000 aggregate preview bytes | One handle can expose at most the native Read-shaped 25,000-byte / 2,000-line preview. Sixteen realistic ASCII source/code previews are about 100k tokens at four bytes/token, leaving space beneath the engine's 128k unknown-model floor for the fixed rubric, call, trajectory, and answer. This is workload calibration, not a universal byte-to-token guarantee: model tokenization and source encoding vary, and unavailable capacity remains incomplete/unresolved. |
...
The evidence benchmark admits sixteen 25,000-byte bounded backends, rejects the seventeenth, and asserts zero body reads during inventory/preflight. It therefore measures metadata allocation rather than silently allocating a full `Workspace.ReadVersion` body and truncating it. The production evidence seam requires `Size` plus bounded `ReadAt`; a source without that interface is incomplete and receives no body handle.
