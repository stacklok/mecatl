...
- `UploadArtifact(stream UploadArtifactRequest) → UploadArtifactResponse` and
  `DownloadArtifact(DownloadArtifactRequest) → stream DownloadArtifactResponse`
  carry private, session-owned bytes in bounded chunks. They accept only
  `application/pdf`. The service authorizes the exact session before
  object-store access. A PDF prompt carries only the returned artifact ID, and
  a recognized PDF tool result projects `ContentBlock.KIND_ARTIFACT` with an ID
  and bounded metadata after its bytes are externalized. Both RPCs require
  configured artifact storage (`ServerCapabilities.artifacts`); PDF prompt input
  also requires the selected model's PDF capability.
...
| `POST /v1/sessions/{id}/artifacts?name=...` | `UploadArtifact` | streamed PDF body; returns private artifact metadata |
| `GET /v1/sessions/{id}/artifacts/{artifact_id}` | `DownloadArtifact` | streamed PDF body after ownership check |
