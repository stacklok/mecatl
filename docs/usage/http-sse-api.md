...
| `GET /v1/sessions/{id}` | — | `200` authoritative session snapshot, including title/provenance, title-generation lifecycle, and canonical auxiliary token usage when present |
...
The per-run prompt SSE stream has no out-of-band title notification. A client that
needs automatic title changes reads the authoritative `GET /v1/sessions/{id}` snapshot
or reconciles the durable `/events` or `/watch` stream when an EventLog is configured;
a `session.title` event contains title lifecycle metadata only, never title-source
prompts or provider-error text.
