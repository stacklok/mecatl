...
| `--memory-consolidate-interval` | `0` | interval for optional background consolidation of the per-project memory store; `0` disables. Planning only names existing-key survivor/superseded relationships. Automatic retirement requires lifecycle convergence support and byte-identical active value+description duplicates; base-only/non-identical proposals are skipped. Only meaningful with `--memory-dir`. |
...
| `--user-model-consolidate-interval` | `0` | independently authorize process-wide background consolidation of the cross-project user-model store's `user/` namespace; `0` disables. It uses the same exact-duplicate lifecycle-retirement boundary as project memory; base-only/non-identical proposals are skipped. A project `learning.mode: off` cannot suppress a positive operator schedule. |
