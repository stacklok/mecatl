...
| `--no-project-trust` | `false` | **OPERATOR-TIER ONLY.** Suppress project-tier steering ingestion (AGENTS.md/CLAUDE.md, project rules/agents/skills/soul/commands, ALLOW rules, git snapshot) WITHOUT lowering `TrustProject` — so `--posture auto`'s approvals and the read-only subagent shell stay active while the cloned repo's own steering is dropped. Use on scheduler runs over third-party repos where the git ref is an untrusted supply-chain surface; supply your own instructions via `--instructions`. The pin is a separate dimension from `--trust-project`; the two combine via `ingestProjectTier(cfg) = cfg.TrustProject && !cfg.NoProjectIngest`. An explicit `--no-project-trust=false` is also supported. Operator-global `settings.yaml` key `no-project-trust:` is a peer of the flag (CLI out-ranks it). **See the workspace-trust note below.** |
