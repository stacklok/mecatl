...
| `--learning-store-url` | `""` | `host:port` of one distributed-learning gRPC driver. Startup capability negotiation requires the complete `AttemptRepositoryService` + `ProposalRepositoryService` + `SkillRepositoryService` set; a partial or legacy driver fails startup instead of silently mixing remote and local repositories. Principal/project repository partitions are opaque hashes on this transport. Full workload-authenticated ownership enforcement remains a separate deployment requirement. |
