...
| `--posture` | `auto` | operator posture ladder (strict < trusted < auto < yolo); `auto` is the recommended unattended single-tenant tier. On this headless root project-tier ingestion is opt-in — only an explicit `--trust-project` admits the repo's steering; the fail-safe default gives allow-all approvals + a working child shell with NO repo steering ([ADR 0094](../adr/0094-opt-in-project-ingestion.md)). |
