...
| `--max-run-tokens` / `--max-team-tokens` | `0` | per-engine / team-wide cumulative token ceilings (`0` = unlimited). `--max-run-tokens` is inherited independently by child engines; it does not cap delegation-tree spend. |
