...
| `--drain-addr` | `0.0.0.0:8082` | Plaintext drain-only listener serving only `GET /drain` for the kubelet preStop hook; it remains outside the Service and requires operator NetworkPolicy/mesh isolation against direct Pod-IP access ([ADR 0288](../adr/0288-mecak8s-drain-listener.md)). |
