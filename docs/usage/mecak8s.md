...
| `--no-project-trust` | `false` | **OPERATOR-TIER ONLY.** Suppress project-tier steering ingestion without lowering `TrustProject`. Use on unattended cloned-repo runs to keep `auto`'s approvals and the subagent shell while dropping the cloned repo's AGENTS.md/steering. Supply your own instructions via `--instructions`. (See [workspace trust](workspace-trust.md#suppressing-project-tier-ingestion---no-project-trust).) |
