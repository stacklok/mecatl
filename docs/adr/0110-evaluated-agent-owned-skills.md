...
Ship `engine/adapter/skillfs.AtomicCatalog` and `LiveTool` beside the unchanged `SkillSource`/`FSSource` snapshot contracts. A complete immutable generation contains metadata, activation bodies, and lookup indexes behind one atomic pointer. External/operator/project/user/driver skills merge first; active learned versions merge last, and a collision leaves the external skill visible while activation is refused for the learned version. Build shares this one pointer through `assembleCatalog`, including selector and no-fs catalogs, and `ListSkills` reads it live. Body-only v1 adds no dynamic read root.
...
An active learned version becomes visible without restart, including to already-built per-session catalogs. Shared catalogs publish only the ownerless deployment partition and the trusted launch-root project partition; other caller/project partitions remain staged until a partition-bound catalog is available, preventing cross-caller instruction leakage. The conservative default evaluator means automatic activation requires an explicitly wired evaluator that returns PASS.
