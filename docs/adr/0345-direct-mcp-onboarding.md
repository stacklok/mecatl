...
Native keyring support adds platform behavior and a process-lifetime dependency already used by other root-module credential consumers. File-key fallback improves availability but cannot protect against a process running as the same OS account that can read both key and ciphertext.
