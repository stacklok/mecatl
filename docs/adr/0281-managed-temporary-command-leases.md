...
`temporary_storage.managed_root` names one private managed namespace beneath the inherited system temporary directory. It is organized first by a shared stable workspace-instance identity, giving an operator one place to find every resource associated with that workspace. The identity contract belongs to this ADR: a local workspace derives it from its physical root device/inode plus worktree-specific Git-administration identity (or an unambiguous non-Git creation marker); a remote, virtual, or on-demand-sandbox workspace provider must supply an opaque ID stable across rematerialization and reconnection, but different for a replacement workspace or isolated worktree/copy. It is distinct from a materialization path and `session.EnvironmentRef`.
...
Each command/job lease directory is named `<kind>-<allocation-id>`, where `kind` is `cmd` or `job` and `allocation-id` is an opaque, cryptographically random 128-bit identifier. The allocation ID is never derived from command text, a PID, or a timestamp. The manifest repeats the directory's ID and kind; a reaper rejects a mismatch.
