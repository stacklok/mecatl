# ADR 0290 — Asynchronous session-title generation

- Status: Accepted
- Date: 2026-08-30
- Scope: session-title lifecycle and provenance, mecatui title UX, title-specific server operation, composition model-slot selection, coordinator lifecycle, and title events
- Supersedes: none
- Superseded by: none
...
The engine owns only durable session title truth: `SessionTitle` is the canonical generated-title provenance and lifecycle projection, guarded aggregate title mutation, bounded title-source/lifecycle state, and the `session.title` event value. Title lifecycle attempts retain only identity, outcome, and time. Token accounting for title calls is governed separately by ADR 0291.
