...
- Status: Proposed
- Date: 2026-09-11
- Scope: redisstore client-generation ownership, follower admission and shutdown, and mecak8s operator sizing
- Supersedes: ADR 0250's shared-pool sizing deferral, and ADR 0240's no-force-close rule for isolated follow clients only; both ADRs remain authoritative elsewhere
- Superseded by: None
...
ToolHive Core v0.0.46 exposes `PoolSize` and `MaxActiveConns` through the supported
`redisconn.Config`, providing the connection-layer seam needed to make the follow pool a hard,
operator-visible bound. Issue [#876](https://github.com/stacklok/mecatl/issues/876) records the
...
1. Each Redis credential generation owns an atomic pair of clients: one durability client using
   ...
   generation. Migrate redisstore from the deprecated `toolhive-core/redis` facade to
   `toolhive-core/redisconn` v0.0.46.

2. Configure the follow client with both `PoolSize` and `MaxActiveConns` equal to the effective
...
3. Give each Store one fail-fast follower admission limit, held once for the full lifetime of a
...
4. Make the Store own follower cancellation and joining. Linearize close with admission so a
...
5. Expose `--redis-follow-pool-size` and `--redis-max-followers` on mecak8s and
...
6. Inventory two distinct process-lifetime resources in ADR 0027 List 1: the isolated follow
