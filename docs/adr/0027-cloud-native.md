...
| 78 | Session-owned artifact objects, including staged uploads and private fork copies (`application/pdf` only; ADR 0367) | `pdfartifact.Store` over a dedicated private S3-compatible bucket, composed by `mecak8s` | durable object store under the artifact-generic `artifacts/v1/` prefix, keyed by a hash of the owning session ID and a server-generated artifact ID | a staging marker precedes each write; an unused upload becomes unusable after 24 hours; fork copies referenced objects before successor publication, discards unpublished copies on a pre-publication failure, and preserves a published successor's objects after a later failure; deletion and retention enqueue prefix cleanup, while reconciliation removes failed-upload and failed-fork residue after an age grace and authoritative snapshot/lease checks | **persisted** outside the pods: a surviving replica opens referenced objects through the same bucket and session authorization; stale objects are recovered through Redis staging and deletion records plus object reconciliation | `docs/adr/0367-session-owned-pdf-artifacts.md` (`Decision`); `docs/acceptance/sdk-pdf-artifacts.md` (interface and AC4) |
| 79 | Bounded artifact metadata, upload staging markers, and deletion outbox | `redisstore.Store` alongside the session snapshot | durable Redis keyspace with artifact-generic `mecatl:artifacts:` metadata/outbox keys and `mecatl:artifact-upload-active:` / `mecatl:artifact-fork-source:` lease keys, scoped to the owning session; metadata includes validated `mime_type: application/pdf` | successful snapshot saves commit newly referenced IDs; an interrupted marker update is repaired from the authoritative snapshot. Atomic session deletion and retention pruning revoke new reads and enqueue prefix cleanup with snapshot removal; the cleanup worker retries the outbox until object deletion settles | **persisted** across replica failover: staging, references, and cleanup intent reload from Redis; PDF bytes remain in the object store and never enter these records | `docs/adr/0367-session-owned-pdf-artifacts.md` (`Decision`); `docs/acceptance/sdk-pdf-artifacts.md` (`Events / persistence`) |
| 80 | Artifact cleanup and reconciliation worker | `mecak8s` artifact lifecycle | process worker over the shared Redis and S3-compatible stores | shutdown stops local work; pending outbox entries remain durable. Reconciliation waits for an age grace, excludes active session and fork leases, rereads the authoritative snapshot before deleting an apparently unreferenced object, and retries if Redis is unavailable | **reset-by-design / derived from durable state**: a replacement worker rediscovers Redis staging/outbox records and abandoned objects; no process-local scan cursor is authority | `docs/adr/0367-session-owned-pdf-artifacts.md` (`Decision`); `docs/acceptance/sdk-pdf-artifacts.md` (`Events / persistence`) |
...
| 50 | Session artifact references, staged-upload eligibility, and deletion intent (ADR 0367; List 1 rows 78–80) | the session snapshot and durable events carry artifact IDs and bounded metadata for recognized PDFs; Redis holds staging markers and a deletion outbox; a private S3-compatible store owns the bytes | a replacement replica reloads the references and staging/deletion state from Redis and resolves bytes from the shared bucket. An unused upload is unusable after 24 hours even if cleanup has not run; interrupted cleanup remains in the outbox, and deletion revokes new reads immediately | persist references and metadata with the session, persist staging and cleanup intent in Redis, and derive process-local worker progress by reconciliation against the authoritative snapshot and object store. Fork copies referenced objects before successor publication; Clear copies none | ADR 0367 |
