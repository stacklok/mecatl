...
| 8 | jsonlstore v2 current snapshot + readable v1 history + `.tools.jsonl` audit + `.events.jsonl` event log (Phase 3a) + stable per-family flock/temp generations (issue #586) + derivative inventory catalog (issue #587) | jsonlstore | per-session files plus one store-wide adapter-private catalog | files open per call and close immediately; v2 Save holds the stable owner-only `.family.lock` flock from orphan-temp reaping through same-directory file sync, atomic rename, and directory sync. Temps carry a random process-owner token plus monotonic generation; startup non-blockingly reaps only protocol-valid inactive generations after taking that family lock, and Save reaps them after waiting for the lock. Ordinary failures remove their own temp; lock sentinels persist. `Delete` removes sidecars before snapshots. The owner-only catalog is atomically replaced after a rebuild and contains only bounded discovery metadata plus a fingerprint, never transcript/tool/event content | persisted (v2 `Load` reads one authoritative current snapshot and never falls back to coexisting v1; event/tool sidecars remain cumulative); locks are reconstructible synchronization, and a crashed holder's flock is OS-released so the next startup/Save removes its temp; `SnapshotDurability` reports weaker filesystems explicitly. The catalog is derivative and reconstructible from v2 top-level metadata headers or bounded v1 tails; every ready read revalidates snapshot directory-entry metadata, so restart, corruption, and another process's ordinary family changes cannot make it transcript authority or leave an unchecked process-local view | `internal/adapter/store/jsonlstore/jsonlstore.go` (`withSnapshotFamilyLock`, `reapSnapshotTempsAtStartup`, `replaceCurrentSnapshot`, `SnapshotDurability`); `internal/adapter/store/jsonlstore/inventory_catalog.go` (`inventoryCatalog`, `inventoryFingerprint`); `internal/adapter/store/jsonlstore/metalist.go` (`discoveryMetaList`) |
