...
| 76 | Boat placement provider API client, per-binding sandbox handles, and guest-side staging files | `internal/adapter/boatenv.Provider`, injected through `app.Config.PlacementProvider` (default `app.Build` never constructs it) | process for the client; binding for each sandbox handle; call for guest staging and snapshot files | the client runs no goroutine and has no client-wide timeout: every request is bounded by its context, and a command by its own time limit plus a fixed grace. A handle holds only a readiness bit. The guest helper deletes a staged request once it has read it and a snapshot on its final range read. The remote sandbox is archived by the provisional binding's `Close` and by its TTL, which every resume re-arms | reconstructible: the client from configuration. A handle's readiness is re-derived lazily by one `GET` on the first operation after `Reattach`, and the sandbox and its filesystem persist remotely under the exact `EnvironmentRef`. **List 2 gains no row:** a handle carries no session or run state | `internal/adapter/boatenv/client.go` (`newAPIClient`); `internal/adapter/boatenv/sandbox.go` (`sandbox`); `internal/adapter/boatenv/helper.go` (`fetchSnapshot`) |
