...
**MicroVM lifecycle re-audit (List 1 / List 2 - ADR 0364).** The production
...
execution owner in row 12. Build shutdown retires source generations. Client-local exact-ref
counts cover only owners sharing that Client; they do not protect an acquisition held by
another process. The approved, not-yet-implemented connection-owned acquisition contract in
[the MicroVM plan](../acceptance/microvm-execution-environments.md) replaces that arbitration
with List 1 row 83. Service binding and source-generation references remain separate
lifetimes. List 2 gains no row: source selection and generations
...
**MicroVM operations re-audit (List 1 / List 2 - ADR 0364).** The
