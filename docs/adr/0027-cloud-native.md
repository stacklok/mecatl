...
| 48 | Durable agent-owned skill manifest, immutable version files, and stable flock (issue #510, ADR 0111) | `app.Build` constructs one lazy `skillstore.Store` beside the user-model store and shares it with reflection, server lifecycle APIs, and every catalog assembly | process handle over principal/project-partitioned durable data | the stable `skills.lock` flock is acquired and released per operation; files are opened per call. `manifest.json` and each content-addressed `versions/<version>/SKILL.md` use temp-file fsync, rename, and directory fsync. Empty startup is lazy. A crash before manifest commit can leave only an unreferenced immutable version, which reopen safely ignores | **persisted**: bounded lifecycle metadata, provenance, evaluations, receipts, active selection, and immutable bodies reload together. Cross-store proposal linkage is create-then-CAS-link and reconciles by ProposalID/SkillID without a duplicate. See List 2 row 31 | `internal/adapter/skillstore/store.go` (`Store`, `locked`, `save`, `ensureVersion`); `internal/adapter/skillstore/materialize.go` (`MaterializeProposal`) |
| 49 | Live learned-skill immutable generation pointer (issue #510, ADR 0111) | `app.Build` constructs one `skillfs.AtomicCatalog`; `catalogAssets` shares it with every `assembleCatalog` call and the server's live `ListSkills` lister | process (one atomic pointer to a bounded immutable path-free external-source/body-only-learned generation; no goroutine, watcher, path, asset materialization, or workspace root) | a refresh builds a complete generation off to the side and performs one atomic pointer store. Post-commit publication is bounded and cancel-detached; failure revokes the learned name fail-safe, and startup plus live-list refresh reconcile from the repository. Build teardown needs no explicit close; old generations become garbage after in-flight readers release them | **derived** from List 2 row 31's active durable versions plus the build-time immutable external skill snapshot. Restart reconstructs it before serving; a crash after manifest activation but before publication self-heals on restart. No separate persisted state | `engine/adapter/skillfs/atomic.go` (`AtomicCatalog`, `Refresh`, `LiveTool`); `internal/app/learned_skills.go` (`learnedSkillPublisher`); `internal/app/catalog.go` (`registerSkillFamily`) |
