...
| 76 | Provider resilience breaker, suspended stream-iterator probe, and precommit recovery timer | each `internal/adapter/llmresilience.resilientProvider` owns its breaker; `Stream` owns the iterator probe and recovery window for one model step | provider lifetime for the breaker; one `Stream` call for the probe and timer | the breaker is concurrency-safe and has no goroutine. Iterator cleanup stops a suspended `iter.Pull2` sequence on cancellation or completion. Semantic commit or terminal settlement stops the recovery timer; caller cancellation also releases the probe and timer | reset-by-design: rebuilding a provider starts with a closed breaker; an interrupted Stream loses its tentative chunks, retry timing, and timer. No recovery state is persisted or resumed after restart | `internal/adapter/llmresilience/llmresilience.go` (`resilientProvider`, `Stream`, `pumpAttempt`); `internal/adapter/llmresilience/recovery.go` (`recoveryWindow`, `cancelablePull`) |
...
**Server provider recovery re-audit (List 1 / List 2).** List 1 row 76 inventories the provider-lifetime breaker and the Stream-owned iterator probe and precommit recovery timer. The breaker resets closed when a provider is rebuilt. Iterator, tentative output, and timer state are released at semantic commit, terminal settlement, or cancellation and disappear on process loss. List 2 gains no row: these resources carry no durable session or run authority, and restarting never resumes a recovery window.
