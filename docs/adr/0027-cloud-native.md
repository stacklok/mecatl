...
| 40 | Debug-session kind and exact target lineage (ADR 0254), plus the network-evidence read path (ADR 0255) | `session.Session.Kind` plus `Session.Relationship.DebugTargetID`, round-tripped by sessnap and every SessionStore/driver; sanitized `network.attempt` events use the existing EventLog | restart reloads the debugger's own conversation and exact target binding. `Service.rehydrateSession` rebuilds only through `DebugSessionEngine`; invalid no-fs/empty-workspace metadata, missing factory, or unavailable target fails closed. The target itself is never loaded through run entry, resumed, or leased. Already-relayed network attempts remain in the configured EventLog across restart; in-memory EventLog intentionally does not | **persist-in-snapshot and derive engine; persist evidence in existing EventLog**: kind/lineage/profile are durable; the exact `{InspectSession}` engine and no-fs environment are reconstructed from them. No in-memory target binding or new attempt store is trusted across restart; backend absence/retention/read faults surface as unavailable/incomplete evidence | 0254, 0255 |
