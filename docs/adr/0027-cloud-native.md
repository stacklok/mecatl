...
| 61 | mecak8s projected-file watcher and atomic last-valid server certificate (issue #789, ADR 0239) | `cmd/mecak8s` owns one `filewatch.Watcher` through `certificateReloader`; the reusable adapter owns its fsnotify handle, debounce timer, and goroutine | process (only when server TLS files are configured) | `serve` defers the lifecycle close until its gRPC and HTTP shutdown completes; close is idempotent, cancels the pending timer, closes fsnotify, and joins the goroutine. A valid reload atomically replaces the fully parsed pointer; invalid/mismatched material retains the prior object | **reconstructible**: restart synchronously validates the currently projected cert/key before listening. Certificate state is transport configuration, not session/run state, so List 2 gains no row. Client CA remains restart-required | `internal/adapter/filewatch/filewatch.go` (`Watcher`, `New`, `Close`); `cmd/mecak8s/serve.go` (`certificateReloader`, `buildTLSConfig`) |
| 62 | Redis client generations plus optional projected-file watcher and bounded reload worker (issue #789, ADR 0239) | `redisstore.Store` owns the generation manager; mecak8s enables its reload lifecycle when a Redis CA/username/password file is configured | process; one current client plus only retired generations still leased by in-flight operations/iterators/migration locks | every operation takes a short manager lease without holding its mutex during Redis I/O. A verified candidate swaps atomically; rejected candidates close immediately and retired clients close after their final lease. `Store.Close` first closes the watcher, cancels and joins retry, then rejects acquisitions/swaps and waits for leased generations before closing clients. No configured files means no watcher/retry goroutine | **reconstructible**: restart reads and validates the currently projected complete file set and creates one fresh client. Generations and retry counters are transport state, not session/run state, so List 2 gains no row | `internal/adapter/redisstore/generation.go` (`clientGenerations`, `pin`); `internal/adapter/redisstore/reload.go` (`startReloadLifecycle`, `runReload`, `reloadCandidate`); `internal/adapter/redisstore/migration.go` (`AcquireSessionMigrationJob`) |

**mecak8s credential-reload re-audit (List 1 / List 2 — issue #789, ADR 0239).**
List 1 rows 61–62 inventory both optional projected-file watcher lifecycles, the
last-valid server certificate, the Redis reload retry worker, and the replaceable Redis
client generations. Serving joins the certificate watcher after listener shutdown; the
Redis Store closes its watcher and joins retry before rejecting leases/swaps and waiting for
in-flight generations. Restart reconstructs both last-valid values from the currently
mounted files. These are transport credentials, not session or run state, so List 2 gains no
row.
