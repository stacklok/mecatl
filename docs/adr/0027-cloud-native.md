...
| 7 | Redis snapshots + event/tool sidecars + derivative metadata indexes | redisstore | managed Redis keyspace; one global and owner-specific sorted metadata index plus generation fields | Save atomically replaces the snapshot and its bounded metadata row; Delete atomically removes snapshot, metadata membership, and event/tool sidecars. An explicit authenticated migration job atomically fences each bounded repair with its exact lock token; stable inspection proves every valid snapshot's derived global/owner membership before a constant-work generation/count CAS publishes ready. Invalid snapshots complete with failures and keep paging unavailable until operator repair and a fresh plan. Redis owns persistence/eviction policy and connection cleanup remains `Store.Close` | persisted. The index is derivative from sessnap but never rebuilt by an inventory page: a store opened with legacy snapshots reports paging unsupported until the resumable maintenance job completes; durable caller-bound progress and generation checks survive process replacement | `internal/adapter/redisstore/redisstore.go` (`Save`, `Delete`); `internal/adapter/redisstore/metadata_index.go` (`pageSessionMetadata`); `internal/adapter/redisstore/migration.go` (`FinalizeSessionMigrationCoverage`) |
...
| 59 | Engine-child process liveness registry (`sessionLiveness`) | `app.Build`; shared by main/per-session engine `Deps.SessionLiveness`, direct `RunTeam` supervisors via `agent.WithMemberLiveness`, and `server.Service` | process (reference-counted set of currently queued/running Subagent, Parallel-branch, and Team-member session IDs) | each child registry entry owns one idempotent release, invoked by the existing terminal and pre-start-abort chokepoints; background Subagents remain registered until their detached drive ends. Direct supervisors register every member before round planning and retain it across between-round/synthesis work until `cleanupAll`; Team-tool members receive equivalent coverage through the parent child registry. Background Bash is excluded because it has no session. The map empties naturally; Build/process teardown drops only entries whose child goroutine is also being cancelled/joined by row 13 | **reset-by-design**: it is instantaneous process-local exclusion, not session state. A crash loses both child execution and registry; persisted snapshots remain, while cross-process destructive exclusion continues to use `SessionLease` | `internal/app/session_liveness.go` (`sessionLiveness`); `engine/agent/childregistry.go` (`register`, `markDoneResult`, `remove`); `engine/agent/teamsupervisor.go` (`WithMemberLiveness`, `Run`, `cleanupAll`); `internal/adapter/server/service.go` (`IsLive`) |
...
| 33 | Redis derivative session inventory rows, owner indexes, cursor generations, and resumable adoption jobs (List 1 row 7) | managed Redis keys updated atomically with each current snapshot plus caller-bound maintenance checkpoints | restart reloads the persisted index, generations, and job; an older Redis database with snapshots but no complete index is marked stale at adapter startup | **derive via explicit maintenance / fail honest**: current Save and Delete maintain the projection and advance the rebuild generation; authenticated bounded migration batches CAS-install legacy rows. Paging and cleanup stay unsupported until a stable-generation complete-coverage script atomically publishes ready | ADR 0226 |
