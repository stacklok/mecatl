...
**MicroVM lifecycle re-audit (List 1 / List 2 - ADR 0368).** The production
...
**MicroVM harness-context source re-audit (List 1 / List 2 - ADR 0359).** A selected
`repository` instruction or command source owns a read-only borrow of the session's exact
placement (List 1 row 82). Each source cleanup releases only its borrow; failed or cancelled
binding closes attempt-owned borrows without deleting retained placement or closing the
execution owner in row 12. Build shutdown retires source generations. Each exact reattachment
owns the connection-backed acquisition in List 1 row 83; microvmd keeps the guest registration
alive across clients until its last owner and operation release it. Service binding and
source-generation references remain separate lifetimes. List 2 gains no row: source selection
and generations
are intentionally process-local. After restart, the existing session or scheduled fire
reauthorizes its durable `EnvironmentRef` and rebinds under current operator context policy;
no context reference or source borrow is persisted.

**MicroVM Darwin lifecycle re-audit (List 1 / List 2 - proposed ADR 0365).**
Darwin extends the existing row 73 launch-attempt resource with one launch-owner supervisor,
a daemon-liveness pipe, the supervisor and runner copies of the exact attempt flock, and the
existing durable intent/receipt files. The daemon closes its writer on exit; EOF makes the
supervisor TERM/KILL and reap its direct runner. Normal stop closes the pipe and joins the
supervisor. If the supervisor dies, the runner's inherited flock remains the fail-closed
liveness authority until the runner exits or an operator recovers it. No stored PID is
signaled. List 2 gains no row: durable attempt records already belong to row 47, while the
supervisor, pipe descriptors, and held flock copies are process resources reconstructed only
for a new launch.
...
**MicroVM operations re-audit (List 1 / List 2 - ADR 0368).** The
...
| 18 | Provider discovery owner: frozen borrowed listers, owner context/cancel, per-provider attempt map with deadline/ready channel/returned and terminal flags, cooldown timestamps, atomic immutable observations/outcomes/model-status projection and captured default facts, completion-tail/publication locks, worker join group, and one-shot startup timer/worker (ADR 0360) | `app.Build`, one owner per server/pod Build, including the embedded mecatui server; connected clients use that server's owner and each mecak8s replica owns its own instance even with shared durable storage | process; at most one occupied fetch slot per available lister provider, with one latest immutable snapshot | Bootstrap, startup, ListModels, and Service admission share the owner. Drain only stops/cancels run admission; `providerDiscovery.Close` prohibits new attempts/publications, cancels and joins fetch/startup workers and their deadline callbacks before borrowed credentials close, on Build failure and normal Close. `fetch` stops or joins its deadline callback and marks return; timeout publishes before return but keeps the slot occupied. Publication and waiter notification precede synchronous diagnostics. Physical Close requires cooperative lister cancellation and synchronous diagnostics delivery to return, because Close also joins the worker delivering diagnostics. Attempt/admission deadlines do not bound a blocked collaborator's cleanup. Per-request fan-out waiters join within their caller bound; cancellation ends only their wait. No-lister/native-only startup starts no workers. Replaced snapshots become reclaimable when readers release them | **reset-by-design**: the next Build borrows freshly registered listers and starts without observations, attempts, signals, or cooldowns. Catalog/config floors are derived; native authenticated providers remain demand-only. Replica evidence, cooldowns, and traffic remain Build-local; no cache reattachment or durable discovery state | `internal/app/provider_discovery.go` (`providerDiscovery`, `discoveryAttempt`, `request`, `fetch`, `Close`); `internal/app/build.go` (`Build`) |
| | _(traceability)_ `liveMetaStore` is only a read view over row 18's owner. The engine context resolver, resolved-model echo, picker, DiscoverModels, and schedule validator read this owner's accepted snapshots; each resolution/combined response captures one view, while separate calls can observe different generations. Constructed session capabilities/effort are not generation-bound or rebuilt by publication. Failed/empty latest outcomes retain positive last-good metadata but cannot supply healthy omission evidence. The echo is 0 for a blocked target; the engine's defensive 128000 floor is not admission evidence. No global settlement channel, prior-rejection history, independent metadata/outcome store, or writable inventory survives the migration. | | | | | `internal/app/livemeta.go`; `internal/app/provider_discovery.go` (`resolveModelWindow`); `internal/adapter/server/service.go` (`ListModelSnapshot`) |
...
| 76 | Mecatui prepared admission submission and recovery controls (ADR 0360) | `ui.Model` | one active session/stream generation; one validated text/media send plus detached editable paste/image staging | SessionInit, discard, session replacement/reset, exit, or unrelated terminal failure releases the record. Back transfers editable staging to the editor; Retry transfers the same record to the next stream generation without re-expansion or file/clipboard reads. Deleted markers are excluded using the existing staging filters | **reset-by-design**: client-memory-only; no durable draft, automatic replay, or server pending record. Existing text/paste limits and 10 MiB per-part, 20 MiB aggregate, 16-part media validation precede capture | `cmd/mecatui/ui/admission.go` (`admissionSubmission`, `retainAdmission`, `restoreAdmission`); `cmd/mecatui/client/media.go` (`Clone`); `cmd/mecatui/ui/update.go` (`submitPrompt`, `endRun`) |
...
| 78 | contextual guardrail checker-route health (`guardrailRouteHealth`) | `app.Build` | process | dies with the Build | **lost** (in-memory; starts as not-yet-checked after restart) | `internal/app/guardrails.go` (`guardrailRouteHealth`) |
...
| 33 | Former independent live-listing outcome store and global refresh cooldown | consolidated into row 18's discovery owner by ADR 0360 | process | no independent resource remains | **reset-by-design** with row 18 | `internal/app/provider_discovery.go` |
...
| 82 | Selected harness-context execution-workspace borrows and principal binding generations | `app.Build` owns the harness resolver; each selected principal-scoped instruction or command source owns its exact read-only placement borrow | one source binding generation per session ID in the current Build; inherited children retain the parent's generation reference | source cleanup idempotently releases only its own exact reattachment. Retirement waits for active source borrowers; failed or cancelled construction closes attempt-owned borrows; Build shutdown rejects activation and retires every generation. Connection-owned acquisitions in row 83 protect backend lifetime across Clients and processes. The source view exposes no runner, mutation extension, or execution read ledger | **reset-by-design and reauthorized**: restart discards generations and borrows, resolves current operator policy, and acquires the session or scheduled fire's exact durable placement under current authority. No source identity is persisted, so List 2 gains no row | `internal/app/harness_context.go` (`harnessCommandResolver`, `BorrowWithExecutionWorkspace`, `Retire`); `internal/adapter/server/placement.go` (`executionWorkspaceAcquirer`); `internal/adapter/microvm/client.go` (`Reattach`) |
| 83 | Authenticated MicroVM acquisition socket/handler, ephemeral ID-to-exact-binding ownership, operation pins and pending teardown | returned client acquired binding plus daemon connection handler; one retained Unix socket per independent acquisition, one shared guest registration per exact ref | acquisition connection / daemon process; pins last for admitted operations | idempotent Close sends bounded detach and always closes the socket; EOF retires only that owner. Existing operation-socket cancellation/completion drains pins before final exact-ref unregister; no acquisition-owned cancellation. Daemon shutdown closes and joins retained handlers under existing bounded cleanup. Per-ref deletion admits only sole-owner/zero-pin or unowned/zero-pin state, consumes the caller on every terminal outcome (including `in_use`), and excludes acquisition until deletion or uncertain teardown is reconciled. Final unregister reuses registration-incarnation/pending-teardown protection; ordinary release never deletes durable worktrees | **reset-by-design and reauthorized**: IDs, sockets, pins and pending teardown are never persisted. A fresh harness obtains new ownership by exact authorized load/use. Daemon restart invalidates cached bindings and calls fail visibly; explicit teardown/load/reattach or harness restart clears dead ownership without replacement or replay. Durable session/schedule refs, VM records and worktree inventory remain unchanged; no new List 2 row | [MicroVM plan: private lifecycle v4](../acceptance/microvm-execution-environments.md#private-lifecycle-v4-exact-exchange-rules); `environment/microvm/daemon.go` (`ServeConn`), `repository_daemon.go`, `repository_attachment.go`, `repository_runtime.go`; `internal/adapter/microvm/client.go` |
| 84 | Experimental native Jev guardrail client and eight-slot limiter | `app.Build` owns one `jevguardrail.Driver`, shared by the main and session/worker reviewer factories | process, one concurrent-safe SDK client and limiter per Build | no goroutine or persisted state; each request has a bounded context, the semaphore releases on every return, and the client becomes unreachable after Build cleanup. The standard HTTP transport may retain idle connections until its transport or process closes them | **reconstructible/reset-by-design**: the next Build recreates the client and limiter from explicit operator `guardrails.backend: jev` and the host-supplied credential. No review verdict, candidate, or limiter state is persisted, so List 2 gains no row | `internal/adapter/jevguardrail/jevguardrail.go` (`Driver`, `New`, `Review`); `internal/app/build.go` (`Build`, `buildGuardrailsReviewer`) |
...
| 74 | Prompt-free exact-ask continuation contexts and detached relay join registry | `internal/adapter/server.Service` owns post-acceptance contexts, exact resumed `agent.Run`, recorder/relay goroutines, and `detachedControlWG`; an accepted plan allow also reserves a process-local `runState.planContinuation` before consuming the verdict. The unary caller owns only work before atomic acceptance | one process-local relay per accepted restored ask, plus one reserved fresh-run continuation for an accepted live or restored plan allow | relay and plan-continuation admission add to the Service wait group before a verdict is accepted and are fenced before shutdown wait. `Service.Close` first closes admission, marks every already-durable awaiting handoff `preserveDurable` and every cancellation `cancelSignaled` under the persistence barrier, then cancels contexts/runs and waits with the engine-close bound. The old plan run's terminal relay joins or releases its reservation; a preserved terminal relay cannot overwrite an awaiting snapshot | **derived from durable state / reset local ownership**: contexts, runs, recorders, marker bits, reservations, and join counts disappear on process loss. A replacement acquires the distributed session lease, reloads the authoritative snapshot after acquisition, and repeats exact run/state/ask/origin/placement validation before rehydrating. An accepted plan approval lost between terminal and fresh-run admission remains an uncertain outcome; it is never silently reported as execution complete. See List 2 row 47 | `internal/adapter/server/service.go` (`ResolveRunAsk`, `reserveDetachedControlRelay`, `relayDetachedControlRun`, `Close`, `GracefulDrain`, `cancelRegisteredRunState`), `internal/adapter/server/exact_plan_ask.go` (`ResolvePlanAsk`, `finishExactPlanRun`); ADR 0347, ADR 0366 |
...
what is persisted), **reset-by-design** (documented, acceptable, with explicit
cleanup behavior),
**fix-via-event-log** (Phase 3 makes it durable).

The approved MicroVM acquisition contract (List 1 row 83) adds no persistent entry here.
Acquisition IDs, sockets, operation pins and pending teardown disappear on restart;
exact authorized reattachment creates new ownership. Existing session/schedule refs,
repository VM records and durable worktree inventory remain the restart truth.

| # | Item | Where it lives | On restart today | Decision | Phase |
|---|---|---|---|---|---|
| discovery | Provider observations and their timestamps, latest outcome, attempt identity/deadline, waiter channels, occupied fetch slots, provider-local cooldown, and combined public projection | Build-local `providerDiscovery` (List 1 row 18) | lost; all providers begin unattempted, with no retained observations or cooldown | **reset-by-design**: metadata is rediscovered on eligible startup/demand; positive catalog/config floors remain derivable. No discovery state, effective model identity, or credential is added to session persistence | ADR 0360 |
| 47 | Contextual guardrail transient review detail | process-local `server.ReviewDetailRegistry`, bound to an exact root/child relation and review id | all concern/source/next-action prose disappears | **reset-by-design**: this is an owner-authorized live display channel, not durable audit; restart truthfully returns not found while machine-only review state remains in events | ADR 0363 |
...
| 50 | Contextual current-root trajectory | root-run `agent.reviewRoot.facts` plus completeness/byte accounting | the run and worker correlation ledger disappears | **reset-by-design**: its authority is deliberately one delegation-root run; no cross-session or restart inference is permitted | ADR 0363 |
| 51 | Contextual exact repeat grants | Build-owned `modelhook.WaiverHolder` map and random HMAC key | every digest and the key disappear | **reset-by-design, fail-safe**: a restarted process/session cannot match the old grant and requires fresh review/human approval; it is not permission learning or approval replay | ADR 0363 |
| 52 | Plan approval receipt | Build-owned `app.planApprovalReceipts`, exact ref/session/call/target mode | every unconsumed receipt disappears | **reset-by-design, fail-safe**: restart cannot reconstruct action authority; only a new real plan callback can mint another receipt | ADR 0363 |
...
| 22 | ToolHive protocol-provider observations and safe status | provider-local entries in `providerDiscovery` (List 1 row 18; List 2 discovery row), with no separate outcome store | observations, status, and cooldown reset; `probeToolhive` requests each registered protocol through the owner during Build | **reset-by-design**: bootstrap reacquires observations and registry default-selection evidence. Last-good metadata belongs only to this Build; no model list or effective selection is added to session persistence | ADRs 0064, 0334, 0360 |
...
| 47 | Prompt-free exact-ask handoff and acknowledgement-only resumed-run lifecycle (ADR 0347, ADR 0366; List 1 row 74) | the existing session snapshot persists `StateAwaiting`, exact `RunID`, `PendingAsk` with origin, mode, and `EnvironmentRef`; the accepting Service owns only reconstructed runs/contexts/relays and the process-local plan-continuation reservation described in List 1 | a restart or replica handoff loses local execution and join bookkeeping but retains the last authoritative awaiting or terminal snapshot. An accepting replica acquires the distributed lease, reloads after acquisition, and revalidates exact run, awaiting state, ask ID, required origin, and placement before `ResumeApproval`; a stale pre-lease snapshot cannot replay a verdict. A plan approval accepted before process loss but not yet followed by its fresh proceed run remains uncertain, never proof of execution | **persist existing snapshot, reconstruct under fresh lease**: no new durable field is introduced. Before shutdown cancellation reaches a detached continuation, an already-persisted awaiting point is marked for preservation and the relay is joined boundedly; otherwise terminal cancellation is persisted. Plan verdicts use the dedicated exact control, and an allow starts a fresh run only after `StopPlanApproved` | 0347, 0366 |
| 48 | Mecatui rejected admission submission (ADR 0360; List 1 row 76) | one private `admissionSubmission` on `ui.Model`, keyed to session ID and stream generation | a client restart loses the prepared text/media, editable staging, counters, and recovery controls; the adopted server transcript remains unchanged because admission rejected before execution | **reset-by-design**: retain only in the current client until SessionInit, explicit recovery/disposal, session replacement/exit, or unrelated terminal error. Retry is explicit and byte-identical; Back transfers staging with confirmation before replacing a newer draft. No snapshot, event, or transport schema changes | 0360 |
| 49 | Harness-context policy, source snapshots, and binding generations (ADR 0359; List 1 row 77) | Build-local registrations and process snapshots; owner/profile-bound session generations with reference counts and retirement state | restart loses the selected source objects and lifecycle bookkeeping, not session history or execution placement; existing sessions and schedules resolve current configured sources using their authoritative owner/profile and current source authorization | **reset-by-design**: current deployment composition is authoritative on rebind. No `HarnessContextRef`, source path, policy snapshot, migration, or legacy-session rejection is persisted. Configuration changes can change resumed context | 0359 |
