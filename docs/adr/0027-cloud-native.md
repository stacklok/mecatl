...
and jsonlstore family locks (row 8). List 2 gains row 35: durable progress is
...
| 7 | Redis snapshots + event/tool sidecars + derivative metadata indexes | redisstore | managed Redis keyspace; one global and owner-specific sorted metadata index plus generation fields | Save atomically replaces the snapshot and its bounded metadata row; Delete atomically removes snapshot, metadata membership, and event/tool sidecars. Redis owns persistence/eviction policy and connection cleanup remains `Store.Close` | persisted. The index is derivative from sessnap but intentionally never rebuilt by an inventory page: a store opened with legacy snapshots and no complete index reports paging unsupported until maintenance/migration establishes one | `internal/adapter/redisstore/redisstore.go` (`Save`, `Delete`); `internal/adapter/redisstore/metadata_index.go` (`pageSessionMetadata`) |
...
| 16 | Child-session retention GC goroutine + bounded storage-maintenance status (`storageMaintenanceState`) | `startChildGC` / `app.Build` | process | goroutine exits on ctx done or sticky `ErrPruneUnsupported`; the mutex-protected status dies with Build | reconstructible/reset-by-design (the sweep restarts with the process; last/next sweep and active-job status reset until the startup sweep records them; the swept artifact is the store) | `internal/app/childgc.go` (`startChildGC`); `internal/app/storage_health.go` (`storageMaintenanceState`) |
...
The pure planner adds no resource. Manual confirmation adds List 1 row 56: a process-random
