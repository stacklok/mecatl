...
| 70 | Durable learning-attempt repository (ADR 0294) | `app.Build` constructs one lazy `attemptstore.Store` beside the user-model store only when Review/Auto learning can apply at some admitted root, and shares it with main, per-session, explicit reflection admission, and recovery; Off/unwired constructs none | process handle over caller-partitioned durable attempt metadata | no goroutine or retained file descriptor; the stable flock is acquired per operation and bounded JSON commits use temp-file fsync, rename, and directory fsync. Empty startup creates no directory. Admission verifies the exact source session's persisted non-empty ADR-0249 RunID; explicit admission creates directly, while automatic admission first acquires List 1's deterministic automatic reservation and then idempotently creates before returning `queued` | **persisted**: queued/running/terminal state, immutable source/prompt/digest provenance, opaque CAS versions, claims, checkpoints, and safe links reload across processes. See List 2 row 43 | `internal/adapter/attemptstore/store.go` (`Store`, `Create`, `Get`, `Pending`); `internal/app/reflection_observer.go` (`createDurableAttempt`, `submit`); `internal/app/build.go` |
| 71 | Durable learning-attempt recovery worker (ADR 0294) | `app.Build` starts one `attemptRecovery` only when Review/Auto is wired and the durable attempt repository contains nonterminal work | process / one bounded sequential worker over the startup snapshot | the worker runs under its own child of the Build lifecycle context. It reconstructs exact source evidence through `learningEvidenceLoader`, crosses the model boundary only through the fenced canonical projection, and advances claim-fenced proposal/skill checkpoints. `Built.Close` cancels and joins it before closing borrowed composition resources. Off, unwired, and an empty repository allocate no recovery worker | **derived from persisted authority**: every replacement process enumerates List 2 row 43's nonterminal attempts and resumes legal idempotent boundaries; process-local iteration position is disposable. A live unexpired claim remains fenced rather than being stolen | `internal/app/attempt_recovery.go` (`attemptRecovery`, `startAttemptRecovery`, `recoverAttempt`); `internal/app/source_evidence.go` (`learningEvidenceLoader`); `internal/app/build.go` |
...
| 43 | Durable learning-attempt lifecycle and admission provenance (ADR 0294; List 1 rows 70–71) | caller-partitioned `AttemptRepository` records keyed by deterministic caller/session/RunID/canonical-digest IDs | restart or replica replacement reloads the same queued, running, or terminal attempt plus immutable current-prompt/source binding, opaque version, claim generation/expiry, checkpoints, safe failure code, and authorized proposal/skill links. The Build-owned List 1 row 71 worker enumerates nonterminal records, reconstructs exact bounded evidence, and resumes claim-fenced work; duplicate admission converges to that record | **persist-in-store and re-drive**: admission first verifies the exact non-empty RunID from the authoritative SessionStore, then idempotently creates the attempt before reporting `queued`. Skipped/non-admitted completions and Off/unwired composition write no record; process-local coordinator and receipt state are not authority | 0294 |
