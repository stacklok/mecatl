...
| 83 | Native-execution active run and operation renewal handles | `executionclient.Provider` / `executioncontroller.Store` | run / operation | context cancellation joins renewal, while terminal receipts or explicit release clear only the matching durable holder; uncertain expiry is retained and fenced | persisted in `ExecutionEnvironment.status.activeRun` and `status.activeOperation`; restart observes rather than adopts an unknown holder | `internal/adapter/executionclient/client.go` (`Provider`); `internal/adapter/executioncontroller/store.go` (`Store`) |
...
| 89 | Mecatui pending-approval recovery watch and retained cursor | `ui.Model` for one exact `--resume` selection | client process / one exact session and run | the UI closes the caller-owned watch on terminal result, watch failure, stale or refused control, pre-verdict escape or quit, and process teardown; an accepted verdict keeps the watch until the exact run terminates, while Ctrl+C uses `CancelRun` before cleanup | **reattached by the client under fresh authorization**: startup replays the durable event log from the beginning, requires one gap-free live boundary and one trailing unresolved ordinary ask, then retains the returned cursor. No client state or verdict is persisted or replayed | `cmd/mecatui/ui/pending_approval.go` (`pendingApprovalRecovery`, `closePendingApprovalWatch`); `cmd/mecatui/client/pendingapproval.go` (`DiscoverPendingApproval`, `WatchPendingApprovalRun`) |
