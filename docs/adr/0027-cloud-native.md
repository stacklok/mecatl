...
| 12 | Per-session environment overrides (`Service.sessionEnvironments`: nofs, ACP buffers) | `server.Service` | session | evicted at `CloseSession` (`service.go:1694`) | lost; the no-fs override is re-registered by rehydration; ACP override is re-registered on editor reconnect (phase-2 decision: in-process only, restart loses it — the ACP editor is gone anyway) | `internal/adapter/server/service.go:722` |
