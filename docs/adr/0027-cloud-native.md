...
| 51 | Optional per-server MCP OAuth controller: one official handler, persistent token source/CAS version, authorization flight, and dedicated hardened HTTP client's idle pool; plus its durable credential record (issue #521, ADR 0110) | `internal/adapter/mcp.Server` | connected server / one credential identity | `Server.Close` closes the MCP session first, then idempotently closes the controller's HTTP idle pool; the injected credential store is borrowed and remains injector-owned. No background refresh, listener, browser, or timer goroutine | **persisted / explicitly reattached**: a new controller derives the same opaque key from injected identity/config, reloads the store record, and reattaches it through the official handler's initial-token-source hook. In-memory flight/idle state resets. No List 2 row: credentials are adapter-owned state, not session/run state | `internal/adapter/mcp/oauth.go` (`OAuthController`, `NewOAuthController`, `Close`); `internal/adapter/mcp/oauth_tokensource.go`; `internal/adapter/mcp/mcp.go` (`Close`) |
...
**MCP OAuth controller re-audit (List 1 / List 2 — issue #521, ADR 0110).** The
optional controller adds ONE outlives-a-call resource (List 1 row 51), owned by one MCP
`Server` and absent unless an embedding supplies `ServerConfig.OAuth`. Its durable record
is adapter credential state and therefore adds NO List 2 session/run row. Reattachment
requires the same explicit subject, resource, issuer, client registration, store, and
external store key; no composition or implicit key acquisition is added.
