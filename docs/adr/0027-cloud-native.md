...
| 76 | Harness-context process snapshots and per-session source-binding generations | `internal/app.Built` owns process-scoped sources; `internal/app.harnessCommandResolver` owns session bindings for all five kinds and their in-flight references | process plus canonical session ID and owner/profile | `Retire` rejects new borrows of that generation and defers cleanup until its final borrower releases. Explicit owner-authorized reload can activate a fresh generation; old releases cannot evict it. Session teardown and unpublished-create failure retire bindings. `Built.Close` rejects activation and defers process cleanup until outstanding references drain. Failed binding creation is retryable, and cleanup is idempotent | **reset-by-design**: sessions and schedules rebind current operator policy under current source authorization after restart, without a durable context identity, migration, or legacy-state rejection. See List 2 row 48 | `internal/app/harness_context.go` (`harnessCommandResolver`); `internal/app/harness_context_binding.go` (`cacheProcessHarness`, `bindSessionSources`); `internal/app/harness_context_generation.go` (`harnessGeneration`); `internal/adapter/server/placement_discovery.go` (`ListCommandsForSession`) |

**Harness-context re-audit (List 1 / List 2 — ADR 0354).** List 1 row 76 inventories process snapshots, session generations, and in-flight accounting. List 2 row 48 explicitly records their reset-by-design treatment; no durable harness-context reference is added.
