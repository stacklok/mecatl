...
| 76 | Standalone remote-broker admission gate, active-operation coordinator, public/admin listeners, and drain completion channel | `cmd/mecabroker` (`mcpbrokerserver.Production`) | broker process | preStop and SIGTERM share the once-guarded drain: admission closes before propagation wait, active work drains to its configured deadline, listeners stop, then verifier, ToolHive, and transport close; `Close` joins all owned sweepers | **reset-by-design**: a replacement starts a fresh singleton broker. It never transfers callback state, attachment handles, or active work; remote clients detect the new incarnation and fail protected continuations closed | `cmd/mecabroker/main.go` (`run`); `internal/adapter/mcpbrokerserver` (`Production`) |
| 77 | Remote broker attachment, logical-session, lifecycle/Execute receipt, pending-control, and active-Execute registries plus retention sweepers | `internal/adapter/mcpbrokergrpc.Server` | broker process / bounded by configured capacities and absolute deadlines | admission rejects new entries at capacity; `MaxActiveExecutes` reserves a process-wide slot before any goroutine or upstream dispatch, retains a bounded immutable capacity receipt on rejection, and releases the slot on every terminal/cancel/panic path; periodic sweeps reclaim expired unattached/idle state; server close cancels and joins active executions and stops sweepers before owned ToolHive resources close | **reset-by-design**: registries and active accounting deliberately do not survive broker replacement. A response lost after possible Execute remains ambiguous and is never replayed | `internal/adapter/mcpbrokergrpc/broker.go` (`Server`, `Config`, `Shutdown`) |
| 78 | mecak8s remote broker TLS client factory and projected audience-token reader | `cmd/mecak8s` composition | agent process / one factory per configured remote broker | each RPC rereads the bounded projected broker token; connection close is owned by the attachment/factory lifecycle; the Kubernetes API service-account token remains independently mounted for `SessionLease` | **reconstructible**: restart rebuilds the factory from the same CA and projected credentials. The remote broker is the authority; mecak8s persists no callback or ToolHive state | `internal/adapter/mcpbrokergrpc/factory.go` (`NewRemoteFactory`, `ProjectedTokenCredentials`); `deploy/helm/mecak8s/templates/deployment.yaml` |
| 79 | Protected ToolHive Redis client | `mcpbroker.Process` via composition-supplied `ProtectedStorageConfig.Redis.Client` | process | protected storage close is idempotent; the encrypted decorator closes the native store/client exactly once during rollback, process close, or auth-server shutdown | reconstructible from operator Redis configuration and mounted credentials/CA at next broker start; no client handle is persisted | `internal/adapter/mcpbroker/toolhive_protected_storage.go` (`protectedToolHiveStorage`); `cmd/mecabroker/config.go` (`toolHive`) |
| 80 | Protected encrypted ToolHive storage decorator and immutable KEK ring | `mcpbroker.Process` | process over managed Redis/key Secret | decorator close delegates once to native storage; key-ring references die after process shutdown; startup failure closes acquired client | reconstructible from Redis configuration and operator-provided KEK Secret at next start; encrypted records remain in Redis | `internal/adapter/mcpbroker/toolhive_encrypted_storage.go` (`encryptedAuthStorage`); `internal/adapter/mcpbroker/toolhive_protected_storage.go` |
| 81 | Credential custody core and native-expiry admission state | `mcpbroker.Process` | deployment-wide encrypted custody records plus process-local custody reader | custody borrows the protected client; process rollback/close tears down the owning storage; no separate custody close | custody records are persisted encrypted in Redis (see List 2 row 47); process-local reader/refresher state is rebuilt at next start | `internal/adapter/mcpbroker/toolhive_credential_custody.go` (`credentialCustody`) |
| 82 | Bounded Stage/Recover continuity receipt registry | `mcpbrokergrpc.Server` | broker process | the server sweeper removes every receipt at its absolute attempt deadline (or Stage custody expiry when earlier); `Shutdown` rejects waiters, clears the registry, and joins the sweeper. Entries are keyed by one-way authenticated workload partition plus operation and request ID, retain only a semantic digest, bounded response, and terminal status, and never enter diagnostics | **reset-by-design**: receipts are retry aids only. A replacement broker starts empty and must not replay B1 custody Stage or recovered provisional authority. See List 2 row 48 | `internal/adapter/mcpbrokergrpc/continuity_receipt.go` (`reserveContinuityReceipt`, `finishContinuityReceipt`); `internal/adapter/mcpbrokergrpc/server_registry.go` (`sweep`) |
| 83 | Broker recovery-attempt correlation map and deferred replaced-client closers (`Service.brokerRecovery`, `brokerRetiredCloses`) | `server.Service` | process; bounded per live-session capacity | exact custody/incarnation/binding-keyed attempts are removed after Commit, invalidation, session deletion, expiry, or Service close; replaced client closers run only after no cached attachment has that generation | reset-by-design: a host crash loses request correlation, leaving an unpublished B2 provisional attempt to expire at its fixed two-minute deadline; a restart may only exact-reattach a durably saved recovered binding and Commit before engine construction | `internal/adapter/server/workspace_enrollment.go` (`recoverBrokerAttachment`, `invalidateBrokerCustody`); `internal/adapter/server/mcp_broker.go` (`recoveryAttemptKey`, `releaseRetiredBrokerClient`) |

**Remote-broker ownership re-audit (ADRs 0313–0315).** The standalone broker, not `mecak8s`, owns all browser callback routes and singleton process-local authority. Rows 76–77 capture the broker's bounded lifecycle and registry resources; row 78 records the separate agent-side client credential lifecycle. List 2 row 45 records the restart boundary: remote client transport reattaches only to a live matching incarnation, while broker owners, receipts, and authorization state reset fail closed.
...
| 48 | Protected ToolHive credential custody records (encrypted staged/current/tombstoned metadata; List 1 rows 79–81) | encrypted Redis records in the protected ToolHive namespace, keyed by an opaque random recovery reference; the protected client and KEK ring are process-owned by `mcpbroker.Process` | records survive broker replacement; nothing resolves them yet, and later reads never extend the stored expiry | **persist encrypted, bounded by native ToolHive session expiry**: custody is subordinate evidence, never authority on its own; any later resolution must validate the full session/owner/workload/profile guard through an authorized host path | broker credential continuity |
| 49 | Stage/Recover continuity receipt registry | `mcpbrokergrpc.Server`'s process-local receipt map | a broker replacement loses every in-flight and completed receipt; no raw guard, handle, binding, recovery reference, or credential is persisted | **reset-by-design**: an identical retry joins or replays only while the same broker process owns the bounded receipt. After process loss the host must use the separate fresh-B2 recovery choreography; this registry never grants cross-process authority or execution replay | Planner 08 bounded receipt ownership |
| 50 | Host broker recovery request correlation and candidate-adoption flight | `Service.brokerRecovery` and replacement generation state | process-local | a crash drops the request ID/deadline and any unpersisted candidate; the broker's provisional receipt is not authority and naturally expires within two minutes | reset-by-design: unchanged durable custody can authorize a later fresh recovery; only a durable exact recovered binding may reattach and Commit before engine construction | Part 09 continuity |
