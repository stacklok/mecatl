...
- **Adapters** (the EventLog-seam template): the in-memory reference
  ...
  single-host flock lease (`internal/adapter/flocklease`, a retained per-id flock
  handle + an atomic record file; the kernel hold denies live contenders beyond
  JSON expiry and SIGKILL releases it for immediate crash takeover), the gRPC driver
...
and, for the local adapter, the retained per-session flock fd. Ownership is split
between the Service lifecycle and `flocklease.Lease`; cleanup = renewer-cancel +
exact-token `Release`, which tombstones and closes the fd. Re-attach = reacquired
on the next run-entry; a crashed process has all retained fds released by the OS,
so a survivor takes over immediately without waiting for JSON expiry. List 2 is
...
| 27 | held session leases + per-session renewer goroutines (`Service.heldLeases`, Phase 4) + local retained flock fds | `server.Service` owns the lifecycle; `flocklease.Lease` owns each local fd | session (one lease + renewer per leased session; one retained fd per locally held lease) | renewer cancelled + `SessionLease.Release` (cancel-detached short-timeout ctx) on `CloseSession` and shutdown `Close`; local exact-token Release durably tombstones then closes/unlocks its fd, stale release cannot touch a successor, and failure paths close newly opened handles. A lost lease cancels the run and drops the hold; SIGKILL makes the kernel close every retained fd | **reconstructible** (a restart re-acquires on the next run-entry; a crashed local holder's kernel flock releases immediately, so a survivor takes over without waiting for JSON expiry, then increments the token preserved in the durable record — lease state remains derived). Constructed when a lease backend is explicitly wired or store-provided, and automatically as the existing flock lease beneath every local JSONL StoreDir. Other no-lease shareable stores do not gain destructive-maintenance authority; automatic retention fails closed | `internal/adapter/server/service.go` (`heldLeases`, `acquireLease`, `renewLoop`, `releaseLease`); `internal/adapter/flocklease/flocklease.go` (`Lease`, `heldLease`, `Acquire`, `Release`); wired at `internal/app/build.go` (`buildSessionLease`) |
