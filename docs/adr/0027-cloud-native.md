...
| 7 | Redis snapshots + event/tool sidecars + derivative metadata indexes | redisstore | managed Redis keyspace; one global and owner-specific sorted metadata index plus generation fields | Save atomically replaces the snapshot and its bounded metadata row; Delete atomically removes snapshot, metadata membership, and event/tool sidecars. An explicit authenticated migration job CAS-installs legacy rows in bounded batches and atomically publishes ready only after stable-generation complete-coverage verification. Redis owns persistence/eviction policy and connection cleanup remains `Store.Close` | persisted. The index is derivative from sessnap but never rebuilt by an inventory page: a store opened with legacy snapshots reports paging unsupported until the resumable maintenance job completes; durable caller-bound progress and generation checks survive process replacement | `internal/adapter/redisstore/redisstore.go` (`Save`, `Delete`); `internal/adapter/redisstore/metadata_index.go` (`pageSessionMetadata`); `internal/adapter/redisstore/migration.go` (`FinalizeSessionMigration`) |
...
| 56 | Durable session-migration job registry (issue #589, ADR 0226) | `jsonlstore.Store`; authenticated `server.Service` orchestrates caller-driven batches | store-wide adapter-private `sid-v1/migration-jobs/`, one bounded record per explicit job; no process map or worker goroutine | records are owner-only atomic replacements checkpointed after each family; every mutating load-to-checkpoint drive holds the job's stable cross-process flock, and a stale overlapping resume receives conflict. Completed records remain inspectable. Per-family work borrows existing run-entry/lease/family locks and closes files per operation | **persisted**: restart reloads caller-bound counters, terminal item handles, cancellation, and bounded stable errors. The registry deliberately excludes raw session ids, principals, paths, backend errors, and content. See List 2 row 35 | `internal/adapter/store/jsonlstore/migration.go` (`SaveSessionMigrationJob`, `LoadSessionMigrationJob`); `internal/adapter/server/migration.go` (`driveSessionMigration`) |
