...
| 70 | Durable learning-attempt repository (ADR 0294) | `app.Build` constructs one lazy `attemptstore.Store` beside the user-model store and shares it with main, per-session, and explicit reflection admission | process handle over caller-partitioned durable attempt metadata | no goroutine or retained file descriptor; the stable flock is acquired per operation and bounded JSON commits use temp-file fsync, rename, and directory fsync. Empty startup creates no directory. Admission verifies the exact source session's persisted non-empty ADR-0249 RunID, then idempotently creates the deterministic attempt before returning `queued` | **persisted**: queued/running/terminal state, immutable source/prompt/digest provenance, opaque CAS versions, claims, checkpoints, and safe links reload across processes. See List 2 row 43 | `internal/adapter/attemptstore/store.go` (`Store`, `Create`, `Get`); `internal/app/reflection_observer.go` (`createDurableAttempt`, `submit`); `internal/app/build.go` |
...
| 43 | Durable learning attempts (ADR 0294; List 1 row 70) | caller-partitioned `learning.AttemptRecord` values in the injected `AttemptRepository`, including queued/running/terminal state, exact session/run/prompt/digest provenance, fenced claims, checkpoints, and safe artifact links | restart reloads the exact workflow state and immutable provenance; deterministic IDs make duplicate admission converge, while claims and CAS versions fence stale workers | **persist-in-store**: accepted work is created durably before `queued` is returned; process-local scheduling may reset, but repository state remains authoritative and restart workers reconcile it | 0294 |
