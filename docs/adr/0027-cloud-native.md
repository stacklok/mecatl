...
| 18 | Provider discovery owner: frozen borrowed listers, owner context/cancel, per-provider attempt map with deadline/ready channel/returned and terminal flags, cooldown timestamps, atomic immutable observations/outcomes/model-status projection and captured default facts, completion-tail/publication locks, worker join group, and one-shot startup timer/worker (ADR 0357) | `app.Build` | process; at most one occupied fetch slot per available lister provider, with one latest immutable snapshot | `providerDiscovery.Close` prohibits new attempts/publications, cancels and joins fetch/startup workers and their deadline callbacks before borrowed credentials close, on Build failure and normal Close. `fetch` stops or joins its deadline callback and marks return; timeout publishes before return but keeps the slot occupied. Physical Close requires cooperative lister cancellation. Per-request fan-out waiters join within their caller bound; cancellation ends only their wait. No-lister/native-only startup starts no workers. Replaced snapshots become reclaimable when readers release them | **reset-by-design**: the next Build borrows freshly registered listers and starts without observations, attempts, signals, or cooldowns. Catalog/config floors are derived; native authenticated providers remain demand-only. No cache reattachment or durable discovery state | `internal/app/provider_discovery.go` (`providerDiscovery`, `discoveryAttempt`, `request`, `fetch`, `Close`); `internal/app/build.go` (`Build`) |
...
| 33 | Former independent live-listing outcome store and global refresh cooldown | consolidated into row 18's discovery owner by ADR 0357 | process | no independent resource remains | **reset-by-design** with row 18 | `internal/app/provider_discovery.go` |
