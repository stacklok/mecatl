...
List 2 row 40 records the durable debug kind/target relationship and fail-closed factory
rehydration. No target run, lease, subscription, or live-follow resource is created.
...
| 40 | Debug-session kind and exact target lineage (ADR 0248) | `session.Session.Kind` plus `Session.Relationship.DebugTargetID`, round-tripped by sessnap and every SessionStore/driver | restart reloads the debugger's own conversation and exact target binding. `Service.rehydrateSession` rebuilds only through `DebugSessionEngine`; invalid no-fs/empty-workspace metadata, missing factory, or unavailable target fails closed. The target itself is never loaded through run entry, resumed, or leased | **persist-in-snapshot and derive engine**: kind/lineage/profile are durable; the exact `{InspectSession}` engine and no-fs environment are reconstructed from them. No in-memory target binding is trusted across restart | 0248 |
