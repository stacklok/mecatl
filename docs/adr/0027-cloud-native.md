...
| 47 | Evidence-reflection worker pool, fair per-principal queues, singleflight map, and bounded receipt cache (issue #509, ADR 0109) | `app.Build` constructs exactly one `reflectionCoordinator` in every mode; construction is dormant and the first admitted automatic or explicit job starts the bounded worker set | process (global worker count; bounded per-principal FIFO queues with fair principal rotation; in Off the coordinator object exists but starts no worker until explicit reflection) | every job has a timeout and per-job byte cap; oversized raw trajectory material is rejected before projection/marshal or queue allocation, and retained jobs share an aggregate queued-byte cap. The effective receipt bound is raised to at least queue capacity plus workers, then a live receipt is reserved before admission; receipt pressure rejects rather than accepting an unreportable job. The coordinator owns a lifecycle context. `Built.Close` stops admission, publishes bounded closed receipts for queued waiters, clears queued/pending state, cancels active jobs, and joins every started worker before returning. Completed receipts evict oldest-first, while principal+session+trajectory digest singleflight collapses duplicates | **reset-by-design**: queued/running jobs and bounded receipts are transient coordination state. Durable staged proposals and promoted memory remain authoritative; restart does not retrospectively sweep old completed sessions. See List 2 row 30 | `internal/app/reflection_coordinator.go` (`reflectionCoordinator`, `newReflectionCoordinator`, `Close`); `internal/app/reflection_observer.go` (`reflectionObserver`) |
| 48 | Durable agent-owned skill manifest, immutable version files, and stable flock (issue #510, ADR 0111) | `app.Build` constructs one lazy `skillstore.Store` beside the user-model store and shares it with reflection, server lifecycle APIs, and every catalog assembly | process handle over principal/project-partitioned durable data | the stable `skills.lock` flock is acquired and released per operation; files are opened no-follow and regular-file status is checked from the opened descriptor. `manifest.json` and each content-addressed `versions/<version>/SKILL.md` use temp-file fsync, rename, and directory fsync. Empty startup is lazy. A crash before manifest commit can leave only an unreferenced immutable version, which reopen safely ignores | **persisted**: bounded lifecycle metadata, provenance, validation disposition, evaluations, receipts, active selection, and immutable bodies reload together. Exact convergence preserves the stricter similarity disposition. Cross-store proposal linkage is create-then-CAS-link and reconciles by ProposalID/SkillID without a duplicate. See List 2 row 31 | `internal/adapter/skillstore/store.go` (`Store`, `locked`, `save`, `ensureVersion`); `internal/adapter/skillstore/materialize.go` (`MaterializeProposal`) |
| 49 | Live learned-skill immutable generation pointer and publication serializer (issue #510, ADR 0111) | `app.Build` constructs one `skillfs.AtomicCatalog` plus one `learnedSkillPublication`; `catalogAssets` shares them with every `assembleCatalog` call and the server's live `ListSkills` lister | process (one mutex and atomic pointer to a bounded immutable path-free external-source/body-only-learned generation; no goroutine, watcher, path, asset materialization, or workspace root) | publication holds the mutex while reading durable active state and CAS-swapping one complete generation. Each execution retains one snapshot. Post-commit publication is bounded and cancel-detached; failure revokes the affected learned name fail-safe, and startup plus bounded live-list refresh reconcile from the repository. Build teardown needs no explicit close; old generations become garbage after in-flight readers release them | **derived** from List 2 row 31's active durable versions plus the build-time immutable external skill snapshot. The mutex/generation reset on restart; reconstruction occurs before serving, and a crash after manifest activation but before publication self-heals on restart. No separate persisted state | `engine/adapter/skillfs/atomic.go` (`AtomicCatalog`, `Refresh`, `LiveTool`); `internal/app/learned_skills.go` (`learnedSkillPublication`, `learnedSkillPublisher`); `internal/app/catalog.go` (`registerSkillFamily`) |
