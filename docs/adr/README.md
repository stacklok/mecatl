...
- [0337 - Deno uses the TypeScript SDK HTTP/SSE entry point](./0337-typescript-sdk-deno.md) *(supersedes ADR 0279 only for the supported-runtime set)*
- [0338 - Deno owns local daemons through Deno.Command](./0338-typescript-sdk-deno-command.md) *(supersedes ADR 0337 for the Deno public entry-point and local-process decisions)*
- [0339 - Deno reuses the ConnectRPC gRPC transport](./0339-typescript-sdk-deno-grpc.md) *(supersedes ADRs 0337 and 0338 for the HTTP-only transport and Node-compatibility exclusions)*
