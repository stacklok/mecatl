...
A steer carries the same `session.Content` media parts as an ordinary prompt. `Steer`
and `SteerEcho` add repeated `Content` fields without renumbering existing fields.
A new additive `ServerCapabilities.multimodal_steer` bit is advertised from the
same engine enablement as `steer`. New mecatui requires both bits for native steer;
a steer-capable older server therefore keeps all mid-run input in the local merge
queue instead of silently discarding unknown media field 3.
...
`StartRunContent`. Existing text-only engine, Service, and client methods remain as
wrappers around additive multimodal siblings.
...
Text-only clients remain wire- and API-compatible. Media-only steers are legal.
Pending media remains run-scoped and restart-lost by the same deliberate policy as
