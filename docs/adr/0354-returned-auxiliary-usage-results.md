...
4. A current Service owner synchronously merges returned results only through an already-valid run/session capability. Guardrail hooks use `port.AuxiliaryUsageReporter`, a request-scoped synchronous callback installed only by the owning Engine; `modelhook.CheckResult` carries verdict plus usage and the runner forwards usage only through that callback. A detached queued/recovery or post-lease-loss report is dropped with bounded diagnostics; it is never made durable by reacquiring a lease, loading/reloading, replaying, or persisting a source session solely for accounting, and never mutates a stale pointer.
