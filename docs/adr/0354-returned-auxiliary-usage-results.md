...
4. A current Service owner synchronously merges returned results only through an already-valid run/session capability. A detached queued/recovery or post-lease-loss report is dropped with bounded diagnostics; it is never made durable by reacquiring a lease, loading/reloading, replaying, or persisting a source session solely for accounting, and never mutates a stale pointer.
