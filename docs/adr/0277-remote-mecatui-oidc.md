...
- Superseded by: ADR 0274 (logout provider budget only), ADR 0275 (credential-recovery classification only), [ADR 0284](./0284-optional-system-trust-remote-oidc.md) (mandatory issuer-CA requirement only), [ADR 0286](./0286-issuer-transport-split.md) (private-only issuer transport clauses)
