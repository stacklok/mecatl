...
Atrium works around this by synthesizing its OWN `resumeAskID` and parsing the askID
grammar to extract the call id (stacklok/atrium#552) — fragile coupling to an internal
format.
...
2. **`agent.RunOptions.AskIDDiscriminator` (`string`)** — when non-empty and colon-free,
   ...
   when valid, else `fmt.Sprintf("r%d", serial)`). A durable host (e.g. Atrium) passes
   its own RunID. (#117)
...
- A durable host can reconstruct an askID across processes from persisted state alone,
  unblocking stacklok/atrium#581 — Atrium passes its durable RunID as the discriminator
...
- The CWE-863 guard is now SHARED responsibility when a discriminator is supplied: the
  ...
  explicitly. The LOAD-BEARING obligation an adopting host (e.g. Atrium for
  stacklok/atrium#581) must honour is that the discriminator is unique per run-ATTEMPT,
  ...
  (including a resume after Interrupt) MUST carry a fresh discriminator. Atrium's durable
