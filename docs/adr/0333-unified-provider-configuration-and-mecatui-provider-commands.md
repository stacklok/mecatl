...
Replace `llm.endpoints` with an OIDC authentication form under `providers.<provider-name>`. Use `credential_store.api_key.file` for the configurable file-backed API-key input (optional; default `$XDG_CONFIG_HOME/mecatl/auth.yaml`) and `credential_store.oidc` for the existing deployment-scoped shared protected OIDC credential home/key scope (optional; default the current XDG state/keyring behavior). The API-key file temporarily also carries the experimental OpenAI Codex manual-token record; this exception is tracked separately and does not change the API-key-oriented command/flag vocabulary. The per-provider OIDC shape is `auth.method: oidc` plus `auth.oidc` carrying issuer, client ID, scopes, optional resource audience, and independent issuer/gateway trust. Strict parsing, bounded credential custody, and identity binding remain unchanged. Retain `provider_overrides:` unchanged to keep this work bounded. Future OIDC discovery/DCR may populate this same provider configuration from a URL, but is not implemented or implied here.
