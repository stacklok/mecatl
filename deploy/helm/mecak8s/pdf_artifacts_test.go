...
func TestPDFArtifactStorage_HelmValues(t *testing.T) {
	...
	if err != nil { t.Fatal(err, baseline) }
	if strings.Contains(baseline, "--artifact-s3-") { t.Fatal("default render enables PDF artifact storage") }
	...
	if err != nil { t.Fatal(err, configured) }
	for _, flag := range []string{"--artifact-s3-bucket=private-pdf", "--artifact-s3-region=eu-west-1", "--artifact-s3-endpoint=https://minio.example:9000"} {
		if !strings.Contains(configured, flag) { t.Fatalf("configured render missing %s", flag) }
	}
	for _, values := range [][]string{
		...
		for _, value := range values { args = append(args, "--set", value) }
		if output, err := helm(t, args...); err == nil { t.Fatalf("invalid artifact values rendered: %q", output) }
	}
}
