...
func marshalMethodParams(t *testing.T, p *Provider, req port.LLMRequest) string {
	t.Helper()
...
func TestCacheDialectNoneEmitsNoHints(t *testing.T) {
	...
	raw := marshalMethodParams(t, p, cacheReq("gpt-4o"))
...
func TestCacheDialectOpenAISendsPromptCacheKeyOnly(t *testing.T) {
	...
	// openaichat scopes NO retention gating and has no OpenRouter dialect at
	// all (Chat Completions has no request-root cache_control extension) —
	// neither must ever appear.
	if strings.Contains(raw, "prompt_cache_retention") {
		t.Errorf("openaichat must never send prompt_cache_retention (out of scope for this adapter): %s", raw)
	}
	if strings.Contains(raw, "cache_control") {
		t.Errorf("openaichat has no OpenRouter dialect; cache_control must never appear: %s", raw)
	}
}
