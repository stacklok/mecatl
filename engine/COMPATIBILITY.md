...
| identity label: `ExternalBinding` (opaque host runtime binding) | **not event-carried** — safe to lose on a pure fold unless the host requires exact external-runtime reattachment, in which case it MUST supply the binding out-of-band | the snapshot (`sessnap`, restored by direct assignment); the event annotation is log-only and the fold neither requires nor re-derives it, so a fold-rebuilt session keeps whatever the snapshot restored (**never** fabricated or backfilled) |
