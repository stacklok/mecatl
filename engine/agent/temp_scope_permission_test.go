...
func scopedBashCall(id, command, scope string) session.ToolCall {
	return session.NewToolCall(session.ToolCallID(id), tool.BashToolName, []byte(`{"command":`+quoteJSON(command)+`,"temp_scope":`+quoteJSON(scope)+`}`))
}
...
func runScopedBash(t *testing.T, rules []governance.Rule, call session.ToolCall, runner *fakeBashRunner) ([]session.Event, session.ToolResult) {
	t.Helper()
...
// TestADR_0281_BashManagedScopeDefaultAndValidation pins managed as the enabled
// default and ensures an unrecognised scope never reaches a command runner.
func TestADR_0281_BashManagedScopeDefaultAndValidation(t *testing.T) {
	t.Parallel()
	rules := []governance.Rule{{Tool: tool.BashToolName, Effect: governance.Allow}}
	for _, tc := range []struct{ name, args string }{
		...
		t.Run(tc.name, func(t *testing.T) {
			runner := &fakeBashRunner{res: tool.CommandResult{Stdout: "ok"}}
			call := session.NewToolCall("c", tool.BashToolName, []byte(tc.args))
			_, result := runScopedBash(t, rules, call, runner)
			if tc.name == "unknown" {
...
func TestADR_0281_SystemScopeRequiresIndependentCapability(t *testing.T) {
	t.Parallel()
	call := scopedBashCall("c", "go test ./...", "system")
	for _, tc := range []struct {
		name  string
		rules []governance.Rule
		allow bool
	}{
		{"bash alone is insufficient", []governance.Rule{{Tool: "Bash", Pattern: "go test*", Effect: governance.Allow}}, false},
		{"both allows", []governance.Rule{{Tool: "Bash", Pattern: "go test*", Effect: governance.Allow}, {Tool: bashSystemTempToolName, Effect: governance.Allow}}, true},
		{"capability deny dominates", []governance.Rule{{Tool: "Bash", Pattern: "go test*", Effect: governance.Allow}, {Tool: bashSystemTempToolName, Effect: governance.Allow}, {Tool: bashSystemTempToolName, Effect: governance.Deny}}, false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			runner := &fakeBashRunner{res: tool.CommandResult{Stdout: "ok"}}
			_, result := runScopedBash(t, tc.rules, call, runner)
...
// TestADR_0281_SystemTempCapabilityIsGlobalButNotBashAllow pins that the
// synthetic capability does not itself grant Bash, while its explicit allow is
// independent of the ordinary command pattern.
func TestADR_0281_SystemTempCapabilityIsGlobalButNotBashAllow(t *testing.T) {
	t.Parallel()
	systemCapability := governance.Rule{Tool: bashSystemTempToolName, Effect: governance.Allow}
	for _, tc := range []struct {
		command string
		allow   bool
	}{
		...
		t.Run(tc.command, func(t *testing.T) {
			...
			if tc.allow {
				rules = append(rules, governance.Rule{Tool: "Bash", Pattern: "go test*", Effect: governance.Allow})
			}
			_, result := runScopedBash(t, rules, scopedBashCall("c", tc.command, "system"), runner)
			if got := runner.command != ""; got != tc.allow {
...
func TestADR_0281_SystemTempApprovalDoesNotLeakPath(t *testing.T) {
	t.Parallel()
	runner := &fakeBashRunner{res: tool.CommandResult{Stdout: "ok"}}
	events, _ := runScopedBash(t, []governance.Rule{{Tool: "Bash", Pattern: "go test*", Effect: governance.Allow}}, scopedBashCall("c", "go test ./...", "system"), runner)
