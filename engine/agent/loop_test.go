...
func TestCancelMidToolThenResumeSucceeds(t *testing.T) {
	...
	r2 := e2.Run(context.Background(), sess, agent.EnvForWS(ws, nil), agent.RunRequest{Text: "second prompt"})
...
func TestCancelAwaitingApprovalThenResumeSucceeds(t *testing.T) {
	...
	r := e.Run(context.Background(), sess, agent.EnvForWS(ws, nil), agent.RunRequest{Text: "write a"})
...
func (a *argRecorder) Execute(_ context.Context, in session.ToolCall, _ tool.Environment) (session.ToolResult, error) {
	a.mu.Lock()
...
func TestPostToolUseHookNoMutationKeepsResult(t *testing.T) {
	...
	sess := newSession(t, session.Limits{})
	e := newEngine(agent.Deps{LLM: llm, Catalog: catalogWith(t, tl), Hooks: hooks})
	drain(e.Run(context.Background(), sess, agent.MemEnv("/ws"), agent.RunRequest{Text: "go"}))
