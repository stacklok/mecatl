...
func TestRoutingReasonPayloadEventSafeAllowlist(t *testing.T) {
	cases := []struct {
		...
		{"Jev transport miss passes", "jev-error", "jev-error"},
		{"Jev invalid response passes", "jev-invalid-response", "jev-invalid-response"},
		{"Jev low confidence passes", "jev-low-confidence", "jev-low-confidence"},
		{"Jev request limit passes", "jev-over-limit", "jev-over-limit"},
		{"Jev queue timeout passes", "jev-queue-timeout", "jev-queue-timeout"},
