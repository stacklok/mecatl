...
func TestTitleSeededOnceFromFirstGenuinePrompt(t *testing.T) {
	...
	evs := drain(e.Run(context.Background(), sess, ws, "Fix the flaky CI job"))
	...
	drain(e2.Run(context.Background(), sess, ws, "A completely different second prompt"))
...
func TestTitleEmptyForMultimodalOnlyPrompt(t *testing.T) {
	...
	r := e.RunContent(context.Background(), sess, ws, "", parts)
	drain(r)
...
func TestTitleSeededOnReopenRun(t *testing.T) {
	...
	drain(e.Run(context.Background(), sess, ws, "the original goal"))
