...
- **Versioned bounded reflection evidence materialization (ADR 0298)** — adds the storage-neutral `learning.MaterializeEvidence` protocol, immutable aggregate manifests, distinct selected-local and durable source coordinates, and closed no-work outcomes. `learning.Input` and `learning.EvidenceRef` gain manifest/protocol fields so pre-version references remain explicitly `reflection-evidence/legacy-v0`. Added plus Changed (breaking, pre-v1 minor).
