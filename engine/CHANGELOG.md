...
- **`tool.MemoryAttribution.ForceInstructionScan`** — a classification-only
  flag that forces instruction-shaped user-memory validation without changing
  persisted `Writer`, `Origin`, or `Source` provenance. Added public struct
  field: Added (minor).

- **`tool.MemoryRecord.Truncated`** — reports when a lifecycle store's retention
  cap has discarded older revisions for the record's key. Added public struct
  field: Added (minor).
...
- **`tool.ValidateMemoryWrite`, `tool.ValidateMemoryEntry`, and
  `tool.ValidateMemoryContent`** — removed zero-attribution validation shims.
  Callers must use `ValidateMemoryEntryWrite` or `ValidateMemoryContentWrite`
  and explicitly supply `MemoryAttribution`. Removed/breaking per
  COMPATIBILITY.md (pre-v1 minor bump).
