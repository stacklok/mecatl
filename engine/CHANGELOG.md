...
- **Versioned bounded reflection evidence materialization (ADR 0298)** — adds the storage-neutral `learning.MaterializeEvidence` protocol, immutable aggregate manifests, distinct selected-local and durable source coordinates, and closed no-work outcomes. Added (minor).
...
- **Versioned bounded reflection evidence materialization (ADR 0298)** — `learning.Input` now carries its aggregate `MaterializationManifest`; `learning.EvidenceRef` now carries the explicit protocol, manifest index, durable original coordinate, and aggregate digest needed to distinguish `reflection-evidence/v1` references from historical input-local `reflection-evidence/legacy-v0` ordinals; and `learning.MaterializeEvidence` now accepts a `context.Context` so full-source scans are cancellable. Adding fields to these exported structs and changing the function signature break external callers and unkeyed literals, so this is Changed/breaking (pre-v1 a minor bump).
