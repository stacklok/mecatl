...
- **`agent.Run.RetractPermissionAsk`** ([ADR 0290](../docs/adr/0290-session-correlation-and-affinity.md)) — lets a lease-owning host atomically withdraw one still-pending local permission ask without resolving it, emitting the matching retraction before cancellation while leaving an already-durable awaiting snapshot untouched for successor handoff. Added (minor).

- **`port.SessionIDHeaderName` and `port.ValidSessionIDHeaderValue`** ([ADR 0290](../docs/adr/0290-session-correlation-and-affinity.md)) — define the canonical `X-Mecatl-Session-ID` field name and exact, non-normalizing legal-value predicate for root server and client consumers. Added (minor); independently versioned provider modules retain their private ADR 0216 constants and validators until a later engine/provider release cycle.
