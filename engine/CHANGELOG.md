...
- **`tool.CommandStreamer`** (background-Bash feature, task 3) — a new OPTIONAL
  ...
  identifier changed).
- **`session.SubagentPayload.RoutingReason` / `session.ParallelPayload.RoutingReason` / `session.TeamMemberSpec.RoutingReason` and the `session.RoutingReason*` gate constants** (issue #397) — the three delegation-start events now carry a bounded, bare-metadata reason WHY the OPT-IN semantic model router did not classify a delegation: EMPTY on a routed hit, otherwise one of the gate constants (`RoutingReasonPinnedModel` / `RoutingReasonAgentDefPinned` / `RoutingReasonResume` / `RoutingReasonFork` / `RoutingReasonRouterDisabled` / `RoutingReasonBreakerOpen` / `RoutingReasonAborted`) or a classifier/composition miss string (`RouterMiss*`, `category-selector-empty`, …) passed through verbatim. This lets a UI distinguish router-absent from pinned-model from agent-def-pinned from classifier-failure from breaker-open — previously every miss/gate collapsed to empty `routed_*`. The reason is clamped at the emit site (`routingReasonPayload`, 200-rune cap) and is gauntlet-#7 safe (never the task prompt or classifier reasoning). Classified Added per COMPATIBILITY.md (new struct fields + constants are a minor bump). See ADR 0083.
