...
// `"` (NeutraliseDelegationResult's %q floor relies on a quoted value matching no marker, which is
...
// to start with a header can be erased entirely — is bounded by NeutraliseDelegationResult's
...
// NeutraliseFraming (surfaceAll) — the narrowing exists for NeutraliseDelegationResult.
...
// NeutraliseDelegationResult is NeutraliseFraming for the model-influenced text a harness-composed
...
// NeutraliseDelegationResult's %q floor.
...
const (
	...
	// Subagent/Team tool result and the Parallel join report. NeutraliseDelegationResult is the
...
func framingHeaderSurfaces(trimmed string) framingSurface {
	...
	// builds. A delegation result contains none of them, so NeutraliseDelegationResult skips
