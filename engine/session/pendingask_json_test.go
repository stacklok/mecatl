...
func TestPendingAskRunLocalPolicyHintsAreNotSerialized(t *testing.T) {
	b, err := json.Marshal(PendingAsk{AskID: "a1", Origin: ApprovalOriginPermission, ConfiguredAsk: true, FlooredConfiguredAllow: true})
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(string(b), "ConfiguredAsk") || strings.Contains(string(b), "FlooredConfiguredAllow") {
