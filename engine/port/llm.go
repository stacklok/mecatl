...
const (
	...
	ChunkPhase
	// ChunkProviderRoute carries the DOWNSTREAM provider slug that actually served
	// a routed request on Text (issue #480). It is emitted by the OpenAI adapter
	// ONLY for the openrouter registry entry (which arms the X-OpenRouter-Metadata
	// header and parses the returned openrouter_metadata), at the terminal
	// response.completed event. Like ChunkPhase it is DISPLAY-ONLY and provider-
	// private in CONTENTS: the loop relays it verbatim onto an EvProviderRoute
	// event for clients, never branches on it, never replays it, never anchors
	// TTFT or feeds usage. It is absent on a cache hit (OpenRouter strips
	// openrouter_metadata from cached responses) — the loop simply emits nothing.
	// The STRUCTURE is neutral (one opaque slug per turn); mecatl's "provider"
	// stays the wire adapter — this is the DOWNSTREAM inference provider OpenRouter
	// routed to. Same discipline as ChunkPhase.
	ChunkProviderRoute
)
