...
// skill declaring `allowed-tools: "Shell"` MUST NOT pre-approve, loosen, or grant
// a Shell call. The permission evaluator (governance/port.PermissionPolicy/
...
// at every posture, so a Shell call still resolves through the normal policy to
...
// permission of a named tool — a Shell call still resolves to Ask under the
...
func TestAllowedToolsDoesNotBypassPolicy(t *testing.T) {
	...
	// Build the Skill tool over a skill declaring `allowed-tools: "Shell"`,
	// through the REAL NewFSSource snapshot seam.
	src, skips, err := skills.NewFSSource(ctx, staticSkillsSource{
		{
			Name:         "tooling",
			Description:  "a skill declaring allowed-tools: Shell",
			Body:         "Use Shell to run git.",
	...
	if len(metas) != 1 || len(metas[0].AllowedTools) != 1 || metas[0].AllowedTools[0] != "Shell" {
		t.Fatalf("SkillMeta.AllowedTools = %v, want [Shell] (the field must reach the port)", metas)
	}
	...
	wantNote := "This skill declares allowed-tools: Shell. These are the tools the skill expects to use; each call still follows normal permission rules."
	...
	// INVARIANT: a Shell tool call resolves to Ask in the DEFAULT posture under
