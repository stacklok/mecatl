...
// AC1.1: one credential registers both protocol entries (ADR 0343 decision 2,
...
// TestADR_0343_OpenRouterAnthropicBaseDerivation pins AC1.2. The SDK appends
...
func TestADR_0343_OpenRouterAnthropicBaseDerivation(t *testing.T) {
	tests := []struct {
		name, in, want string
	}{
...
// TestADR_0343_OpenRouterAnthropicListsAnthropicOnly pins AC1.5: OpenRouter's
// Anthropic surface does not serve non-Anthropic models, so advertising them
// would offer ids that cannot execute.
func TestADR_0343_OpenRouterAnthropicListsAnthropicOnly(t *testing.T) {
...
// TestADR_0343_OpenRouterResponsesEntryByteIdentical pins AC1.6: adding a sibling
// must not disturb the existing entry's endpoint.
func TestADR_0343_OpenRouterResponsesEntryByteIdentical(t *testing.T) {
	reg, err := buildProviderRegistry(orCfg(), noEnv)
	if err != nil {
		t.Fatalf("build registry: %v", err)
	}
...
// TestADR_0343_NonClaudeDefaultPrecedenceUnchanged pins AC2.3: the preference is
// narrow. A non-Anthropic default must resolve exactly as it does today.
func TestADR_0343_NonClaudeDefaultPrecedenceUnchanged(t *testing.T) {
...
// TestADR_0343_ExplicitDefaultProviderOverridesCachingPreference pins AC2.4: the
// preference applies to a DEFAULT, never over an operator who named the provider.
func TestADR_0343_ExplicitDefaultProviderOverridesCachingPreference(t *testing.T) {
	cfg := orCfg()
	cfg.Model = "anthropic/claude-opus-4.8"
...
// TestADR_0343_PromptCachedProjectedOncePerModel pins AC2.5: the picker signal is
...
func TestADR_0343_PromptCachedProjectedOncePerModel(t *testing.T) {
	reg, err := buildProviderRegistry(orCfg(), noEnv)
	if err != nil {
		t.Fatalf("build registry: %v", err)
	}
	...
	if got := promptCacheSource(reg, orCfg(), providerOpenRouterAnthropic); !strings.Contains(got, "unconditional") {
		t.Errorf("source for the Messages entry = %q, want it to say unconditional", got)
	}
}

// TestADR_0343_PromptCachedFalseWithoutDialect pins the case the incident came
// from: an endpoint with NO dialect reports false, and --no-prompt-cache forces
// false everywhere so the signal matches the wire.
func TestADR_0343_PromptCachedFalseWithoutDialect(t *testing.T) {
	// A base-URL override drops the openrouter entry to CacheDialectNone.
	cfg := orCfg()
	...
	if reg.promptCached(providerOpenRouter, "anthropic/claude-opus-4.8") {
		t.Error("an overridden base URL has no dialect, so prompt_cached must be false")
	}
	if got := promptCacheSource(reg, cfg, providerOpenRouter); !strings.Contains(got, "NOT cache") {
		t.Errorf("source = %q, want it to name the consequence for an Anthropic model", got)
	}
...
// TestADR_0343_AnthropicFamilyClassifier pins the narrow matcher behind the
// default preference: it keys on the vendor namespace and the product name,
// never a substring anywhere in the id.
func TestADR_0343_AnthropicFamilyClassifier(t *testing.T) {
...
// TestUnifiedPromptCache_Scenario2_GatewayClaudeDefaultPrefersAnthropicEntry
// pins AC2.2. This is the reported incident's exact shape:
// preferredDefaultProvider ranks intent-driven entries by SORTED ORDER, and
// "toolhive" sorts before "toolhive-anthropic", so a gateway-only deployment
// defaulted onto the surface that emits no breakpoints.
func TestUnifiedPromptCache_Scenario2_GatewayClaudeDefaultPrefersAnthropicEntry(t *testing.T) {
	cfg := Config{
	...
	if got := reg.Default(); got != providerToolhiveAnthropic {
		t.Errorf("gateway default for a Claude model = %q, want %q", got, providerToolhiveAnthropic)
	}

	// Sort order must still decide when the model is NOT Anthropic-family: the
	// preference is narrow by design.
	plain := Config{ToolhiveLLMBaseURL: "http://127.0.0.1:14000/v1", Model: "openai/gpt-5"}
	regPlain, err := buildProviderRegistry(plain, noEnv)
	if err != nil {
		t.Fatalf("build registry (non-Claude): %v", err)
	}
	if got := regPlain.Default(); got != providerToolhive {
		t.Errorf("gateway default for a non-Claude model = %q, want the unchanged %q", got, providerToolhive)
	}
}

// TestADR_0343_LeakGuardBothDirectionsStillHolds pins AC4.4. ADR 0100's dialect
// table is deliberately UNTOUCHED by ADR 0343 — routing replaced the declaration
...
func TestADR_0343_LeakGuardBothDirectionsStillHolds(t *testing.T) {
