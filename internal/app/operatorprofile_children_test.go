...
func rememberProfile(t *testing.T, ctx context.Context, store tool.MemoryStore, entry tool.MemoryEntry) {
	t.Helper()
...
func TestUserFacingChildRolesInjectFreshVolatileOperatorProfile(t *testing.T) {
	for _, role := range []string{"task", "parallel", "member:lead"} {
		t.Run(role, func(t *testing.T) {
			store := memmemory.New()
			rememberProfile(t, context.Background(), store, tool.MemoryEntry{Key: "user/output/language", Value: "Prefer French by default."})
			...
			rememberProfile(t, context.Background(), store, tool.MemoryEntry{Key: "user/output/language", Value: "Prefer Japanese by default."})
...
func TestOperatorProfileSourceIsCallerScoped(t *testing.T) {
	...
	rememberProfile(t, alice, store, tool.MemoryEntry{Key: "user/preference", Value: "alice-profile"})
	rememberProfile(t, bob, store, tool.MemoryEntry{Key: "user/preference", Value: "bob-profile"})
