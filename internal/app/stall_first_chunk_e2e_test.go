...
func TestStallFirstChunkTimeoutTerminatesRunAsError(t *testing.T) {
	...
	sess := session.New("stall-82", session.ModeDefault, session.EnvironmentRef{Kind: session.EnvKindLocal, ID: "/ws", Revision: "in-tree-v1"}, session.Limits{MaxTurns: 5}, time.Unix(0, 0))
