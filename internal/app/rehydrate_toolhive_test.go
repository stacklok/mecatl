...
// two-Build (the TestApproveAfterRestartE2E precedent), for both protocol-
// specific ToolHive provider identities:
//
//  1. built1: the ToolHive LLM gateway config is detected and the proxy
//     responds (a mock transport, never a real port). A session selects one
//     ToolHive provider explicitly; one turn runs successfully.
...
//     persisted ToolHive selector with "unknown or unavailable provider"
...
func TestToolhiveRehydrationWithProxyDownE2E(t *testing.T) {
	for _, providerID := range []string{providerToolhive, providerToolhiveAnthropic} {
		t.Run(providerID, func(t *testing.T) {
			testToolhiveRehydrationWithProxyDownE2E(t, providerID)
		})
	}
}

func testToolhiveRehydrationWithProxyDownE2E(t *testing.T, providerID string) {
	...
	sess, err := built1.Service.CreateSessionWithProvider(ctx, session.ModeDefault, session.Limits{},
		server.ProviderSelector{ProviderID: providerID, ModelID: "claude-sonnet-4-6"})
	if err != nil {
		built1.Close()
	...
	if err != nil {
		if strings.Contains(err.Error(), "unknown or unavailable provider") {
			t.Fatalf("StartRunContent rejected the persisted %s selector as unknown/unavailable "+
				"(register-on-intent must survive a down proxy): %v", providerID, err)
		}
