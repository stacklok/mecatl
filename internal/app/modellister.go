...
// family (ADR 0346). OpenRouter's Anthropic Messages surface does not serve
...
func (l openRouterLister) ListModels(ctx context.Context) ([]modelEntry, error) {
	...
	for _, m := range raw {
		out = append(out, modelEntry{
			...
			// has no per-model max_tokens resolver), but since ADR 0346 the
...
func projectModelEntry(reg *providerRegistry, providerID string, m modelEntry) *mecatlv1.ModelInfo {
	...
	// ADR 0346: computed ONCE here, the same single-projection discipline
