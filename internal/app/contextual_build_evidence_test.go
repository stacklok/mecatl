...
func TestBuildDoesNotDiscloseEvidenceWithoutReadPermission(t *testing.T) {
	for _, effect := range []string{"deny", "ask"} {
		t.Run(effect, func(t *testing.T) {
			...
			built.Service.FinishRun(sess.ID, run)
			if provider.reviewCalls != maxReviewAttempts || strings.Contains(provider.reviewInput, secret) {
				t.Fatalf("review calls=%d want=%d secret disclosed=%t", provider.reviewCalls, maxReviewAttempts, strings.Contains(provider.reviewInput, secret))
			}
		})
	}
}
