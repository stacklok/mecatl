...
const (
	...
	// by the same OpenRouter credential (ADR 0343), the sibling of
...
type providerEntry struct {
	...
	// (ADR 0343): Anthropic's cache_control is native to Messages, so those
...
type providerRegistry struct {
	...
	// (ADR 0343). A CLOSURE over the build's Config so the picker projection
...
func buildProviderRegistryContext(ctx context.Context, cfg Config, detect envDetector) (*providerRegistry, error) {
	...
	if key := providerKey(cfg.OpenRouterKey, providerOpenRouter, detect); key != "" {
		...
		// openrouter-anthropic (ADR 0343): the SAME credential also reaches
	...
	// BUILD-ONCE posture line (ADR 0343 decision 7). Logged here, where the
...
func newOpenAICompatEntry(cfg Config, id, key, baseURL string, extra ...openai.Option) providerEntry {
	...
	construct := func(effort string, caps port.ProviderCapabilities) port.LLMProvider {
		...
		// Prompt-cache key salt (ADR 0343): the SAME per-process value on every
		...
		// Redirect refusal (ADR 0343 Scenario 5, applying ADR 0334's existing
...
func resolveDefaultModel(cfg Config, reg *providerRegistry) (providerID, modelID string) {
	...
	// ADR 0343 decision 3: never DEFAULT onto a protocol sibling that cannot cache
...
// Anthropic-Messages entry registered from the SAME credential or intent. Both
// pairs are registered together, so a present key implies the sibling exists
// whenever that identity was configured.
var anthropicProtocolSibling = map[string]string{
	providerOpenRouter: providerOpenRouterAnthropic,
	providerToolhive:   providerToolhiveAnthropic,
}
...
//
// The model id is carried VERBATIM. For the OpenRouter pair that is exact: both
// surfaces take the same OpenRouter-namespaced id. For the ToolHive pair it
// ASSUMES the gateway lists one id shape across both of its surfaces; if a
// gateway ever diverges there, the model simply will not resolve on the sibling
// and the existing default-model validation surfaces it — a loud failure, never
// a silent misroute onto an uncached path.
//
