...
func TestMigrationReceiptSchemaAdmission(t *testing.T) {
	crd, _ := loadExecutionEnvironmentCRD(t)
	field := crd.Spec.Versions[0].Schema.OpenAPIV3Schema.Properties["status"].Properties["lastMigrationFromSchema"]
	var internal apiextensions.JSONSchemaProps
	if err := apiextensionsv1.Convert_v1_JSONSchemaProps_To_apiextensions_JSONSchemaProps(&field, &internal, nil); err != nil {
		t.Fatal(err)
	}
	...
	for _, value := range []any{int64(0), int64(1), int64(-1), int64(2), "0"} {
		errs := apiservervalidation.ValidateCustomResource(nil, value, validator)
		valid := value == int64(0) || value == int64(1)
		if (len(errs) == 0) != valid {
			t.Fatalf("receipt source schema %v: valid=%t errors=%v", value, valid, errs)
		}
	}
}
...
func TestExecutionEnvironmentCRDPreservesControllerStatus(t *testing.T) {
	...
	obj := map[string]any{"apiVersion": "execution.mecatl.dev/v1alpha1", "kind": "ExecutionEnvironment", "metadata": map[string]any{"name": "env"}, "spec": map[string]any{}, "status": map[string]any{
		...
		"migrationOperation": map[string]any{"id": "migration", "fromSchema": int64(1), "expectedPodUID": "pod-uid", "expectedPVCUID": "pvc-uid"}, "lastMigrationOperationID": "migration", "lastMigrationFromSchema": int64(0),
