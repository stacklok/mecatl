...
func TestPDFArtifactServiceScaffoldFailsClosed(t *testing.T) {
	...
	if _, _, err := svc.DownloadPdf(context.Background(), sess.ID, "opaque-id"); !errors.Is(err, server.ErrPDFArtifactsUnavailable) {
	...
	if _, _, err := svc.DownloadPdf(context.Background(), "unknown", "opaque-id"); !errors.Is(err, server.ErrNotFound) {
