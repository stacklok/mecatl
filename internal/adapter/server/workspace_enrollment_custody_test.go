...
type custodyRecorder struct {
	...
	recoverWith func(context.Context, session.SessionID) (brokercontract.Attachment, error)
	recovers    int
}
...
func (b *custodyBroker) AttachSession(ctx context.Context, id session.SessionID) (brokercontract.Attachment, brokercontract.AttachOutcome, error) {
	if _, outcome, err := b.enrollmentBroker.AttachSession(ctx, id); err != nil {
		return nil, outcome, err
	}
	if b.wrapped == nil || b.wrapped.enrollmentAttachment != b.attachment {
		b.wrapped = &custodyAttachment{enrollmentAttachment: b.attachment, rec: b.rec}
	}
	return b.wrapped, brokercontract.AttachReattached, nil
}

func (b *custodyBroker) AttachSessionExpectedBinding(ctx context.Context, id session.SessionID, _ session.ExternalBinding) (brokercontract.Attachment, brokercontract.AttachOutcome, error) {
	b.rec.mu.Lock()
...
func (f *custodyFixture) begin(t *testing.T) WorkspaceEnrollmentProjection {
	...
	f.broker.attachment.result = brokercontract.WorkspaceEnrollmentResult{Ref: started.Ref, Status: brokercontract.WorkspaceEnrollmentConnected, Catalogue: catalogue}
	f.rec.mu.Lock()
	f.rec.events = nil
	f.rec.mu.Unlock()
...
func replacementBroker(t *testing.T) func(context.Context, session.SessionID) (brokercontract.Attachment, error) {
	t.Helper()
	b2 := testBrokerRuntime(t)
	t.Cleanup(func() { _ = b2.Close() })
	return func(ctx context.Context, id session.SessionID) (brokercontract.Attachment, error) {
...
func (*custodyFixture) ownerContext(ctx context.Context) context.Context {
