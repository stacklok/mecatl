...
func TestADR_0280_CreateSessionAcceptsOnlyDefaultOrNoFS(t *testing.T) {
	h := NewHarnessServer(newADR0280Service(t))
...
func TestADR_0280_PublicHarnessContractContainsNoFilesystemPaths(t *testing.T) {
...
func TestADR_0280_DiscoveryIsSessionScopedAndOwnerAuthorized(t *testing.T) {
	svc := newADR0280Service(t)
	h := NewHarnessServer(svc)
	created, err := h.CreateSession(context.Background(), &mecatlv1.CreateSessionRequest{})
	if err != nil {
...
func TestADR_0280_ClearSessionCreatesEmptyInheritedSuccessor(t *testing.T) {
	svc := newADR0280Service(t)
	h := NewHarnessServer(svc)
	created, err := h.CreateSession(context.Background(), &mecatlv1.CreateSessionRequest{})
	if err != nil {
