...
func TestSDKRunControls_Scenario3_ResolveOrdinaryAsk(t *testing.T) {
	...
	svc := newAskingService(t, store, permstore.New(), &ran,
		mockllm.New(mockllm.ToolCallTurn(call("w1", "Write", `{"path":"a.go"}`)), mockllm.TextTurn("done")), true)
...
func TestADR_0346_StaleControlCannotMutateSuccessor(t *testing.T) {
	svc := newService(t, mockllm.New(mockllm.TextTurn("first"), mockllm.ChunksTurn(blockingChunks()...)), allowRules())
	...
	defer func() { second.Cancel(); drainRun(t, second); svc.FinishRun(sess.ID, second) }()
