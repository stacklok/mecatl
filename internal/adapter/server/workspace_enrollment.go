...
//
//nolint:gocyclo // one ordered begin/recover/build transaction over the enrollment lifecycle states.
...
//nolint:gocyclo // one ordered load/reopen/rebind-or-recover transaction guarding the enrollment target.
func (s *Service) recoveringWorkspaceEnrollmentTarget(ctx context.Context, id session.SessionID) (*session.Session, brokercontract.WorkspaceEnrollmentAttachment, func(), enrollmentRecovery, error) {
	...
	if sess.State == session.StateCompleted {
		if err := sess.Reopen(); err != nil {
			return sess, nil, nil, recovery, fmt.Errorf("%w: reopen workspace enrollment session", ErrFailedPrecondition)
		}
		if err := s.saveSession(ctx, sess); err != nil {
			return sess, nil, nil, recovery, fmt.Errorf("%w: persist workspace enrollment reopen", ErrInternal)
		}
	}
	if !s.brokerConfigured() || sess.State != session.StateIdle || sess.Conversation == nil {
		return nil, nil, nil, recovery, fmt.Errorf("%w: workspace enrollment requires an idle session", ErrFailedPrecondition)
	}
...
//nolint:gocyclo // one ordered stage transaction over custody eligibility and persistence.
func (s *Service) stageInitialCustody(ctx context.Context, sess *session.Session, enroller brokercontract.WorkspaceEnrollmentAttachment, pending session.PendingWorkspaceEnrollment) error {
...
//nolint:gocyclo // one ordered recovery transaction over generation/custody/rebind outcomes.
