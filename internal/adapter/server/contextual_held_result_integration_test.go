...
func TestGRPCHeldResultIsPrivateUntilExactRelease(t *testing.T) {
	...
	for {
		...
		httpResponse, err := http.Post(httpServer.URL+"/v1/sessions/"+string(id)+"/controls/resolve-ask", "application/json", strings.NewReader(invalidHTTP))
		if err != nil {
			t.Fatal(err)
		}
		...
		httpServer.Close()
		if readErr != nil || httpResponse.StatusCode != http.StatusBadRequest || !strings.Contains(string(httpBody), `"code":"invalid_argument"`) {
...
func TestServiceRestartCannotReleaseOrRerunLostHeldResult(t *testing.T) {
	...
	if parked.RunID() == "" {
		t.Fatal("persisted awaiting session has no run ID")
	}
	invalidBody := `{"ask_id":"` + askID + `","review_id":"` + reviewID + `","guardrail_kind":"` + string(approvalKind) + `","verdict":"future_verdict","expected_run_id":"` + parked.RunID() + `"}`
	resp, err := http.Post(httpServer.URL+"/v1/sessions/"+string(id)+"/controls/resolve-ask", "application/json", strings.NewReader(invalidBody))
	if err != nil {
		t.Fatal(err)
	}
	...
	if resp.StatusCode != http.StatusBadRequest || !strings.Contains(string(responseBody), `"code":"invalid_argument"`) {
