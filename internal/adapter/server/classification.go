...
var serviceAccessTable = map[string]ClassificationEntry{
	...
	"UploadArtifact":            {KindCallerOwned, "checks exact session ownership before the PDF upload seam can accept any bytes"},
	"DownloadArtifact":          {KindCallerOwned, "checks exact session ownership before the PDF download seam can release any bytes"},
