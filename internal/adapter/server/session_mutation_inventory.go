...
var sessionMutationInventory = map[string]SessionMutationEntry{
	...
	"resolveAuthorizationWithFailureLocked": {SessionMutationLeaseProven, "shared terminal-resolution body for resolveAuthorizationLocked and the granted-continuation argument-rejection path inside continueGrantedAuthorizationLocked, both already holding runEntryMu and the acquired session lease"},
	...
	"recoveringWorkspaceEnrollmentTarget":   {SessionMutationLeaseProven, "shared body behind workspaceEnrollmentTarget: reached only after the same run-entry and session lease acquisition, plus brokerMu for the id before any broker rebind/recovery save"},
