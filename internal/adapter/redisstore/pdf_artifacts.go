...
// deletion. The active key uses Redis's TTL clock, avoiding pod clock skew.
// The metadata hash remains after the session is deleted.
...
func (st *Store) FinishPDFDeletion(ctx context.Context, id session.SessionID) error {
	...
	defer release()
	if err := client.Del(ctx, pdfArtifactKey(id)).Err(); err != nil {
		return errors.New("pdf artifact: metadata unavailable")
	}
	if err := client.ZRem(ctx, pdfDeletionOutboxKey, string(id)).Err(); err != nil {
		return errors.New("pdf artifact: metadata unavailable")
	}
	return nil
}
