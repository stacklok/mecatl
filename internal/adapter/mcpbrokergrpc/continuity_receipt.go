...
func (s *Server) finishContinuityReceipt(key continuityReceiptKey, receipt *continuityReceipt, stage *brokerv1.StageCredentialCustodyResponse, recoverResp *brokerv1.RecoverCredentialAttachmentResponse, err error, expiresAt time.Time) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	recoverResp = cloneRecoverReceipt(recoverResp)
	retained := 0
	if stage != nil {
		retained = proto.Size(stage)
	}
	if recoverResp != nil {
		retained = proto.Size(recoverResp)
	}
	accepted := retained <= receipt.reservation
	if !accepted {
		stage, recoverResp, err = nil, nil, continuityUnavailable()
		retained = 0
	}
	s.resizeContinuityReceiptLocked(receipt, retained)
	receipt.stage = stage
	receipt.recover = recoverResp
	receipt.err = err
...
func continuityDigest(parts ...[]byte) [sha256.Size]byte {
	...
	for _, part := range parts {
		var length [4]byte
		binary.BigEndian.PutUint32(length[:], uint32(len(part))) // #nosec G115 -- continuity guard/assertion fields, not attacker-sized payloads.
