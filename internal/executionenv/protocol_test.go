...
func TestGrantRoundTripAndExactBindings(t *testing.T) {
	...
	if _, err := v.Verify(token, GrantExpectation{Client: claims.Client, OwnerHash: "owner", BindingID: claims.BindingID, RunID: claims.RunID, ClaimID: claims.ClaimID, Environment: claims.Environment, Epoch: 7, GrantGeneration: claims.GrantGeneration, Operation: OpFileRead}); err != nil {
	...
	for name, mutate := range map[string]func(*GrantExpectation){
		...
		t.Run(name, func(t *testing.T) {
			e := GrantExpectation{Client: claims.Client, OwnerHash: claims.OwnerHash, BindingID: claims.BindingID, RunID: claims.RunID, ClaimID: claims.ClaimID, Environment: claims.Environment, Epoch: claims.Epoch, GrantGeneration: claims.GrantGeneration, Operation: OpFileRead}
			mutate(&e)
...
func TestGrantRejectsExpiryRevocationAndUnknownFields(t *testing.T) {
	...
	now := time.Unix(1_800_000_000, 0).UTC()
	c := GrantClaims{KeyID: "k1", Issuer: "i", Audience: "a", Client: "spiffe://c", OwnerHash: "o", BindingID: "b", RunID: "run", ClaimID: "claim", Environment: EnvironmentRef{ID: "e", Revision: "r"}, Epoch: 1, GrantGeneration: 1, Operations: []Operation{OpAttach}, NotBefore: now.Add(-time.Minute), ExpiresAt: now.Add(-time.Second), Nonce: "n"}
	...
	if _, err := v.Verify(tok, GrantExpectation{Client: c.Client, OwnerHash: c.OwnerHash, BindingID: c.BindingID, RunID: c.RunID, ClaimID: c.ClaimID, Environment: c.Environment, Epoch: 1, GrantGeneration: c.GrantGeneration, Operation: OpAttach}); err == nil {
