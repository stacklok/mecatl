...
func NewLibkrunBackend(ownedArtifactDir ...string) *LibkrunBackend {
	...
	backend := &LibkrunBackend{userNamespaceUID: int(workload.UID), userNamespaceGID: int(workload.GID)}
...
func (b *LibkrunBackend) Start(ctx context.Context, launch GoMicroVMLaunch) (GoMicroVMInstance, error) {
	...
	return &libkrunInstance{vm: vm, launch: launch, ownedArtifactsRoot: ownedArtifactsRoot}, nil
}
...
func (i *libkrunInstance) Remove(ctx context.Context) error {
	return errors.Join(i.vm.Remove(ctx), os.RemoveAll(i.ownedArtifactsRoot))
}
