...
Add a read-only `/session` details surface using the active session metadata and existing clipboard abstraction. Render full IDs reversibly/safely, copy exact valid UTF-8 IDs, use ADR-0285's fixed ordinary handle in the width-safe header, and keep every rebind path authoritative. Update help/docs/user-docs; generated docs remain orchestrator-owned.
