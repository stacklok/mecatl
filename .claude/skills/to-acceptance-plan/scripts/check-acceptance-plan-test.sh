...
sed_in_place '/^\*\*Contract:\*\*/d' "$root/missing-contract.md"
sed_in_place 's/\*\*Contract:\*\* human-reviewed\/v1/\*\*Contract:\*\* human-reviewed\/v2/' "$root/invalid-contract.md"
sed_in_place '/Tool schemas/d' "$root/missing-category.md"
sed_in_place 's/None — no tool change\./None/' "$root/bare-none.md"
sed_in_place 's/None — no operator change\./TBD/' "$root/placeholder.md"
sed_in_place 's/\*\*Delivery:\*\* Split/\*\*Delivery:\*\* Later/' "$root/bad-delivery.md"
sed_in_place 's/\*\*Status:\*\* draft/\*\*Status:\*\* done/' "$root/bad-status.md"
sed_in_place 's/\*\*Status:\*\* draft/\*\*Status:\*\* proposed/' "$root/proposed-unchecked.md"
sed_in_place 's/None — the fixture leaves no decision for a human\./- [ ] Choose the fixture behavior./' "$root/proposed-unchecked.md"
sed_in_place '/^## Human decisions$/,/^## Interface contract$/ { /^## Human decisions$/d; /^None — the fixture leaves no decision for a human\.$/d; }' "$root/missing-human.md"
sed_in_place 's/None — the fixture leaves no decision for a human\./None — <rationale>/' "$root/placeholder-human.md"
sed_in_place 's/None — the fixture leaves no decision for a human\./- [x] Choose the fixture behavior./' "$root/checked-without-decision.md"
sed_in_place 's/None — no operator change\./Adds --fixture operator configuration./' "$root/combined-material-category.md"
...
- AC2.1: The second fixture is rejected for Combined delivery.
  - verify: inspection — shell fixture
PLAN
sed_in_place '/\*\*Expected tasks:\*\* 1/d' "$root/combined-missing-expected-tasks.md"
sed_in_place 's/\*\*Expected tasks:\*\* 1/\*\*Expected tasks:\*\* 2/' "$root/combined-wrong-expected-tasks.md"
sed_in_place '/\*\*Combined rationale:\*\*/d' "$root/combined-missing-rationale.md"
sed_in_place 's/\*\*Combined rationale:\*\*.*/\*\*Combined rationale:\*\* TBD/' "$root/combined-placeholder-rationale.md"
